...
DefineTests[
	RequireResources,
	{
		...
		Test["Auto-generate a container resource for a water resource:",
			...
			Variables :> {protocol},
			Stubs:>{
				ProcedureFramework`Private`$ExperimentWaterPreparation = False
			}
		],
...
DefineTests[fulfillableResourceQ,
	{
		...
		Example[{Additional, "Do not check resources that are simulated (backward compatible version, remove after SM is dead):"},
			fulfillableResourceQ[
				{Resource[<|Sample -> Object[Sample, "id:J8AY5jDZxZ8B"], Type -> Object[Resource, Sample]|>], Resource[<|Sample -> Object[Container, Vessel, "id:Q0ht2lIIr0CX"], Type -> Object[Resource, Sample]|>], Resource[ <|Operator -> Model[User, Emerald, Operator, "Baseline"], Time -> Quantity[10, "Minutes"],  Type -> Object[Resource, Operator]|>], Resource[  <|Operator -> Model[User, Emerald, Operator, "Baseline"],    Time -> Quantity[0, "Minutes"],Type -> Object[Resource,Operator]|>], Resource[<|Operator -> Model[User, Emerald,      Operator, "Baseline"], Time -> Quantity[30, "Minutes"], Type -> Object[Resource, Operator]|>], Resource[<|Operator -> Model[User, Emerald,      Operator, "Baseline"],    Time -> Quantity[1, "Hours"], Type -> Object[Resource, Operator]|>], Resource[<|Operator -> Model[User, Emerald, Operator, "Baseline"], Time -> Quantity[15, "Minutes"], Type -> Object[Resource, Operator]|>]},
				Cache -> {
			...
			fulfillableResourceQ[
				{
					...
					Resource[<|Operator -> Model[User, Emerald, Operator, "Baseline"], Time -> Quantity[10, "Minutes"],  Type -> Object[Resource, Operator]|>],
					Resource[<|Operator -> Model[User, Emerald, Operator, "Baseline"], Time -> Quantity[0, "Minutes"],Type -> Object[Resource,Operator]|>],
					Resource[<|Operator -> Model[User, Emerald, Operator, "Baseline"], Time -> Quantity[30, "Minutes"], Type -> Object[Resource, Operator]|>],
					Resource[<|Operator -> Model[User, Emerald, Operator, "Baseline"], Time -> Quantity[1, "Hours"], Type -> Object[Resource, Operator]|>],
					Resource[<|Operator -> Model[User, Emerald, Operator, "Baseline"], Time -> Quantity[15, "Minutes"], Type -> Object[Resource, Operator]|>]
				},
				Simulation->Simulation[
					{
...
DefineTests[
	ConsolidationInstances,
	{
		...
		Test["For a Model[Sample] request with a count, returns a list with possible samples which can be consolidated to fulfill the resource:",
			ConsolidationInstances[
			...
			Association[
				"PossibleSamples" -> {ObjectP[Object[Sample]], ObjectP[Object[Sample]], ObjectP[Object[Sample]]},
				"SamplesAmounts" -> {OrderlessPatternSequence[1,2,3]},
				"UserOwned" -> {False, False, False},
				"RequestedModel" -> ObjectP[Model[Sample, "Ibuprofen tablets 500 Count"]]
			],
			Stubs:>{
				$DeveloperSearch=True,
	...
	SymbolSetUp :> (
		Module[{objs, existingObjs},
			objs = {
				Object[Sample, "Test ibuprofen sample 1 for ConsolidationInstances unit tests" <> $SessionUUID],
				Object[Sample, "Test ibuprofen sample 2 for ConsolidationInstances unit tests" <> $SessionUUID],
				Object[Sample, "Test ibuprofen sample 3 for ConsolidationInstances unit tests" <> $SessionUUID],
				Object[Sample, "Test ibuprofen sample 4 for ConsolidationInstances unit tests" <> $SessionUUID],
				Object[Container, Vessel, "Test ibuprofen container 1 for ConsolidationInstances unit tests" <> $SessionUUID],
				Object[Container, Vessel, "Test ibuprofen container 2 for ConsolidationInstances unit tests" <> $SessionUUID],
				Object[Container, Vessel, "Test ibuprofen container 3 for ConsolidationInstances unit tests" <> $SessionUUID],
				Object[Container, Vessel, "Test ibuprofen container 4 for ConsolidationInstances unit tests" <> $SessionUUID],
				Object[Container, Bench, "Test bench for ConsolidationInstances unit tests" <> $SessionUUID]
			};
			existingObjs = PickList[objs, DatabaseMemberQ[objs]];
			EraseObject[existingObjs, Force -> True]
		];
		Block[{$DeveloperUpload = True},
			Module[{testBench, sample1, sample2, sample3, sample4, container1, container2, container3, container4},
				testBench = Upload[
					<|
						Type -> Object[Container, Bench],
						Model -> Link[Model[Container, Bench, "The Bench of Testing"], Objects],
						Name -> "Test bench for ConsolidationInstances unit tests" <> $SessionUUID
					|>
				];
				{
					container1,
					...
					FastTrack -> True,
					Name -> {
						"Test ibuprofen container 1 for ConsolidationInstances unit tests" <> $SessionUUID,
						"Test ibuprofen container 2 for ConsolidationInstances unit tests" <> $SessionUUID,
						"Test ibuprofen container 3 for ConsolidationInstances unit tests" <> $SessionUUID,
						"Test ibuprofen container 4 for ConsolidationInstances unit tests" <> $SessionUUID
					}
				];
				{
					sample1,
					...
					Name -> {
						"Test ibuprofen sample 1 for ConsolidationInstances unit tests" <> $SessionUUID,
						"Test ibuprofen sample 2 for ConsolidationInstances unit tests" <> $SessionUUID,
						"Test ibuprofen sample 3 for ConsolidationInstances unit tests" <> $SessionUUID,
						"Test ibuprofen sample 4 for ConsolidationInstances unit tests" <> $SessionUUID
					},
					InitialAmount -> {
						400 Unit,
						3 Unit,
