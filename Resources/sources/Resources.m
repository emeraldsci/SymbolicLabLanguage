		...
		With[
			{
				...
				approximateTransfer = resourceSpec[ApproximateTransfer]
			},
			...
			BoxForm`ArrangeSummaryBox[
				...
				sampleIcon[],
				{
					...
					If[Not[MissingQ[approximateTransfer]],
						BoxForm`SummaryItem[{"ApproximateTransfer: ", approximateTransfer}],
						Nothing
					]
				},
				{},
	...
	prepareTests[in_] := Module[{resourceType, trimmedPattern, presentKeys, selectedPattern, allowedKeys, tests},
		...
		tests = Switch[resourceType,
			...
			TypeP[Object[Resource, Sample]],
			{
				...
				{
					"If a Resource is requesting water AND $ExperimentWaterPreparation is NOT True, then a container must also be specified:",
					If[MatchQ[Lookup[in,Sample],ListableP[WaterModelP]]&&MatchQ[ProcedureFramework`Private`$ExperimentWaterPreparation,Except[True]],
						MemberQ[presentKeys,Container],
						True
					],
					True
				...
				{
					"If a Resource is requesting water AND $ExperimentWaterPreparation is True, then an empty container (model OR object with no contents) must also be specified:",
					If[MatchQ[Lookup[in,Sample],ListableP[WaterModelP]]&&MatchQ[ProcedureFramework`Private`$ExperimentWaterPreparation,True],
						And[
							MemberQ[presentKeys,Container],
							Or[
								MatchQ[Lookup[in, Container],{ObjectP[Model[Container]]..}|ObjectP[Model[Container]]],
								MemberQ[ToList[Lookup[in, Container]],ObjectP[Object[Container]]]&&MatchQ[Download[Cases[ToList[Lookup[in,Container]],ObjectP[Object[Container]]],Contents],{{}..}]
							]
						],
						True
					],
...
$EquivalentInstrumentModelLookup = Map[
	...
	]&,
	{
		...
		(* IQ 7010 is an equivalent for Integral 3, but not the other way around because Integal 3 is smaller and more likely to run into issues *)
		{
			{Model[Instrument, WaterPurifier, "id:eGakld01zVXG"](*Model[Instrument, WaterPurifier, "MilliQ Integral 3"]*)},
			{
				Model[Instrument, WaterPurifier, "id:AEqRl9qA8WZa"](*Model[Instrument, WaterPurifier, "Milli-Q IQ 7010 Type I Water Purifier"]*)
			}
		},
		{
...
RequireResources[myPackets : {PacketP[{Object[Protocol], Object[Qualification], Object[Maintenance], Object[Program], Object[Method], Object[Resource], Object[Primitive],Object[UnitOperation]}]..}, ops : OptionsPattern[]] := Module[
	...
	If[MatchQ[ProcedureFramework`Private`$ExperimentWaterPreparation,True],
		(* We no longer need to create a container resource for the water resource since we are going to be creating this in ExperimentWaterPreparation *)
		Nothing,
	...
	allPackets = If[MatchQ[ProcedureFramework`Private`$ExperimentWaterPreparation,True],
		Flatten[{nonDuplicateRequiredResourcesPackets, packetsWithoutResources, allNonDuplicateResourcePackets, operatorResourcesWithCheckpoint,updatesForSampleResourcePackets,transportInstrumentResourcePackets}],
		Flatten[{nonDuplicateRequiredResourcesPackets, packetsWithoutResources, allNonDuplicateResourcePackets, waterPrepContainerResourcePackets, operatorResourcesWithCheckpoint,updatesForSampleResourcePackets,transportInstrumentResourcePackets}]
	];
...
resourceToPacket[myResources : {__Resource}, ops : OptionsPattern[]] := Module[
	{
		...
		sampleVolumeOfUses, simulationMode, steriles, unusedIntegratedInstrument,approximateTransferBools
	},
	...
	(* pull out the ApproximateTransfer key from the Sample resource blobs - default to Null (which means False) *)
	approximateTransferBools = Map[
		If[MissingQ[#[ApproximateTransfer]],
			Null,
			#[ApproximateTransfer]
		]&,
		sampleResourceBlobs
	...
	(* note that if you have a naked number, add Unit to the end *)
	tolerances = Map[
		Switch[#[Tolerance],
			_?MissingQ, Null,
			NumberP, #[Tolerance] * Unit,
			_, #[Tolerance]
		]&,
		sampleResourceBlobs
	];
	...
	sampleResourcePackets = MapThread[
		Function[{resourceID, amount, numberOfUses, volumeOfUses, specifiedSample, container, containerName, well, root, fresh, rent, rentContainer, updateCount, untracked, thawRequired, exactAmount, tolerance, automaticDisposal, sterile, approximateTransfer},
			Join[
				<|
					Object -> resourceID,
					Type -> Object[Resource, Sample],
					...
					(* use Null to mean False *)
					If[MatchQ[approximateTransfer,True],
		...
		{sampleResourceIDs, sampleAmounts, sampleNumberOfUses, sampleVolumeOfUses, specifiedSamples, sampleContainers, sampleContainerNames, sampleWells, sampleRootProts, freshBools, rentBools, rentContainerBools, updateCountBools, untrackedBools, thawRequiredBools, exactAmountBools, tolerances, automaticDisposals, steriles, approximateTransferBools}
	];
...
fulfillableResourceQ[myResourceBlobs : {__Resource}, ops : OptionsPattern[]] := TraceExpression["fulfillableResourceQ-blob",Module[
...
(* CORE FUNCTION *)
fulfillableResourceQ[myResources : {ObjectP[Object[Resource]]..}, ops : OptionsPattern[]] := TraceExpression["fulfillableResourceQ",Module[
	{
...
fulfillableResources[myResourceBlobs : {__Resource}, ops : OptionsPattern[]] := TraceExpression["fulfillableResources-blob",Module[
...
fulfillableResources[myResources : {ObjectP[Object[Resource]]..}, ops : OptionsPattern[]] := TraceExpression["fulfillableResources",Module[
	{
			...
			ObjectP[Object[Resource, Sample]],
				{
					...
					(*3*) Packet[Sample[RequestedResources][{Status, Amount, NumberOfUses, VolumeOfUses, ExactAmount, Tolerance, Sterile, Rent, ApproximateTransfer}]],
	...
	*)
	...
	matchingTrainingModelTrainingProtocol = MapThread[
		Function[{modelPacket, rootProtocol, containerModel, resourcePacket},
			Which[
				...
				(* we also want to allow receiving and parameterizations here to avoid errors with new arrivals *)
				MatchQ[Lookup[rootProtocol,RootProtocol], ObjectP[{Object[Qualification, Training], Object[Maintenance, Training], Object[Maintenance, ReceiveInventory], Object[Maintenance, ParameterizeContainer], Object[Maintenance, ParameterizeCover]}]],
					True,
	...
	(* get all the resource requests for these specific samples (including others, but only those that are not InCart/Canceled/Fulfilled/Shipping)*)
	allConsumableSampleResources = MapThread[
		Function[{resourcePacket, consumableBool, allPackets},
			If[consumableBool && Not[NullQ[Lookup[resourcePacket, Sample]]],
				Select[allPackets, MatchQ[Lookup[#, Status], Except[InCart | Shipping | Canceled | Fulfilled, ResourceStatusP]] && Not[MatchQ[#, ObjectP[Lookup[resourcePacket, Object]]]]&],
				Nothing
			]
		],
	...
	Options :> {
		...
		IndexMatching[
			...
			},
			{
				...
				Widget -> Widget[
					Type -> Expression,
					Pattern :> MassP | VolumeP | GreaterEqualP[0, 1] | Null,
					Size -> Line
				]
			...
			{
				OptionName -> ApproximateTransfer,
				Default -> False,
				AllowNull -> True,
...
]:=TraceExpression["modelInstanceItems",Module[{
...
ConsolidationInstances[
	...
	(* get the position of self-contained samples, we wont even try to consolidate them b/c SP framework does not really support it now *)
	...
	*)
	...
	availableInstancesLookup = ModelInstances[
		...
		Map[
			If[MatchQ[#, CountP],
				(* still needs to be a multiple of count *)
				Max[# * $ConsolidationCutOff, 1 Unit],
				# * $ConsolidationCutOff
			]&,
			modelAmountTuplesFiltered[[All, 2]]
		],
