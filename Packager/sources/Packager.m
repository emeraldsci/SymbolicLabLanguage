...
(* These internal quantity unit values don't get loaded until after you Quit and then reload SLL on the kernel. So we just set them now to avoid that *)
addUnitDownValues[]:=(
	QuantityUnits`Private`CUF1R["KelvinsDifference"] = 1;
	QuantityUnits`Private`CUF1R["CelsiusDifference"] = 1;
	QuantityUnits`Private`CUF1R["FahrenheitDifference"] = 1;
	QuantityUnits`Private`CUF0R["KelvinsDifference"] = 1;
	QuantityUnits`Private`CUF0R["CelsiusDifference"] = 1;
	QuantityUnits`Private`CUF0R["FahrenheitDifference"] = 1;
);

(* InterpolatingFunction is broken after Quitting and reloading within the same Front End session in 14.2. The reason for this is unclear. *)
(* However, defining the below function fixes the issue. This function was taken from the Wolfram paclet Quantity-Units-4.0.2. *)
(* See https://redmine.wolfram.com/redmine/issues/3996 and https://app.asana.com/1/84467620246/project/1150770667411537/task/1212972786434229?focus=true for more details *)

(* deep threading version of UnitConvert that finds the inner-most alignment of inputs and units(if any) and threads accordingly *
 * General system is:
 * 1) check dimensions of input and units
 * 2) find the sequence of dimensions that are aligned from the deepest part of the expression
 *   (i.e. the shortest list of dimensions aligns with the last elements of the longest, or are identical)
 * 3) if there is no alignmnet (e.g. {2, 3, 2} and {3, 1}) fail
 * 4) MapThread using the dimensional alignment
 * Note: final result should have the same dimensions as the longer of the two input dimensions
 *)
defineDeepThreadUnitConvert[]:=(
	Unprotect[QuantityUnits`DeepThreadUnitConvert];
	QuantityUnits`DeepThreadUnitConvert[list_, units_] := With[
		{ldims = Dimensions[list, AllowedHeads -> "ListLike"], udims = Dimensions[units, AllowedHeads -> "ListLike"]},
		With[
			{n = Min[Length[ldims], Length[udims]]},
			Which[
				Take[ldims, -n] === udims,
				Map[MapThread[UnitConvert, {#, units}, n] &, list, {Length[ldims] - n}],

				Take[udims, -n] === ldims,
				Map[MapThread[UnitConvert, {list, #}, n] &, units, {Length[udims] - n}],

				True,
				Message[UnitConvert::arrdpt]; $Failed
			]
		]
	]
);
...
LoadDistro[configFile_String, options:OptionsPattern[]]:=Module[
	...
	Quiet[
		...
		(* avoid shadowing for ContourLabelStyle *)
		System`ContourLabelStyle;
		ECL`ContourLabelStyle;

		(* avoid shadowing for Qualified, which weirdly behaves differently on Mac/PC/Linux *)
		...
		(* avoid shadowing for Databases *)
		Databases`Databases;
		ECL`Databases;

		(* avoid shadowing for RowLabels *)
		System`RowLabels;
		ECL`RowLabels;

		(*avoid shadowing for Handle*)
		Wolfram`ErrorTools`Handle;
		ECL`Handle;
	...
	(* note that I'm also doing this in Core.m; seemingly this makes a difference when loading from distro or locally; once you remove this because it is nonsense, you should remove it from there too*)
