...
DefineObjectType[Model[Qualification, IonChromatography], {
	...
	Fields -> {
		...
		SamplePreparationUnitOperations -> {
			...
			Description -> "A list of unit operations that will be performed in the order listed to prepare samples for the tests.",
			Category -> "Sample Preparation"
		},
		CationBlankFrequency -> {
			Format -> Single,
			Class -> Expression,
			Pattern :> Null | None | First | Last | FirstAndLast,
			Relation -> Null,
		...
		CationBlankInjectionVolume -> {
			...
			Pattern :> GreaterEqualP[0 * Microliter],
			Units -> Microliter,
			Description -> "The volume of blank to be injected into the IonChromatography instrument's cation channel.",
			Category -> "Blank"
		},
		CationBlankGradient -> {
			Format -> Single,
			Class -> Link,
			Pattern :> _Link,
			Relation -> Alternatives[Object[Method, Gradient], Object[Method, IonChromatographyGradient]],
			Description -> "The gradient to run when injecting a blank sample into the IonChromatography instrument's cation channel.",
			Category -> "Blank"
		},
		CationSuppressorMode -> {
			...
			Description -> "For each member of CationSamples and CationBlanks, the operation method of the cation suppressor. Under DynamicMode, constant voltage is supplied to the suppressor with variable current while under LegacyMode, constant current is supplied.",
			Category -> "General"
		},
		CationSuppressorCurrent -> {
			Format -> Single,
			Class -> Real,
			Pattern :> GreaterEqualP[0 * Milliampere],
			Units -> Milliampere,
			Description -> "For each member of CationSamples and CationBlanks, the rate of electric charge flow between the electrodes in the suppressor during the run for cation channel.",
			Category -> "General"
		},
		AnionBlankFrequency -> {
			Format -> Single,
		...
		AnionBlankInjectionVolume -> {
			...
			Pattern :> GreaterEqualP[0 * Microliter],
			Units -> Microliter,
			Description -> "The volume of blank to be injected into the IonChromatography instrument's anion channel.",
			Category -> "Blank"
		},
		AnionBlankGradient -> {
			Format -> Single,
			...
			Pattern :> _Link,
			Relation -> Alternatives[Object[Method, Gradient], Object[Method, IonChromatographyGradient]],
			Description -> "The gradient to run when injecting a blank sample into the IonChromatography instrument's anion channel.",
			Category -> "Blank"
		},
		AnionSuppressorMode -> {
			Format -> Single,
			Class -> Expression,
			Pattern :> SuppressorModeP,
			Description -> "For each member of AnionSamples and AnionBlanks, the operation method of the anion suppressor. Under DynamicMode, constant voltage is supplied to the suppressor with variable current while under LegacyMode, constant current is supplied.",
			Category -> "General"
		},
		AnionSuppressorCurrent -> {
			...
			Pattern :> GreaterEqualP[0 * Milliampere],
			Units -> Milliampere,
			Description -> "For each member of AnionSamples and AnionBlanks, the rate of electric charge flow between the electrodes in the suppressor during the run for anion channel.",
			Category -> "General"
		}
	}
}];
