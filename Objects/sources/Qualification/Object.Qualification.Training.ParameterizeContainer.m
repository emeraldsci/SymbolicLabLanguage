...
DefineObjectType[Object[Qualification, Training, ParameterizeContainer], {
	...
	Cache -> Session,
	(* all the UserInput fields are kept for historical purposes. they are not used anymore. *)
	Fields -> {
		DimensionsUserInput -> {
			Format -> Single,
		...
		HeightGauge -> {
			...
			Pattern :> _Link,
			Relation -> Alternatives[Object[Instrument, DistanceGauge], Model[Instrument, DistanceGauge]],
			Description -> "The distance measurement device used to perform height measurements in this protocol.",
			Category -> "General"
		},
		DepthGauge -> {
			Format -> Single,
			...
			Pattern :> _Link,
			Relation -> Alternatives[Object[Instrument, DistanceGauge], Model[Instrument, DistanceGauge]],
			Description -> "The distance measurement device used to perform depth measurements in this protocol.",
			Category -> "General"
		},
		(* TODO these fields (PossibleMatchingContainers, PossibleMatchingContainersResult) should be migrated to the actual ParameterizeContainer maintenance when this feature is added to the maintenance *)
		PossibleMatchingContainers -> {
			Format -> Multiple,
			Class -> Link,
			Pattern :> _Link,
			Relation -> Alternatives[Model[Container, Plate]],
			Description -> "A set of candidate container models presented for operator verification to identify the correct match for the container being parameterized. If no match is found, the operator proceeds with parameterizing the container.",
			Category -> "Qualifications & Maintenance",
			Developer -> True
		},
		PossibleMatchingContainersResult -> {
			Format -> Multiple,
			Class -> Boolean,
			Pattern :> BooleanP,
			Description -> "The operator's assessment of each of the displayed PossibleMatchingContainers.",
			Category -> "Qualifications & Maintenance",
			Developer -> True
		}
	}
