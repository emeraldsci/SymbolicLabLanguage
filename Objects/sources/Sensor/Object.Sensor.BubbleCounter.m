...
DefineObjectType[Object[Sensor, BubbleCounter], {
	Description->"Device for measuring the air bubble count in a line.",
	...
	Fields -> {
		...
		MaxBubbleCount -> {
			Format -> Computable,
			Expression :> SafeEvaluate[{Field[Model]}, Download[Field[Model],MaxBubbleCount]],
		...
		MinBubbleCount -> {
			Format -> Computable,
			Expression :> SafeEvaluate[{Field[Model]}, Download[Field[Model],MinBubbleCount]],
			Pattern :> GreaterEqualP[0],
