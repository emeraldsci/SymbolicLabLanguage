...
DefineObjectType[Object[Protocol, FluorescenceIntensity], {
	Description->"Protocol for measuring the fluorescence response at a emission wavelength of samples illuminated with light at a excitation wavelength.",
	...
	Fields -> {
		...
		FocalHeights -> {
			...
			Units -> Meter Milli,
			Description -> "For each member of EmissionWavelengths, indicate the height above the assay plates from which the fluorescence intensity readings are made.",
			Category -> "Fluorescence Measurement",
			IndexMatching -> EmissionWavelengths
		},
		AutoFocalHeights -> {
			...
			Class -> Expression,
			Pattern :> BooleanP,
			Description -> "For each member of EmissionWavelengths, indicates if the FocalHeight will be determined automatically using the AdjustmentSample to calibrate.",
			Category -> "Fluorescence Measurement",
			IndexMatching -> EmissionWavelengths
		},
		NumberOfReadings -> {
		...
		AdjustmentSamples -> {
			...
			Description -> "For each member of EmissionWavelengths, indicate the sample used to determine the Gain percentage and focal height adjustments.",
			Category -> "Fluorescence Measurement",
			IndexMatching -> EmissionWavelengths
		},
