...
DefineObjectType[Object[Protocol, ManualSamplePreparation], {
	Description->"A protocol that prepares samples (ex. incubation, mixing, aliquotting, centrifugation, etc.) for further experimentation, manually in the laboratory.",
	...
	Fields -> {
		...
		(* Failure Response - relevant for protocols in which an IncubateCells subprotocol does not result in intended cell concentration/colony count. *)
		FailureState -> {
			Format -> Single,
			Class -> Boolean,
			Pattern :> BooleanP,
			Description -> "Indicates if remaining subprotocols of this protocol are to be skipped and the FailureResponse is to be executed due to the failure of one or more samples to reach the intended quantification target(s).",
