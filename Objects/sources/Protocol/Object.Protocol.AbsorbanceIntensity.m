...
DefineObjectType[Object[Protocol, AbsorbanceIntensity], {
	Description->"A protocol for reading the intensity of absorbances for a given plate well in a plate reader.",
	...
	Fields -> {
		...
		QuantifyConcentration -> {
			...
			Class -> Boolean,
			Pattern :> BooleanP,
			Description -> "Indicates if the concentration of the samples is determined.",
		...
		QuantifyConcentrations -> {
			...
			Pattern :> BooleanP,
			IndexMatching -> SamplesIn,
			Description -> "For each member of SamplesIn, indicates if the concentration of the sample is determined.",
			Category -> "Analysis & Reports"
		},
		Wavelengths -> {
			Format -> Multiple,
			...
			Units -> Nanometer,
			Description -> "For each member of SamplesIn, the wavelength at which absorbance is measured.",
			Category -> "Experimental Results",
			IndexMatching -> SamplesIn
		},
		SpectralBandwidth ->{
		...
		RecoupSample->{
			...
			Class->Boolean,
			Pattern:>BooleanP,
			Description->"For each member of SamplesIn, indicates whether the sample should be recovered back into the container after absorbance measurement.",
			Category->"Cleaning",
			IndexMatching -> SamplesIn
		},
		NitrogenPurge->{
