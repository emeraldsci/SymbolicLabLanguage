...
DefineObjectType[Object[Protocol, MeasureViscosity], {
	...
	(*-- Sample Preparation --*)
		...
		PressurizationInjectionRate -> {
			Format -> Single,
			Class -> Real,
			Pattern :> GreaterP[0*Millimeter/Second],
			Units -> Millimeter/Second,
			Description ->"The rate at which the Autosampler syringe injects air into the dead space of the sample vial if AutosamplerPrePressurization is set to True.",
			Category -> "General"
		},
		InjectionVolumes -> {
			Format -> Multiple,
