...
DefineObjectType[Object[Protocol,FragmentAnalysis],
	{
		...
		Fields->{
			...
			RunningBufferPlatePlaceholderPlacement -> {
				...
				Pattern :> {_Link,_Link, _String},
				Relation -> {Object[Container],Object[Instrument], Null},
				Description -> "The placement used to place the RunningBufferPlatePlaceholder in the dedicated position inside the Instrument.",
			...
			LadderData -> {
				...
				Description -> "For each member of Ladders, data collected for the ladder run alongside the samples generated by this protocol.",
			...
			BlankData -> {
				...
				Pattern :> _Link,
				Relation -> Object[Data][Protocol],
				Description -> "For each member of Blanks, data collected for the blank run alongside the samples generated by this protocol.",
				Category -> "Experimental Results",
				IndexMatching->Blanks
			},
			SeparationRequiredRetries -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Object[Data][Protocol],
				Description -> "The data collected for well contents of all separation runs that are either unsuccessful or collected as part of the required 96 samples for every run.  The successful data are stored in the Data, LadderData and BlankData fields. The data objects in this field are for tracking purposes only.",
				Category -> "Experimental Results"
			},
			...
			LadderCalibrationFailure -> {
				Format -> Single,
				Class -> Boolean,
				Pattern :> BooleanP,
				Description -> "Indicates if the data collected for the ladder in well H12 did not yield a calibration curve for peak size vs concentration with a proper fit. If True, the following data fields are not populated: SimulatedGelImage, FragmentSizeAnalysesImage,RNAFragmentSizeAnalyses,DNAFragmentSizeAnalyses,SampleQualityNumber,DNATotalConcentration,RNATotalConcentration,RNAPeakTable,DNAPeakTable. Raw data such as Electropherogram and ElectropherogramImage are still available.",
				Category -> "Data Processing"
			},
			RetryLadderCalibrationFailures -> {
				Format -> Multiple,
				Class -> Boolean,
				Pattern :> BooleanP,
				Description -> "Indicates if the data collected for the ladder in well H12 did not yield a calibration curve for peak size vs concentration with a proper fit for each retry step. If True, the following data fields are not populated: SimulatedGelImage, FragmentSizeAnalysesImage,RNAFragmentSizeAnalyses,DNAFragmentSizeAnalyses,SampleQualityNumber,DNATotalConcentration,RNATotalConcentration,RNAPeakTable,DNAPeakTable. Raw data such as Electropherogram and ElectropherogramImage are still available.",
				Category -> "Data Processing"
			},
			FlushMaintenance -> {
				Format -> Single,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Object[Maintenance, FlushCapillaryArray],
				Description -> "The flush maintenance protocol that is enqueued in the protocol and is to run immediately after the protocol is completed in order to ensure proper capillary array conditions for the next sample run.",
				Category -> "General"
			},
			(* Retry *)
			MaxNumberOfRetries->{
				...
				Class->Integer,
				Pattern:>GreaterEqualP[1,1],
				Description->"The maximum number of separation runs that can be performed for contents of the sample plate when the raw data (electropherogram) for a sample or ladder indicates no peak(s) detected as assessed by the lack of any baseline-corrected signal above 100 RFU.",
				Category->"Retry"
			},
			RetrySamplePlacements->{
				Format->Multiple,
				Class->Expression,
				Pattern:>{{
					KeyValuePattern[
						{
							Sample->_Link,
							SampleWell->WellPositionP,
							SampleType->Sample|Ladder|Blank,
							SourceWell->WellPositionP,
							DestinationWell->WellPositionP
						}
					]..
				}..},
				Description->"A list of sample information for any sample or ladder that requires a rerun of the separation, as well the corresponding blank that is utilized for the well position switch for the separation rerun.",
				Category->"Retry",
				Developer->True
			},
			RetrySeparationGels -> {
				...
				Pattern :> _Link,
				Relation -> Alternatives[Model[Sample],Object[Sample]],
				Description -> "The list of gel reagents that serve as sieving matrix to facilitate the optimal separation of the analyte fragments in each sample by size for the separation reruns.",
				Category -> "Retry"
			},
			RetryDyes -> {
				Format -> Multiple,
				...
				Pattern :> _Link,
				Relation -> Alternatives[Model[Sample],Object[Sample]],
				Description -> "The list of solutions of a dye molecule that fluoresces when bound to DNA or RNA fragments in the sample and is used in the detection of the analyte fragments as it passes through the detection window of the capillary for the separation reruns.",
				Category -> "Retry"
			},
			RetryConditioningSolutions -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[Model[Sample],Object[Sample]],
				Description -> "The list of solution(s) used in the priming of the capillaries prior to a loading of sample into the capillaries for the separation reruns. The conditioning step helps maintain a low and reproducible electroosmotic flow for more precise analyte mobilities and migration times by stabilizing buffer pH and background electrolyte levels for the separation reruns..",
				Category -> "Retry"
			},
			RetrySamplePlates -> {
				Format -> Multiple,
				...
				Description -> "The list of plates that hold the samples, ladders and blanks and is loaded into the instrument for the separation reruns.",
				Category -> "Retry"
			},
			RetryRunningBufferPlates -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of plates that hold the SampleRunningBuffer, LadderRunningBuffer and BlankRunningBuffer and is loaded into the instrument for the separation reruns.",
				Category -> "Retry"
			},
			RetryMarkerPlates -> {
				Format -> Multiple,
				...
				Description -> "The list of plates that hold the SampleMarker,LadderMarker,BlankMarker and is loaded into the instrument for the separation reruns.",
				Category -> "Retry"
			},
			RetryPreMarkerRinseBufferPlates -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of plates that hold the PreMarkerRinseBuffer and is loaded into the instrument for the separation reruns.",
				Category -> "Retry"
			},
			RetryPreSampleRinseBufferPlates -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of plates that hold the PreSampleRinseBuffer and is loaded into the instrument for the separation reruns.",
				Category -> "Retry"
			},
			RetryWastePlates -> {
				Format -> Multiple,
				...
				Description -> "The list of plates that hold the solution the comes out of the capillary array during flush and separation runs, and is loaded into the instrument for the separation reruns.",
				Category -> "Retry"
			},
			RetryRawDataArchiveFiles -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Object[EmeraldCloudFile],
				Description -> "The list of *.zip data files containing all the raw data files generated for the separation reruns.",
				Category -> "Retry"
			},
			(* Retry Developer Fields *)
			RetryState->{
				Format->Single,
				Class->Expression,
				Pattern:>BooleanP,
				Description->"Indicates whether another separation run for contents of the sample plate is required due to lack of detected peak(s) for at least one of the samples or ladders.",
				Category->"Retry",
				Developer->True
			},
			RetryGelDyeContainers -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of container(s) that hold the Separation Gel and Dye Solution for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryConditioningSolutionContainers -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of container(s) that holds the ConditioningSolution for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetrySamplePlateCovers -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of seal(s) or lid(s) that cover the RetrySamplePlate(s) after preparation and at any time it is not inside the Instrument for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryRunningBufferPlateCovers -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of seal(s) or lid(s) that covers the RetryRunningBufferPlates after preparation and at any time it is not inside the Instrument for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryMarkerPlateCovers -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of seal(s) or lid(s) that covers the RetryMarkerPlate(s) after preparation and at any time it is not inside the Instrument for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryPreMarkerRinseBufferPlateCovers -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of seal(s) or lid(s) that covers the RetryPreMarkerRinseBufferPlate(s) after preparation and at any time it is not inside the Instrument for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryPreSampleRinseBufferPlateCovers -> {
				Format -> Multiple,
				Class -> Link,
				Pattern :> _Link,
				Relation -> Alternatives[
				...
				Description -> "The list of seal(s) or lid(s) that covers the RetryPreSampleRinseBufferPlate(s) after preparation and at any time it is not inside the Instrument for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryGelDyeUnitOperations -> {
				Format -> Multiple,
				Class -> Expression,
				Pattern :> {_Link..},
				Description -> "The list of UnitOperation objects used to prepare the combined SeparationGel and Dye for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetrySamplePlateUnitOperations -> {
				Format -> Multiple,
				Class -> Expression,
				Pattern :> {_Link..},
				Description -> "The list of UnitOperation objects used to prepare the SamplePlate for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryRunningBufferPlateUnitOperations -> {
				Format -> Multiple,
				Class -> Expression,
				Pattern :> {_Link..},
				Description -> "The list of UnitOperation objects used to prepare the RetryRunningBufferPlates for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryMarkerPlateUnitOperations -> {
				Format -> Multiple,
				Class -> Expression,
				Pattern :> {_Link..},
				Description -> "The list of UnitOperation objects used to prepare the RetryMarkerPlates for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryPreSampleRinseBufferPlateUnitOperations -> {
				Format -> Multiple,
				Class -> Expression,
				Pattern :> {_Link..},
				Description -> "The list of UnitOperation objects used to prepare the RetryPreSampleRinseBufferPlates for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryPreMarkerRinseBufferPlateUnitOperations -> {
				Format -> Multiple,
				Class -> Expression,
				Pattern :> {_Link..},
				Description -> "The list of UnitOperation objects used to prepare the RetryPreMarkerRinseBufferPlates for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryGelDyePlacements -> {
				Format -> Multiple,
				Class -> {Link, Link, String},
				Pattern :> {_Link,_Link, _String},
				Relation -> {Object[Container],Object[Instrument], Null},
				Description -> "The list of placement(s) used to place the GelDyeContainer in the dedicated position inside the Instrument for the separation reruns.",
				Headers -> {"GelDyeContainer", "Instrument", "Instrument Position"},
				Category -> "Retry",
				Developer -> True
			},
			RetryConditioningSolutionPlacements -> {
				Format -> Multiple,
				Class -> {Link, Link, String},
				Pattern :> {_Link,_Link, _String},
				Relation -> {Object[Container],Object[Instrument], Null},
				Description -> "The list of placement(s) used to place the ConditioningSolutionContainer in the dedicated position inside the Instrument for the separation reruns.",
				Headers -> {"ConditioningSolution", "Instrument", "Instrument Position"},
				Category -> "Retry",
				Developer -> True
			},
			RetrySamplePlatePlacements -> {
				Format -> Multiple,
				Class -> {Link, Link, String},
				Pattern :> {_Link,_Link, _String},
				Relation -> {Object[Container],Object[Instrument], Null},
				Description -> "The list of placement(s) used to place the RetrySamplePlates in the dedicated position inside the Instrument for the separation reruns.",
				Headers -> {"SamplePlate", "Instrument", "Instrument Position"},
				Category -> "Retry",
				Developer -> True
			},
			RetryRunningBufferPlatePlacements -> {
				Format -> Multiple,
				Class -> {Link, Link, String},
				Pattern :> {_Link,_Link, _String},
				Relation -> {Object[Container],Object[Instrument], Null},
				Description -> "The list of placement(s) used to place the RetryRunningBufferPlates in the dedicated position inside the Instrument for the separation reruns.",
				Headers -> {"RunningBufferPlate", "Instrument", "Instrument Position"},
				Category -> "Retry",
				Developer -> True
			},
			RetryMarkerPlatePlacements -> {
				Format -> Multiple,
				Class -> {Link, Link, String},
				Pattern :> {_Link,_Link, _String},
				Relation -> {Object[Container],Object[Instrument], Null},
				Description -> "The list of placement(s) used to place the RetryMarkerPlates in the dedicated position inside the Instrument for the separation reruns.",
				Headers -> {"MarkerPlate", "Instrument", "Instrument Position"},
				Category -> "Retry",
				Developer -> True
			},
			RetryPreSampleRinseBufferPlatePlacements -> {
				Format -> Multiple,
				Class -> {Link, Link, String},
				Pattern :> {_Link,_Link, _String},
				Relation -> {Object[Container],Object[Instrument], Null},
				Description -> "The list of placement(s) used to place the RetryPreSampleRinseBufferPlates in the dedicated position inside the Instrument for the separation reruns.",
				Headers -> {"PreSampleRinseBufferPlate", "Instrument", "Instrument Position"},
				Category -> "Retry",
				Developer -> True
			},
			RetryPreMarkerRinseBufferPlatePlacements -> {
				Format -> Multiple,
				Class -> {Link, Link, String},
				Pattern :> {_Link,_Link, _String},
				Relation -> {Object[Container],Object[Instrument], Null},
				Description -> "The list of placement(s) used to place the RetryPreMarkerRinseBufferPlates in the dedicated position inside the Instrument for the separation reruns.",
				Headers -> {"PreMarkerRinseBufferPlate", "Instrument", "Instrument Position"},
				Category -> "Retry",
				Developer -> True
			},
			RetryWastePlatePlacements -> {
				Format -> Multiple,
				Class -> {Link, Link, String},
				Pattern :> {_Link,_Link, _String},
				Relation -> {Object[Container],Object[Instrument], Null},
				Description -> "The list of placement(s) used to place the RetryWastePlatePlates in the dedicated position inside the Instrument for the separation reruns.",
				Headers -> {"WastePlate", "Instrument", "Instrument Position"},
				Category -> "Retry",
				Developer -> True
			},
			RetryIDs->{
				...
				Pattern :> _String,
				Relation -> Null,
				Description -> "The list of strings that are used as folder suffix for any data folder created for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryBackupRawDataFilePaths->{
				Format -> Multiple,
				Class -> String,
				Pattern :> FilePathP,
				Description -> "The list of file paths on the instrument computer in which a back up copy of the data generated by the experiment is stored locally for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryRawDataBatFilePaths->{
				Format->Multiple,
				Class->String,
				Pattern:>FilePathP,
				Description->"The list of file path of the bat file that is executed in order to copy the Raw Data to the Z Drive for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryRawDataBatFileNames->{
				Format->Multiple,
				Class->String,
				Pattern:>_String,
				Description->"The list of names of the the bat files that are executed in order to copy the Raw Data to the Z Drive for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryRawDataFilePaths->{
				Format->Multiple,
				Class->String,
				Pattern:>FilePathP,
				Description->"The list of file paths of the *.raw files that are generated for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryRawDataDirectoryFilePaths->{
				Format->Multiple,
				Class->String,
				Pattern:>FilePathP,
				Description->"The list of file paths of the folder in the Z drive that contain all the raw data files of the protocol for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryRawDataFileNames->{
				...
				Class->String,
				Pattern:>_String,
				Description->"The list of names of the the *.raw files that are generated for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryRawDataArchiveFilePaths->{
				Format->Multiple,
				Class->String,
				Pattern:>FilePathP,
				Description->"The list of file path of the *.zip data file containing all the raw unprocessed data files generatedfor the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryProcessedDataDirectoryFilePaths->{
				Format->Multiple,
				Class->String,
				Pattern:>FilePathP,
				Description->"The list of file path of the folder in the Z drive that contains all the processed data files of the protocol for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryProcessedTimeDataDirectoryFilePaths->{
				Format->Multiple,
				Class->String,
				Pattern:>FilePathP,
				Description->"The list of file path of the folder in the Z drive that contains all the processed data files of the protocol that involve time units for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			},
			RetryProcessedSizeDataDirectoryFilePaths->{
				Format->Multiple,
				Class->String,
				Pattern:>FilePathP,
				Description->"The list of file path of the folder in the Z drive that contains all the processed data files of the protocol that involve size (bp or nt) units for the separation reruns.",
				Category -> "Retry",
				Developer -> True
			}
		}
