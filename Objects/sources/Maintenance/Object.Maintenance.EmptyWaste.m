...
DefineObjectType[Object[Maintenance, EmptyWaste], {
	...
	Fields -> {
		...
		TwistTies -> {
			...
			Description -> "For each member of EmptiedWasteBins, the fastener used to tighten the DisposedBags during this maintenance.",
			IndexMatching->EmptiedWasteBins,
			Category -> "General",
			Developer -> True
		},
		SecondBiosafetyWasteBags -> {
			Format -> Multiple,
			Class -> Link,
			Pattern :> _Link,
			Relation -> Alternatives[Model[Item,Consumable],Object[Item,Consumable]],
			Description -> "For each member of EmptiedWasteBins, the waste bag that the tied-up trash bag is sealed in.",
			IndexMatching->EmptiedWasteBins,
			Category -> "General",
			Developer -> True
		}
