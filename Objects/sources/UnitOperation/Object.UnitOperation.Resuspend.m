...
DefineObjectType[Object[UnitOperation, Resuspend], {
	Description -> "A detailed set of parameters that specifies the information of how to resuspend samples in a solvent.",
	...
	Fields -> {
		...
		Volume -> {
			...
			Description -> "For each member of SampleLink, total volume of the resuspended sample plus diluent.",
			Category -> "Method Information",
			Abstract -> True,
			IndexMatching -> SampleLink
		},
		...
		ContainerOutExpression -> {
			Format -> Multiple,
			Class -> Expression,
			Pattern :> Alternatives[Automatic, {_Integer|Automatic|_String, _String|ObjectP[{Model[Container], Object[Container]}]|Automatic}],
			Relation -> Null,
			Description -> "For each member of SampleLink, the desired type of container that should be used to prepare and house the diluted samples, with indices indicating grouping of samples in the same plates, if desired.",
			Category -> "Method Information",
			Migration -> SplitField,
		...
		NumberOfMixes->{
			...
			Description->"For each member of SampleLink, the number of times the sample is mixed.",
			IndexMatching->SampleLink,
			Category->"General"
		},
		MixOrder -> {
