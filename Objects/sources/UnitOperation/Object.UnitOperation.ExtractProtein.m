...
DefineObjectType[Object[UnitOperation, ExtractProtein], {
	Description -> "A detailed set of parameters that specifies an ExtractProtein step within a larger protocol.",
	...
	Fields -> {
		...
		SampleLink -> {
			...
			Description -> "The live cell or cell lysate samples from which protein is to be extracted.",
			Category -> "General",
		...
		MethodLink -> {
			...
			Class -> Link,
			Pattern :> _Link,
			Relation -> Object[Method, Extraction, Protein],
		...
		TargetProteinLink -> {
			...
			Description->"For each member of SampleLink, the target protein that is isolated from SamplesIn during protein extraction.",
			Category -> "General",
			IndexMatching -> SampleLink,
			Migration -> SplitField
		},
		...
		ExtractedProteinLabel -> {
			...
			Class -> String,
			Pattern :> _String,
			Description->"For each member of SampleLink, a user defined word or phrase used to identify the sample that contains the extracted Protein, for use in downstream unit operations.",
			Category->"General",
			IndexMatching -> SampleLink
		},
		...
		ExtractedProteinContainerLabel -> {
			...
			Class -> String,
			Pattern :> _String,
			Description->"For each member of SampleLink, a user defined word or phrase used to identify the container that contains the extracted Protein, for use in downstream unit operations.",
			Category->"General",
			IndexMatching -> SampleLink
		},
		...
		RoboticInstrument -> {
			...
			Description->"For each member of SampleLink, a user defined word or phrase used to identify the container that contains the extracted Protein sample, for use in downstream unit operations.",
			Category->"General"
		},
		...
		Lyse -> {
			...
			Class -> Boolean,
			Pattern :> BooleanP,
			Description -> "For each member of SampleLink, indicates if lysis reagents or conditions are applied to the input sample, chemically breaking the cells' plasma membranes (and cell wall, depending on the cell type) and releasing the cell components into the chosen solution, creating a lysate to extract the Protein from. If the input is live cells, Lyse is automatically set to True and lysis is performed before extraction. If the input is already lysate, Lyse is automatically set to False and the function proceeds directly to extraction.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink
		},
		...
		LysisSolutionEquilibrationTime -> {
			...
			Units -> Minute,
			Description -> "For each member of SampleLink, the minimum duration during which the LysisSolution is incubated at LysisSolutionTemperature before addition to the cell sample.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink
		},
		LysisSolutionTemperatureReal -> {
			Format -> Multiple,
			Class -> Real,
			Pattern :> GreaterEqualP[0 Kelvin],
			Units -> Celsius,
			Description -> "For each member of SampleLink, the temperature at which the LysisSolution is incubated during the LysisSolutionEquilibrationTime before addition to the cell sample.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink,
			Migration -> SplitField
		},
		LysisSolutionTemperatureExpression -> {
			Format -> Multiple,
			Class -> Expression,
		...
		SecondaryLysisSolutionEquilibrationTime -> {
			...
			Pattern :> GreaterEqualP[0 Second],
			Units -> Minute,
			Description -> "For each member of SampleLink, the minimum duration during which the SecondaryLysisSolution is incubated at SecondaryLysisSolutionTemperature before addition to the cell sample.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink
		},
		SecondaryLysisSolutionTemperatureReal -> {
			Format -> Multiple,
			Class -> Real,
			Pattern :> GreaterEqualP[0 Kelvin],
			Units -> Celsius,
			Description -> "For each member of SampleLink, the temperature at which the SecondaryLysisSolution is incubated during the SecondaryLysisSolutionEquilibrationTime before addition to the cell sample.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink,
			Migration -> SplitField
		},
		SecondaryLysisSolutionTemperatureExpression -> {
			Format -> Multiple,
			Class -> Expression,
			Pattern :> Alternatives[Ambient],
			Description -> "For each member of SampleLink, the temperature at which the SecondaryLysisSolution is incubated during the SecondaryLysisSolutionEquilibrationTime before addition to the cell sample.",
		...
		TertiaryLysisSolutionEquilibrationTime -> {
			...
			Pattern :> GreaterEqualP[0 Second],
			Units -> Minute,
			Description -> "For each member of SampleLink, the minimum duration during which the TertiaryLysisSolution is incubated at TertiaryLysisSolutionTemperature before addition to the cell sample.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink
		},
		TertiaryLysisSolutionTemperatureReal -> {
			Format -> Multiple,
			Class -> Real,
			Pattern :> GreaterEqualP[0 Kelvin],
			Units -> Celsius,
			Description -> "For each member of SampleLink, the temperature at which the TertiaryLysisSolution is incubated during the TertiaryLysisSolutionEquilibrationTime before addition to the cell sample.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink,
			Migration -> SplitField
		},
		TertiaryLysisSolutionTemperatureExpression -> {
			Format -> Multiple,
			Class -> Expression,
			Pattern :> Alternatives[Ambient],
			Description -> "For each member of SampleLink, the temperature at which the TertiaryLysisSolution is incubated during the TertiaryLysisSolutionEquilibrationTime before addition to the cell sample.",
		...
		LiquidLiquidExtractionSolventAdditions -> {
			...
			Description -> "For each member of SampleLink, and for each extraction round of a liquid-liquid extraction, the solvent(s) that are added to the sample in order to create a biphasic solution. NOTE: If no solvent will be added for one or more of the liquid-liquid extraction rounds, then set those rounds to Null. This equates to \"None\" in ExperimentExtractProtein.",
			Category -> "Liquid-liquid Extraction",
			IndexMatching -> SampleLink
		},
		...
		SolidPhaseExtractionTechnique -> {
			Format -> Multiple,
			Class -> Expression,
			Pattern :> Centrifuge | AirPressure | Pressure,(*TODO ExperimentExtractProtein just takes AirPressure, and if SPE is called the SPE shared resolver pre-resolves AirPressure to be Pressure, which is what SPE takes. But Filter takes AirPressure again so it gets weird*)
		...
		MagneticBeads -> {
			...
			Pattern :> _Link,
			Relation -> Alternatives[Object[Sample], Model[Sample]],
			Description -> "For each member of SampleLink, the superparamagnetic particles with surface coatings to bind target analyte or contaminants. They exhibit magnetic behavior only in the presence of an exteproteinl magnetic field. The magnetic beads are pulled to the perimeter of the container by applying a magnetic force, thus enables maximal aspiration of the Supernatant.",
			Category -> "Magnetic Bead Separation",
			IndexMatching -> SampleLink
		},
