...
DefineObjectType[Object[UnitOperation,Mix],
	{
		Description->"A detailed set of parameters that specifies a single mix step in a larger protocol.",
		...
		Fields->{
			SampleLink -> {
				...
				Description -> "The samples that we are mixing.",
				Category -> "General",
			...
			ResidualTemperature->{
				...
				Pattern:>GreaterEqualP[0 Kelvin],
				IndexMatching->SampleLink,
				Description->"For each member of SampleLink, the temperature at which the sample(s) should remain mixing after Time has elapsed. This option can only be set if Preparation->Robotic.",
				Category->"Incubation"
			},
			ResidualMix->{
				Format->Multiple,
