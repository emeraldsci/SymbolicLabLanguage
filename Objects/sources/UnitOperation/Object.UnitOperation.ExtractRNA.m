...
DefineObjectType[Object[UnitOperation, ExtractRNA], {
	Description -> "A detailed set of parameters that specifies an ExtractRNA step within a larger protocol.",
	...
	Fields -> {
		...
		MethodLink -> {
			...
			Class -> Link,
			Pattern :> _Link,
			Relation -> Object[Method, Extraction, RNA],
		...
		TargetRNA -> {
			Format -> Multiple,
			Class -> Expression,
			Pattern :> CellularRNAP | Unspecified,
			Description -> "For each member of SampleLink, the type of RNA that is purified during the extraction. TotalRNA is extracted from cell lysate. Subsets of cellular RNA (mRNA, miRNA, tRNA, rRNA) are preferentially purified from a cell lysate by varying extraction conditions such as lysis or wash solutions. ViralRNA, ExosomalRNA, or CellFreeRNA are extracted and purified from harvested media or viral particles without lysing cells.",
			Category -> "General",
			IndexMatching -> SampleLink
		},
		...
		ExtractedRNALabel -> {
			...
			Class -> String,
			Pattern :> _String,
			Description->"For each member of SampleLink, a user defined word or phrase used to identify the sample that contains the extracted RNA, for use in downstream unit operations.",
			Category->"General",
			IndexMatching -> SampleLink
		},
		...
		ExtractedRNAContainerLabel -> {
			...
			Class -> String,
			Pattern :> _String,
			Description->"For each member of SampleLink, a user defined word or phrase used to identify the container that contains the extracted RNA, for use in downstream unit operations.",
			Category->"General",
			IndexMatching -> SampleLink
		},
		...
		RoboticInstrument -> {
			...
			Description->"For each member of SampleLink, a user defined word or phrase used to identify the container that contains the extracted RNA sample, for use in downstream unit operations.",
			Category->"General"
		},
		...
		Lyse -> {
			...
			Class -> Boolean,
			Pattern :> BooleanP,
			Description -> "For each member of SampleLink, indicates if lysis reagents or conditions are applied to the input sample, chemically breaking the cells' plasma membranes (and cell wall, depending on the cell type) and releasing the cell components into the chosen solution, creating a lysate to extract the RNA from. If the input is live cells, Lyse is automatically set to True and lysis is performed before extraction. If the input is already lysate, Lyse is automatically set to False and the function proceeds directly to extraction.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink
		},
		...
		LysisAliquotContainerWell -> {
			Format -> Multiple,
			Class -> String,
			Pattern :> _String,
			Relation -> Null,
			Description -> "For each member of SampleLink, the well of the container into which the LysisAliquotAmount is transferred prior to cell lysis.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink
		},
		...
		LysisAliquotContainerLabel -> {
			...
			Pattern :> _String,
			Relation -> Null,
			Description -> "For each member of SampleLink, a user defined word or phrase used to identify of the container into which the cell sample is aliquoted prior to cell lysis.",
			Category -> "Cell Lysis",
			IndexMatching -> SampleLink
		},
		...
		HomogenizeLysate -> {
			...
			Class -> Boolean,
			Pattern :> BooleanP,
			Description -> "For each member of SampleLink, indicates if lysate samples are disrupted using mechanical forces to shear large subcellular components, after lysis and prior to purification steps. Lysate homogenization can prevent clogging of solid phase extraction cartridges, if SolidPhaseExtraction is employed during Purification.",
		...
		HomogenizationDevice -> {
			...
			Description -> "For each member of SampleLink, the consumable container with an embedded filter which is used to shear lysate components by flushing the lysate sample through the pores in the filter.",
			Category -> "Lysate Homogenization",
			IndexMatching -> SampleLink
		},

		HomogenizationCollectionContainerExpression -> {
			Format -> Multiple,
			Class -> Expression,
			Pattern :> Alternatives[
				ListableP[ObjectP[{Object[Container], Model[Container]}]],
				ListableP[{_Integer, ObjectP[Model[Container]]}],
				ListableP[{_String, ObjectP[{Object[Container], Model[Container]}]}],
				ListableP[{_String, {_Integer, ObjectP[{Model[Container]}]}}]
			],
			Description -> "For each member of SampleLink, the consumable container that the HomogenizationDevice is placed on top of before flushing the lysate through the HomogenizationDevice and into the HomogenizationCollectionContainer below.",
			Category -> "Lysate Homogenization",
			IndexMatching -> SampleLink,
			Migration -> SplitField
		},

		HomogenizationCollectionContainerLink -> {
			Format -> Multiple,
			Class -> Link,
			Pattern :> _Link,
			Relation -> Alternatives[
		...
		HomogenizationTechnique -> {
			...
			Description -> "For each member of SampleLink, indicates the type of force used to flush the cell lysate sample through the HomogenizationDevice in order to shear lysate components.",
			Category -> "Lysate Homogenization",
			IndexMatching -> SampleLink
		},
		...
		HomogenizationInstrument -> {
			...
			Pattern :> _Link,
			Relation -> Alternatives[Object[Instrument], Model[Instrument]],
			Description -> "For each member of SampleLink, the Instrument that generates force to flush the cell lysate sample through the HomogenizationDevice during the HomogenizationTime.",
			Category -> "Lysate Homogenization",
			IndexMatching -> SampleLink
		},
		...
		HomogenizationCentrifugeIntensity -> {
			...
			Pattern :> Alternatives[GreaterEqualP[0 RPM], GreaterEqualP[0 GravitationalAcceleration]],
			Units -> None,
			Description -> "For each member of SampleLink, the rotational speed or gravitational force at which the sample is centrifuged to flush the lysate through the HomogenizationDevice in order to shear lysate components.",
			Category -> "Lysate Homogenization",
			IndexMatching -> SampleLink
		},
		...
		HomogenizationPressure -> {
			...
			Units -> PSI,
			Description -> "For each member of SampleLink, the amount of pressure applied to flush the lysate through the HomogenizationDevice in order to shear lysate components.",
			Category -> "Lysate Homogenization",
			IndexMatching -> SampleLink
		},
		...
		HomogenizationTime -> {
			...
			Pattern :> GreaterEqualP[0 Second],
			Units -> Second,
			Description -> "For each member of SampleLink, the duration of time for which force is applied to flush the lysate through the HomogenizationDevice by the specified HomogenizationTechnique and HomogenizationInstrument in order to shear lysate components.",
			Category -> "Lysate Homogenization",
			IndexMatching -> SampleLink
		},
		...
		DehydrationSolution -> {
			Format -> Multiple,
			Class -> Expression,
			Pattern :> ObjectP[{Object[Sample],Model[Sample]}] | None,
			Description -> "For each member of SampleLink, the solution that is added to the sample prior to a purification step in order to promote binding of the analyte to the Sorbent. When Strategy is set to Positive, DehydrationSolution promotes binding of the TargetCellularComponent to the Sorbent.",
			Category -> "Dehydration",
			IndexMatching -> SampleLink
		},

		DehydrationSolutionVolumeReal -> {
			Format -> Multiple,
			Class -> Real,
			Pattern :> GreaterEqualP[0 Microliter],
			Units -> Microliter,
			Description -> "For each member of SampleLink, the volume of DehydrationSolution that is added to the sample prior to a purification step in order to promote binding of the analyte to the Sorbent. When Strategy is set to Positive, DehydrationSolution promotes binding of the TargetCellularComponent to the Sorbent.",
			Category -> "Dehydration",
			IndexMatching -> SampleLink,
			Migration -> SplitField
		},
		DehydrationSolutionVolumeExpression -> {
			Format -> Multiple,
			Class -> Expression,
			Pattern :> Alternatives[All],
			Description -> "For each member of SampleLink, the volume of DehydrationSolution that is added to the sample prior to a purification step in order to promote binding of the analyte to the Sorbent. When Strategy is set to Positive, DehydrationSolution promotes binding of the TargetCellularComponent to the Sorbent.",
		...
		DehydrationSolutionMixType -> {
			...
			Class -> Expression,
			Pattern :> Alternatives[Shake, Pipette, None],
			Description -> "For each member of SampleLink, the manner in which the sample is agitated following addition of DehydrationSolution to the sample. None specifies that no mixing occurs after adding DehydrationSolution to the sample.",
			Category -> "Dehydration",
			IndexMatching -> SampleLink
		},
		...
		NumberOfDehydrationSolutionMixes -> {
			...
			Pattern :> GreaterEqualP[0],
			Units -> None,
			Description -> "For each member of SampleLink, the number of times that the sample is mixed by pipetting the DehydrationSolutionMixVolume up and down following the addition of DehydrationSolution to the sample.",
			Category -> "Dehydration",
			IndexMatching -> SampleLink
		},

		DehydrationSolutionMixVolume -> {
			Format -> Multiple,
			Class -> Real,
			Pattern :> GreaterEqualP[0 Microliter],
			Units -> Microliter,
			Description -> "For each member of SampleLink, the volume of sample containing DehydrationSolution that is displaced during each mix-by-pipet mix cycle.",
			Category -> "Dehydration",
			IndexMatching -> SampleLink
		},
		...
		DehydrationSolutionMixRate -> {
			...
			Pattern :> GreaterEqualP[0 RPM],
			Units -> RPM,
			Description -> "For each member of SampleLink, the number of rotations per minute at which the sample containing DehydrationSolution is shaken in order to fully mix the DehydrationSolution with the sample, during the DehydrationSolutionMixTime.",
			Category -> "Dehydration",
			IndexMatching -> SampleLink
		},
		...
		DehydrationSolutionMixTime -> {
			...
			Pattern :> GreaterEqualP[0 Second],
			Units -> Second,
			Description -> "For each member of SampleLink, the duration for which the sample and DehydrationSolution are mixed by the selected DehydrationSolutionMixType following combination of the cell sample and the DehydrationSolution.",
			Category -> "Dehydration",
			IndexMatching -> SampleLink
		},
		...
		LiquidLiquidExtractionSolventAdditions -> {
			...
			Description -> "For each member of SampleLink, and for each extraction round of a liquid-liquid extraction, the solvent(s) that are added to the sample in order to create a biphasic solution. NOTE: If no solvent will be added for one or more of the liquid-liquid extraction rounds, then set those rounds to Null. This equates to \"None\" in ExperimentExtractRNA.",
			Category -> "Liquid-liquid Extraction",
			IndexMatching -> SampleLink
		},
		...
		SolidPhaseExtractionTechnique -> {
			Format -> Multiple,
			Class -> Expression,
			Pattern :> Centrifuge | AirPressure | Pressure,(*TODO ExperimentExtractRNA just takes AirPressure, and if SPE is called the SPE shared resolver pre-resolves AirPressure to be Pressure, which is what SPE takes. But Filter takes AirPressure again so it gets weird*)
		...
		MagneticBeadSeparationSelectionStrategy -> {
			...
			Description -> "Specifies if the target analyte (Positive) or contaminants (Negative) binds to the magnetic beads in order to isolate the target analyte. When the target analyte is bound to the magnetic beads (Positive), they are collected as SamplesOut during the elution step. When contaminants are bound to the magnetic beads (Negative), the target analyte remains in the supernatant and is collected as SamplesOut during the loading step.",
		...
		MagneticBeadSeparationEquilibrationCollectionContainerExpression -> {
			...
			Pattern :> Alternatives[
				ListableP[{_Integer, ObjectP[Model[Container]]}],
				ListableP[{_String, ObjectP[{Object[Container],Model[Container]}]}],
				ListableP[{_String, {_Integer, ObjectP[{Object[Container],Model[Container]}]}}],
				ListableP[ListableP[ListableP[Null]]]
			],
