...
DefineObjectType[Object[Product], {
	...
	Fields -> {
		...
		PackingDensity -> {
			...
			Units ->Centimeter^(-3),
			Description -> "The ratio of the number of items to the volume of the container in which they are packed, when stored in a homogeneous pile. This value is determined empirically by measuring how many objects of this model can fit a fixed volume. For kit products, this number refers to number of complete kit that can be packed in a given volume. For non-kit product, this number refers to number of single items can be packed in a given volume.",
