...
DefineObjectType[Object[Container], {
	...
	Fields -> {
		...
		ResourcesOnLiner -> {
			...
			Pattern :> {_Link, _Link},
			Relation -> {
				Alternatives[Object[Resource, Sample], Object[Sample], Object[Item], Object[Container], Model[Sample], Model[Item], Model[Container], Object[Part], Model[Part], Model[Plumbing], Object[Plumbing], Model[Sensor], Object[Sensor], Model[Wiring], Object[Wiring]],
				Alternatives[Object[Protocol], Object[Maintenance], Object[Qualification]]
			},
			Description -> "Resources whose samples are stored on a liner in this container during the execution of the given protocol (if there is no resource for a sample, points to the sample directly).",
		...
		CurrentTransporterProtocol -> {
			...
			Description -> "Indicates the root protocol this container is used under as a transporter to contain samples requiring special transport conditions.",
			Category -> "Sample Storage",
			Developer -> True
		},
		...
		OEBCompoundDisposal -> {
			...
			Description -> "Indicates the highest occupational exposure band of any sample that the container has contacted or may have been contacted by, which determines the banding level used for its disposal, though it is not necessarily designated for disposal at this time.",
			Category -> "Storage & Handling"
		},
		StoreInverted -> {
			Format -> Single,
		...
		AsepticHandling -> {
			...
			Class -> Boolean,
			Pattern :> BooleanP,
			Description -> "Indicates whether aseptic techniques are followed when moving or using the samples in this container. These techniques include sanitization, autoclaving, sterile filtration, or transferring in a biosafety cabinet during experimentation and storage.",
			Category -> "Storage & Handling"
		},
		...
		TopLevelStorageDestination -> {
			...
			Class -> Boolean,
			Pattern :> BooleanP,
			Description -> "Indicates if the container has a different ProvidedStorageCondition than its container or is itself the highest level object in the storage tree. This should generally be set to True for cabinets and other static objects, and Null or False for mobile objects such as racks and shelves. When set to True, the container will not inherit of the ProvidedStorageCondition from its container if moved to a different area of storage. This field is also used by storage task to identify fallback locations.",
			Category -> "Storage Information"
		},
		Parafilm -> {
			Format -> Single,
