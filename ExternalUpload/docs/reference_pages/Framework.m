...
DefineUsage[installDefaultUploadFunction,
	{
		...
		MoreInformation -> {
			...
			"If no AuxiliaryPacketsFunction is provided, no auxiliary packets are generated by the placeholder generateDefaultUploadFunctionAuxiliaryPackets function.",
			...
			"The function performs the following actions: Input validation, option resolution, upload packet generation, auxiliary packet generation, voq style error checking.",
...
(* ::Subsection:: *)
(* RunOptionValidationTests *)
DefineUsage[uploadVerified,
	{
		BasicDefinitions -> {
			{
				Definition -> {"uploadVerified[objectsToVerify]", "updatedObjects"},
				Description -> "Change the Verified field and append to VerifiedLog field.",
				Inputs :> {
					IndexMatching[
						{
							InputName -> "objectsToVerify",
							Description -> "The objects which the Verified field need to be changed.",
							Widget -> Widget[
								Type -> Object,
								Pattern :> ObjectP[]
							],
				...
				Outputs :> {
					{
						...
						Description -> "The list of objects changed.",
						Pattern :> ObjectP[]
					}
				}
			}
...
(* ::Subsection:: *)
(*combineEHSFields*)
DefineUsage[combineEHSFields,
	{
		BasicDefinitions -> {
			{"combineEHSFields[ehsField, sourceEHSValue, destinationEHSValue]", "combinedValue", "combines two environmental health and safety field values for the specified 'ehsField'."},
			{"combineEHSFields[composition, ehsFields, amount, cache]", "fieldValues", "calculates combined environmental health and safety 'fieldValues' based on sample 'composition' and component percentages."},
			{"combineEHSFields[ehsField, sourceObject, destinationObject, transferAmount, destinationAmount, existingCache]", "updatedValue", "updates destination environmental health and safety value when transferring material from 'sourceObject' to 'destinationObject'."}
		},
		MoreInformation -> {
			"The function ensures proper safety classifications are maintained when combining samples with different hazard profiles.",
			"Composition-based calculations convert concentrations to mass percentages for proper weighting.",
			"Properties are typically combined by assuming the worst case scenario. For example, BSL-2 > BSL-1, Flammable -> True > Flammable -> False and shortest shelf life fields are used."
		},
		Input :> {
			{"ehsField", _Symbol, "The EHS field being combined."},
			{"sourceEHSValue", _, "The EHS value from the source object."},
			{"destinationEHSValue", _, "The EHS value from the destination object."},
			{"composition", {{CompositionP | Null, ObjectP[] | Null}..}, "List of sample components and relative amounts."},
			{"ehsFields", {_Symbol..}, "List of EHS field symbols to calculate."},
			{"amount", VolumeP | MassP | CountP | Null, "Total amount of the sample."},
			{"cache", _List, "Pre-downloaded object data."},
			{"sourceObject", ObjectP[], "Source sample object for transfer."},
			{"destinationObject", ObjectP[], "Destination sample object for transfer."},
			{"transferAmount", MassP | VolumeP, "Amount being transferred."},
			{"destinationAmount", MassP | VolumeP, "Existing amount in destination."},
			{"existingCache", _List, "Pre-downloaded object data for transfer calculation."}
		},
		Output :> {
			{"combinedValue", _Rule, "The combined EHS field -> value."},
			{"fieldValues", {_Rule..}, "List of field -> value for the calculated EHS fields."},
			{"updatedValue", {_, {PacketP[], PacketP[]}}, "The updated EHS field value for the destination after transfer, along with updated packets for the source and destination respectively containing the EHSPercentages field."}
		},
		SeeAlso -> {
			"UploadSample",
			"UploadSampleModel",
...
(*approximateDensity*)
DefineUsage[approximateDensity,
	{
		BasicDefinitions -> {
			{"approximateDensity[compositionTuples]", "density", "estimates the 'density' of a mixture based on the 'compositionTuples' containing composition percentages and identity model packets."},
			{"approximateDensity[volumeTuples]", "density", "estimates the 'density' of a mixture based on the 'volumeTuples' containing volumes and identity model packets."}
		},
		MoreInformation -> {
			"Calculates approximate mixture density using a weighted mass contribution approach based on component densities and concentrations.",
			"Components with low concentrations (less than 5 MassPercent, 5 VolumePercent, 0.5 Mole / Liter, or 10 Gram / Liter) are filtered out as they do not significantly affect density.",
			"If no density information is available for a component, the density of water at room temperature (0.997 Gram / Milliliter) is used as the default.",
			"For the volume-based overload, volumes are first converted to volume percentages, then processed using the composition-based algorithm.",
			"When transferring zero volumes or when no valid density data is available, the function defaults to the density of water.",
			"The function handles mixed composition types (MassPercent, VolumePercent, concentration units) by converting them to mass contributions for density calculation."
		},
		Input :> {
			{"compositionTuples", {{CompositionP | Null, PacketP[] | Null}..}, "List of tuples containing composition percentages and corresponding identity model packets."},
			{"volumeTuples", {{VolumeP, PacketP[] | Null}..}, "List of tuples containing volumes and corresponding identity model packets."}
		},
		Output :> {
			{"density", DensityP, "The estimated density of the mixture."}
		},
		SeeAlso -> {
			"UploadSample",
			"combineEHSFields",
		...
		Author -> {"david.ascough", "dima", "steven", "simon.vu"}
	}
];
