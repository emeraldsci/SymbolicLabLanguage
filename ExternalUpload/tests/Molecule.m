	...
	UploadMolecule,
	{
		...
		Example[{Basic, "Upload a new Model[Molecule] of Lysine by its CAS:"},
			UploadMolecule["56-87-1",
				Name -> "Lysine (test for UploadMolecule) 4 " <> $SessionUUID,
				IncompatibleMaterials -> {None}, 
				MSDSRequired -> False
			],
			ObjectP[Model[Molecule]]
		...
		Example[{Options, "Synonyms", "Use the Synonyms option to set the synonyms of this uploaded chemical:"},
			UploadMolecule["56-87-1",
				Name -> "Lysine (test for UploadMolecule) 14 " <> $SessionUUID,
				Synonyms -> {"Lysine (test for UploadMolecule) 14 " <> $SessionUUID, "L-lysine", "lysine", "Lysine", "56-87-1", "L lysine", "Lys", "K"},
				IncompatibleMaterials -> {None},
				MSDSRequired -> False
			],
			ObjectP[Model[Molecule, "Lysine (test for UploadMolecule) 14 " <> $SessionUUID]]
		],
		...
		Example[{Options, "MeltingPoint", "Use the MeltingPoint option to provide the temperature at which the solid form of Tyrosine will melt:"},
			UploadMolecule["Tyrosine",
				...
				NFPA -> {2, 1, 0, {}},
				DOTHazardClass -> "Class 0",
				MSDSFile -> "https://assets.thermofisher.com/directwebviewer/private/results.aspx?page=NewSearch&LANGUAGE=d__EN&SUBFORMAT=d__AGHS&SKU=ALFAAL06228&PLANT=d__ALF"
			],
		...
		Example[{Options, "BiosafetyLevel", "Use the BiosafetyLevel option to specify the biosafety level of this chemical sample, if applicable:"},
			UploadMolecule["76862-65-2",
				...
				Name -> "Conotoxin (test for UploadMolecule) 52 " <> $SessionUUID,
				IncompatibleMaterials -> {None},
				State -> Solid,
				MSDSFile -> "https://assets.thermofisher.com/directwebviewer/private/results.aspx?page=NewSearch&LANGUAGE=d__CE&SUBFORMAT=d__AGHS&SKU=ALFAAJ66628&PLANT=d__ALF",
				NFPA -> {4, 0, 0, Null},
				Flammable -> False,
				DOTHazardClass -> "Class 6 Division 6.1 Toxic Substances Hazard"
			],
			ObjectP[Model[Molecule, "Conotoxin (test for UploadMolecule) 52 " <> $SessionUUID]]|{}|Null,
			TimeConstraint -> 1200 (* 20 minute time limit because we want retry logic with a pause between *)
		],
		...
		Example[{Options, ModifyInputModel, "When a Model[Molecule] is used as input, set this option to False to use the input molecule as a template and upload a new molecule object:"},
			...
			obj:ObjectP[Model[Molecule]]/;With[
				{fields = {UNII, State, MolecularWeight, ExactMass, Flammable, NFPA, DOTHazardClass}},
				And[
					MatchQ[
						Download[obj, fields],
			...
			Flatten@Lookup[
				{
					...
					UploadMolecule[Molecule["Water"], Name -> "Test molecule for UploadMolecule 74 " <> $SessionUUID, IncompatibleMaterials -> {None}, Upload -> False, Output -> Options]
				},
				ModifyInputModel
			],
			{False, True, Null}
		...
		Test["Ensure MolecularFormula retains the correct capitalization of elements:",
			UploadMolecule["100-58-3",
				...
				State -> Solid,
				IncompatibleMaterials -> {None},
				MSDSFile -> "https://assets.thermofisher.com/DirectWebViewer/private/document.aspx?prd=ALFAA87326~~PDF~~MTR~~CLP1~~EN~~2024-11-30%2016:50:48~~Phenylmagnesium%20bromide%20%203M%20in%20ether~~",
				DOTHazardClass -> "Class 4 Division 4.3 Dangerous when Wet Hazard",
				NFPA -> {3, 3, 1, {}}
			];
		...
		Example[{Options, "Monatomic", "Indicates if this molecule contains exactly one atom."},
			UploadMolecule["7440-59-7",
				...
				Monatomic -> True,
				NFPA -> {1, 0, 0, {}},
				MSDSFile -> "https://assets.thermofisher.com/directwebviewer/private/results.aspx?page=NewSearch&LANGUAGE=d__EN&SUBFORMAT=d__AGHS&SKU=ALFAAR21760&PLANT=d__ALF"
			];
		...
		Example[{Messages, "Error::APIConnection", "If the given URL cannot be connected, throw errors."},
			UploadMolecule["https://www.sigmaaldrich.com/catalog/product/aldrich/abcdtestnotexistblahblah", Name -> "abcdtestnotexistblahblah" <> $SessionUUID,
		...
		Example[{Messages, "MoleculeExists", "Throws an error if a molecule already exists in the database that shares unique molecular identifiers:"},
			UploadMolecule["7440-59-7", Name -> "abcdtestnotexistblahblah2" <> $SessionUUID,
				IncompatibleMaterials -> {None}, NFPA -> {4, 3, 1, {}},
				DOTHazardClass -> "Class 3 Flammable Liquids Hazard"
			],
			Null,
	...
	UploadMoleculeOptions,
	{
		...
		Example[{Basic, "Inspect the resolved options when uploading a new Model[Molecule] of Lysine by its CAS:"},
			UploadMoleculeOptions["56-87-1",
				Name -> "Lysine (test for UploadMoleculeOptions) 4 " <> $SessionUUID,
				IncompatibleMaterials -> {None},
				MSDSRequired -> False,
				OutputFormat -> List
			],
...
DefineTests[
	ValidUploadMoleculeQ,
	{
		Example[{Basic, "Make sure that the resulting object will be valid when uploading a Model[Molecule] of DMSO by its name:"},
			ValidUploadMoleculeQ["DMSO", Name -> "DMSO Sample #1", IncompatibleMaterials -> {None}, MSDSFile -> "https://www.caymanchem.com/msdss/700001m.pdf"],
			True
		],
		Example[{Basic, "Make sure that the resulting object will be valid when uploading a new Model[Molecule] of Aspartic Acid by its PubChem ID:"},
			ValidUploadMoleculeQ[PubChem[5960], Name -> "Aspartic Acid Sample #2", IncompatibleMaterials -> {None}],
			True
		],
		Example[{Basic, "Make sure that the resulting object will be valid when uploading a new Model[Molecule] of L-Serine by its InChI:"},
			ValidUploadMoleculeQ["InChI=1S/C3H7NO3/c4-2(1-5)3(6)7/h2,5H,1,4H2,(H,6,7)/t2-/m0/s1", Name -> "L-Serine Sample #3", IncompatibleMaterials -> {None}],
			True
		],
		Example[{Basic, "Make sure that the resulting object will be valid when uploading a new Model[Molecule] of Lysine by its CAS:"},
			ValidUploadMoleculeQ["56-87-1", Name -> "Lysine Sample #3", IncompatibleMaterials -> {None}, MSDSRequired -> False],
			True
		],
		...
		Example[{Options, "Verbose", "Set Verbose->True to see all of the tests that ValidUploadMoleculeQ is running:"},
			ValidUploadMoleculeQ["DMSO", Name -> "DMSO Sample #1", IncompatibleMaterials -> {None}, MSDSFile -> "https://www.caymanchem.com/msdss/700001m.pdf", Verbose -> True],
			True
		],
		Example[{Options, "Verbose", "Set Verbose->Failures to see all of the tests that did not pass when running ValidUploadMoleculeQ:"},
			ValidUploadMoleculeQ["DMSO", Name -> "DMSO Sample #1", IncompatibleMaterials -> {None}, MSDSFile -> "https://www.caymanchem.com/msdss/700001m.pdf", Verbose -> Failures],
			True
		],
		Example[{Options, "Verbose", "Set Verbose->False to see none of the tests when running ValidUploadMoleculeQ (this is the default behavior):"},
			ValidUploadMoleculeQ["DMSO", Name -> "DMSO Sample #1", IncompatibleMaterials -> {None}, MSDSFile -> "https://www.caymanchem.com/msdss/700001m.pdf", Verbose -> False],
			True
		],
		Example[{Options, "OutputFormat", "Set OutputFormat->TestSummary to have the function return a summary of the tests that it ran. Use the Keys[...] function on this test summary to see the keys inside of it:"},
			ValidUploadMoleculeQ["DMSO", Name -> "DMSO Sample #1", IncompatibleMaterials -> {None}, MSDSFile -> "https://www.caymanchem.com/msdss/700001m.pdf", OutputFormat -> TestSummary][Successes],
			{_EmeraldTestResult..}
		]
	},
	TurnOffMessages :> {
