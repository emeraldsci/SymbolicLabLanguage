	...
	Options :> {
		IndexMatching[
			...
			},
			{
				...
				AllowNull -> False,
				Widget -> Widget[Type -> Enumeration, Pattern :> BooleanP],
				Description -> "Indicates if this sample is in capsule form.",
				Category -> "Physical Properties"
			},
			{
				OptionName -> Sachet,
...
resolveUploadSampleModelOptions[myType : Model[Sample], myInputs_List, myMapThreadSafeOptions : {{___}..}, myMapThreadSpecifiedOptions : {{___}..}] := Module[
	...
	optionsWithManuallyResolved = Module[
		{
			resolvedNotebooks, resolvedSynonyms, resolvedNames, resolvedSampleHandling, manuallyResolvedOptions
		},
    ...
    (* Resolve sample handling. *)
    (* If SampleHandling is Automatic, and Tablet or Capsule is set to True, then SampleHandling should be Itemized. *)
    resolvedSampleHandling = MapThread[
      Function[{tabletQ, capsuleQ, inputSampleHandling},
        If[
          Or[
            MatchQ[{tabletQ, inputSampleHandling}, {True, Automatic}],
            MatchQ[{capsuleQ, inputSampleHandling}, {True, Automatic}]
          ],
          Itemized,
          inputSampleHandling
        ]
      ],
      Transpose[Lookup[optionsWithBio, {Tablet, Capsule, SampleHandling}]]
    ];
        ...
        SampleHandling -> #4
			|> &,
			{resolvedNotebooks, resolvedNames, resolvedSynonyms, resolvedSampleHandling}
		];
