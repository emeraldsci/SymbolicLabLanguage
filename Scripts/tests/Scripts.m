...
DefineTests[ExperimentScript,
	{
		Example[{Basic,"Make a new script using an existing object as a template:"},
			ExperimentScript["New Script 1 for ExperimentScript "<>$SessionUUID,Template->Object[Notebook,Script,"Existing Script 1 for ExperimentScript "<>$SessionUUID]],
			ObjectP[Object[Notebook,Script]]
		],
		...
		Example[{Basic,"Generated script notebook should have StyleDefinitions=CommandCenter.nb"},
			Module[{myNotebook, myNotebookCloudFile, styleDefinitions},
				myNotebook = ExperimentScript["New Script 2 for ExperimentScript "<>$SessionUUID<>$SessionUUID,Template->Object[Notebook,Script,"Existing Script 1 for ExperimentScript "<>$SessionUUID]];
				myNotebookCloudFile = ImportCloudFile[myNotebook[PendingNotebookFile]];
				styleDefinitions = Lookup[Options[myNotebookCloudFile, StyleDefinitions], StyleDefinitions];
				MatchQ[styleDefinitions, "CommandCenter.nb"]
			],
			True
		],
		Example[{Additional,"Indicate that two protocols should be forked off and that the script should NOT wait for completion before running the next line:"},
		...
		Example[{Options,TimeConstraint,"Create a new script which will throw an error if the next step within the script takes more than 30 minutes to evaluate:"},
			ExperimentScript["New Script 3 for ExperimentScript "<>$SessionUUID,Template->Object[Notebook,Script,"Existing Script 1 for ExperimentScript "<>$SessionUUID],TimeConstraint->30 Minute],
			ObjectP[Object[Notebook,Script]]
		],
		Example[{Options,IgnoreWarnings,"Indicate that the script should continue even if calls within throw warnings:"},
			ExperimentScript["New Script 4 for ExperimentScript "<>$SessionUUID,Template->Object[Notebook,Script,"Existing Script 1 for ExperimentScript "<>$SessionUUID],IgnoreWarnings->True],
			ObjectP[Object[Notebook,Script]]
		],
		Example[{Options,Template,"Use an existing object as a template:"},
			ExperimentScript["New Script 5 for ExperimentScript "<>$SessionUUID,Template->Object[Notebook,Script,"Existing Script 1 for ExperimentScript "<>$SessionUUID]],
			ObjectP[Object[Notebook,Script]]
		],
	...
	SymbolSetUp:>Module[{templateNotebookFile,templateNotebook,cloudFile},
		...
		Module[
			...
			objects = {
				Object[Notebook,Script,"Existing Script 1 for ExperimentScript "<>$SessionUUID],
				Object[Notebook,Script,"New Script 1 for ExperimentScript "<>$SessionUUID],
				Object[Notebook,Script,"New Script 2 for ExperimentScript "<>$SessionUUID],
				Object[Notebook,Script,"New Script 3 for ExperimentScript "<>$SessionUUID],
				Object[Notebook,Script,"New Script 4 for ExperimentScript "<>$SessionUUID],
				Object[Notebook,Script,"New Script 5 for ExperimentScript "<>$SessionUUID]
			};
		...
		Upload[
			<|
				...
				Name->"Existing Script 1 for ExperimentScript "<>$SessionUUID,
