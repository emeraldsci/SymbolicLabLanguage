...
DefineTests[
	...
	(* ExperimentCentrifuge *)
		...
		Example[{Messages, "CentrifugePrecision", "Throws a warning if the centrifuge intensity applied to the samples prior to starting the experiment needs rounding:"},
			options = ExperimentFluorescenceSpectroscopy[Object[Container, Plate, "Test plate 1 for ExperimentFluorescenceSpectroscopy"<>$SessionUUID], CentrifugeIntensity -> 1001 RPM, Output -> Options];
			Lookup[options, CentrifugeIntensity],
			1000 RPM,
			EquivalenceFunction -> Equal,
			Variables :> {options},
	...
	(* aliquot options *)
		...
		Example[{Messages, "AliquotAmountPrecision", "Throw a warning and rounds the amount option if the value is more precise than the achievable precision:"},
			options = ExperimentFluorescenceSpectroscopy[Object[Sample, "Test sample 7 for ExperimentFluorescenceSpectroscopy"<>$SessionUUID], AliquotAmount -> 100.01 Microliter, Output -> Options];
			Lookup[options, AliquotAmount],
			100 Microliter,
			EquivalenceFunction -> Equal,
			Variables :> {options},
