...
DefineTests[
	Experiment`Private`SimulateResources,
	{
		...
		Example[{Options, IgnoreWaterResources, "If IgnoreWaterResources -> True, then don't simulate the water resources and leave them as models if $ExperimentWaterPreparation = False:"},
			testID = SimulateCreateID[Object[Maintenance, Handwash]];
			RequireResources[
				<|
					Object -> testID,
		...
		Example[{Options, IgnoreWaterResources, "If IgnoreWaterResources -> True, then don't simulate the water resources and leave them as models if $ExperimentWaterPreparation = True:"},
			testID = SimulateCreateID[Object[Maintenance, Handwash]];
			RequireResources[
				<|
					Object -> testID,
			...
			TearDown :> (ExitSimulation[]),
			Stubs:>{
				ProcedureFramework`Private`$ExperimentWaterPreparation = True,
				$PersonID=Object[User,"Test user for notebook-less test protocols"],
				$SearchMaxDateCreated=(Now-1Day)
			}
		],
