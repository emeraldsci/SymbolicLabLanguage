...
DefineTests[
	ExperimentRoboticSamplePreparation,
	{
		...
		Example[{Additional, "Robotic test call with 2 WashPlate unit operations with the multiple buffers specified by LabelSample:"},
			Module[{protocol},
				protocol = ExperimentRoboticSamplePreparation[{
					LabelContainer[
						Label -> {"my plate 1"},
					...
					LabelSample[
						Sample -> {Model[Sample, "Milli-Q water"], Model[Sample, StockSolution, "1x PBS from 10X stock"]},
						Label -> {"my Buffer1", "my Buffer2"},
						Container -> Model[Container, Vessel, "2L Glass Bottle"],
						Amount -> {1.9 Liter, 1.5 Liter}
					],
					WashPlate[
						Sample -> "my plate 1",
						Priming -> True,
			...
			{ObjectP[Model[Sample, "Milli-Q water"]], ObjectP[Model[Sample, StockSolution, "1x PBS from 10X stock"]], Null, Null}
		],
		...
		Example[{Additional, "Robotic test call with 1 WashPlate unit operation results in RequiredResources backlink to PlateWasherBufferA:"},
			...
			{
				{{{ObjectP[Model[Sample, StockSolution, "1x PBS from 10X stock"]]}, {ObjectP[Model[Container, Vessel, "2L Glass Bottle"]]}, EqualP[626 Milliliter]}},
				ObjectP[Model[Sample, StockSolution, "1x PBS from 10X stock"]],
				{BufferA, ObjectP[Model[Sample, StockSolution, "1x PBS from 10X stock"]]}
			}
		],
		Example[{Additional, "Robotic WashPlate unit operation does not accept NIMBUS PlateWasher as Instrument:"},
			ExperimentRoboticSamplePreparation[{
				LabelContainer[
					Label -> {"my plate 1"},
					Container -> {Model[Container, Plate, "96-well Polystyrene Flat-Bottom Plate, Clear"]}
    ...
    Example[{Additional, MultiProbeHead, "Robotic Transfer automatically recognizes MultiProbeHead transfers of the full plate (part 5):"} ,
      ...
      HardwareConfiguration -> HighRAM
		...
		(* -- Filter plate holder resource tests -- *)

		Test["Adds one holder plate resource per filter plate present on deck:",
			Module[{protocol, requiredObjects},
				(* Create a protocol that places a filter plate on deck via the Filter primitive *)
				protocol = ExperimentRoboticSamplePreparation[{
					LabelContainer[
						Label -> "filter plate",
						Container -> Model[Container, Plate, Filter, "Plate Filter, PES, 0.22um, 0.3mL"]
					...
					LabelContainer[
						Label -> "second filter plate",
						Container -> Model[Container, Plate, Filter, "Plate Filter, PES, 0.22um, 0.3mL"]
					],
					Transfer[
						Source -> Model[Sample, "Milli-Q water"],
						Destination -> "filter plate",
						DestinationWell -> "A1",
						Amount -> 100 Microliter
					]
				}];

				(* Download the RequiredObjects field to inspect what resources were added *)
				requiredObjects = Download[protocol, RequiredObjects];

				(* Count how many we have *)
				Count[requiredObjects, ObjectP[Model[Container, Plate, "96-well Polystyrene Flat-Bottom Plate, Clear"]]]
			],
			2,
			Stubs :> {
				$PersonID = Object[User, "Test user for notebook-less test protocols"],
				$SearchMaxDateCreated = (Now - 1Day),
				$FilterHolderModel = Model[Container, Plate, "96-well Polystyrene Flat-Bottom Plate, Clear"]
			}
		],

		Test["Does not add a holder plate resource when no filter plates are present on deck:",
			Module[{protocol, requiredObjects},
				(* Create a protocol using only regular plates — no filter plate on deck *)
				protocol = ExperimentRoboticSamplePreparation[{
					Transfer[
						Source -> Model[Sample, "Milli-Q water"],
						Destination -> Model[Container, Vessel, "2mL Tube"],
				...
				(* Download RequiredObjects and confirm no holder model appears *)
				requiredObjects = Download[protocol, RequiredObjects];
		...
		Test["Does not add a holder plate resource when $FilterHolderModel is Null, even if filter plates are present:",
			Module[{protocol},
				(* Create a protocol with a filter plate on deck; $FilterHolderModel is left at its default Null value *)
				protocol = ExperimentRoboticSamplePreparation[{
					LabelContainer[
						Label -> "filter plate",
						Container -> Model[Container, Plate, Filter, "Plate Filter, PES, 0.22um, 0.3mL"]
				...
				(* Confirm the protocol was created successfully — the Null check prevents a holder from being added *)
				MatchQ[protocol, ObjectP[Object[Protocol, RoboticSamplePreparation]]]
			],
			True
		]
	},
...
DefineTests[
	ExperimentManualCellPreparation,
	{
		...
		Example[{Messages, "WorkCellIsIncompatibleWithMethod", "Manual preparation does not use WorkCell:"},
			ExperimentManualCellPreparation[
				PCR[
					Sample->{Object[Container,Plate,"Test mammalian plate 1 (for ExperimentManualCellPreparation)" <> $SessionUUID]},
					WorkCell -> bioSTAR
				]
			],
			$Failed,
...
DefineTests[ExperimentSamplePreparation,
	{
		...
		Example[{Messages, "WorkCellIsIncompatibleWithMethod", "WorkCell cannot be used when the primitives require Manual Preparation:"},
			ExperimentSamplePreparation[{
				SerialDilute[
					Source -> Object[Sample,"Test water sample 1 in 50mL Tube for ExperimentSamplePreparation" <> $SessionUUID],
		...
		Example[{Messages, "WorkCellIsIncompatibleWithMethod", "WorkCell cannot be used when the primitives is specified to use Manual Preparation:"},
			ExperimentSamplePreparation[{
				SerialDilute[
					Source -> Object[Sample,"Test water sample 1 in 50mL Tube for ExperimentSamplePreparation" <> $SessionUUID],
					FinalVolume -> {1000 Microliter},
