...
DefineTests[
	ExperimentFluorescenceKinetics,
	{
		...
		Example[{Options, Instrument, "Instrument is automatically set to Model[Instrument, PlateReader, \"id:zGj91a7Ll0Rv\"] if TargetCarbonDioxideLevel/TargetOxygenLevel is set:"},
			Lookup[
				ExperimentFluorescenceKinetics[Object[Sample,"Test sample 1 for ExperimentFluorescenceKinetics"<>$SessionUUID], TargetCarbonDioxideLevel -> 5 * Percent, Output -> Options],
				Instrument
			],
		...
		Example[{Options, TargetCarbonDioxideLevel, "TargetCarbonDioxideLevel is automatically set to 5 Percent if sample contains Mammalian cells:"},
			Lookup[
				ExperimentFluorescenceKinetics[Object[Sample, "Test sample with mammalian cells for ExperimentFluorescenceKinetics "<>$SessionUUID], Instrument -> Model[Instrument, PlateReader, "id:zGj91a7Ll0Rv"], Output -> Options],
		...
		Example[{Options,CanaryBranch,"Specify the CanaryBranch on which the protocol is run:"},
			ExperimentFluorescenceKinetics[Object[Sample,"Test sample 1 for ExperimentFluorescenceKinetics"<>$SessionUUID],CanaryBranch->"d1cacc5a-948b-4843-aa46-97406bbfc368"][CanaryBranch],
	...
	(* aliquot options *)
		...
		Example[{Options, {PreparedModelContainer, PreparedModelAmount}, "Specify the container in which an input Model[Sample] should be prepared when Preparation is Robotic:"},
			roboticProtocol = ExperimentFluorescenceKinetics[
				(* Red food dye *)
				{Model[Sample, "id:BYDOjvG9z6Jl"], Model[Sample, "id:BYDOjvG9z6Jl"]},
				(* UV-Star Plate*)
				PreparedModelContainer -> Model[Container, Plate, "id:n0k9mGzRaaBn"],
		...
		Example[{Messages, "ObjectDoesNotExist", "Throw a message if we have a sample that does not exist (name form):"},
			ExperimentFluorescenceKinetics[Object[Sample, "Nonexistent sample"]],
			$Failed,
			Messages :> {Download::ObjectDoesNotExist}
		],
		Example[{Messages, "ObjectDoesNotExist", "Throw a message if we have a container that does not exist (name form):"},
			ExperimentFluorescenceKinetics[Object[Container, Vessel, "Nonexistent container"]],
			$Failed,
			Messages :> {Download::ObjectDoesNotExist}
		],
		Example[{Messages, "ObjectDoesNotExist", "Throw a message if we have a sample that does not exist (ID form):"},
			ExperimentFluorescenceKinetics[Object[Sample, "id:12345678"]],
			$Failed,
			Messages :> {Download::ObjectDoesNotExist}
		],
		Example[{Messages, "ObjectDoesNotExist", "Throw a message if we have a container that does not exist (ID form):"},
			ExperimentFluorescenceKinetics[Object[Container, Vessel, "id:12345678"]],
			$Failed,
			Messages :> {Download::ObjectDoesNotExist}
		],
		Example[{Messages, "ObjectDoesNotExist", "Do NOT throw a message if we have a simulated sample but a simulation is specified that indicates that it is simulated:"},
			Module[{containerPackets, containerID, sampleID, samplePackets, simulationToPassIn},
				...
				ExperimentFluorescenceKinetics[sampleID, Simulation -> simulationToPassIn, Output -> Options]
			],
			{__Rule},
		...
		Example[{Messages, "ObjectDoesNotExist", "Do NOT throw a message if we have a simulated container but a simulation is specified that indicates that it is simulated:"},
			Module[{containerPackets, containerID, sampleID, samplePackets, simulationToPassIn},
				...
				ExperimentFluorescenceKinetics[containerID, Simulation -> simulationToPassIn, Output -> Options]
			],
			{__Rule},
			Messages:>{Warning::SinglePlateRequired}
		]
	...
	SymbolSetUp:>Module[{platePacket,vesselPacket,bottlePacket,incompatibleChemicalPacket,
		plate1,plate2,plate3,plate4,plate5,plate6,emptyPlate,vessel1,vessel2,vessel3,vessel4,vessel5,bottle1,
		...
		{plate1,plate2,plate3,plate4,plate5,plate6,emptyPlate,vessel1,vessel2,vessel3,vessel4,vessel5,bottle1,incompatibleChemicalModel}=Upload[
			Join[
				Append[platePacket,Name->"Test plate "<>ToString[#]<>" for ExperimentFluorescenceKinetics"<>$SessionUUID]&/@Range[6],
		...
		samples=UploadSample[
			...
			],
			{
				...
				{"A1",vessel1},{"A1",vessel2},{"A1",vessel3},{"A1",vessel4},{"A1",plate5},{"A1",plate6}
			},
			Name->Join[sampleNames,injectionSampleNames,{"High volume sample for ExperimentFluorescenceKinetics testing"<>$SessionUUID, "Test sample with mammalian cells for ExperimentFluorescenceKinetics "<>$SessionUUID}],
			InitialAmount->Join[
...
fkBackUpCleanup[]:=Module[{namedObjects,lurkers},
	namedObjects=Flatten@{
		...
		Object[Sample,"Test sample with mammalian cells for ExperimentFluorescenceKinetics "<>$SessionUUID],
		...
		(Object[Container, Plate, "Test plate "<>ToString[#]<>" for ExperimentFluorescenceKinetics"<>$SessionUUID]&)/@Range[6],
