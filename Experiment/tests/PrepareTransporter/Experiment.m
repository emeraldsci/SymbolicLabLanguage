DefineTests[ExperimentPrepareTransporter,
	{
		...
		Example[{Options, LinerRequired, "If LinerRequired -> True, Function will resolve Model[Container, Rack] and Liners to transport the input sample:"},
			Download[
				ExperimentPrepareTransporter[
					{Model[Container, Plate, "Test ExposedSurface container for ExperimentPrepareTransporter unit test "<>$SessionUUID]},
					TransportTemperature -> Null,
					LinerRequired -> True,
					ParentProtocol -> Object[Protocol, HPLC, "Test protocol 1 for ExperimentPrepareTransporter" <> $SessionUUID]
				],
		...
		Module[{},
			...
			Upload[{
				...
				Association[
					...
					Name -> "Test ExposedSurface container for ExperimentPrepareTransporter unit test "<>$SessionUUID,
					ExposedSurfaces -> True,
					ExposedInterior -> True,
					DeveloperObject -> True,
					Dimensions -> {10 Centimeter, 5 Centimeter, 2 Centimeter}
				]
			}];
