...
DefineTests[ExperimentELISA,
	{
		...
		Example[{Basic, "Accepts a liquid sample object and generates a protocol using a coated ELISA Plate:"},
			...
			ObjectP[Object[Protocol, ELISA]],
			Stubs :> {
				Search[Model[Container, Plate], ___, SubTypes -> False] = {Model[Container, Plate, "Coated Plate Model for ExperimentELISA tests" <> $SessionUUID]}
			}
		],
		...
		Example[{Messages, "CentrifugePrecision", "Throws a warning if the centrifuge intensity applied to the samples prior to starting the experiment needs rounding:"},
			options=ExperimentELISA[Object[Sample,"ExperimentELISA test object sample 1" <> $SessionUUID],CentrifugeIntensity->1001RPM,Output->Options];
			Lookup[options,CentrifugeIntensity],
			1000*RPM,
			EquivalenceFunction->Equal,
			Variables:>{options},
		...
		Example[{Messages, "AliquotAmountPrecision", "Throw a warning and rounds the amount option if the value is more precise than the achievable precision:"},
			options = ExperimentELISA[Object[Sample, "ExperimentELISA test object sample 1" <> $SessionUUID] ,AliquotAmount -> 0.5001 Milliliter, Output -> Options];
			Lookup[options, AliquotAmount],
			0.5 Milliliter,
			EquivalenceFunction -> Equal,
			Variables :> {options},
		...
		Block[{$AllowSystemsProtocols = True},
			Module[
				...
				(* Object samples, Target Antigen Objects, Antibody Objects *)
				{
					...
					InitialAmount -> Join[ConstantArray[1.8 Milliliter, 14], {50 Milliliter, 100 Milliliter, 1 Milliliter, 1 Milliliter}],
