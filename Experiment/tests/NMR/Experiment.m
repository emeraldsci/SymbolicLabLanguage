...
DefineTests[ExperimentNMR,
	{
		...
		Example[{Messages, "CentrifugePrecision", "Throws a warning if the centrifuge intensity applied to the samples prior to starting the experiment needs rounding:"},
			options = ExperimentNMR[Object[Sample, "ExperimentNMR New Test Chemical 1 (100 uL)" <> $SessionUUID], CentrifugeIntensity -> 1001 RPM, Output -> Options];
			Lookup[options, CentrifugeIntensity],
			1000 RPM,
			EquivalenceFunction -> Equal,
			Variables :> {options},
		...
		Example[{Options, FiltrationType, "Set the FiltrationType option:"},
			options = ExperimentNMR[Object[Sample, "ExperimentNMR New Test Chemical 8 (1.5 mL)" <> $SessionUUID], FiltrationType -> Syringe, Output -> Options];
			Lookup[options, FiltrationType],
			Syringe,
			Variables :> {options}
		],
		Example[{Options, FilterInstrument, "Set the FilterInstrument option:"},
			options = ExperimentNMR[Object[Sample, "ExperimentNMR New Test Chemical 8 (1.5 mL)" <> $SessionUUID], FilterInstrument -> Model[Instrument, SyringePump, "NE-1010 Syringe Pump"], Output -> Options];
			Lookup[options, FilterInstrument],
			ObjectP[Model[Instrument, SyringePump, "NE-1010 Syringe Pump"]],
			Variables :> {options}
		],
		Example[{Options, Filter, "Set the Filter option:"},
			options = ExperimentNMR[Object[Sample, "ExperimentNMR New Test Chemical 8 (1.5 mL)" <> $SessionUUID], Filter -> Model[Item,Filter,"Disk Filter, PES, 0.22um, 30mm"], Output -> Options];
		...
		Example[{Options, FilterSyringe, "Set the FilterSyringe option:"},
			options = ExperimentNMR[Object[Sample, "ExperimentNMR New Test Chemical 8 (1.5 mL)" <> $SessionUUID], FiltrationType -> Syringe, FilterSyringe -> Model[Container, Syringe, "20mL All-Plastic Disposable Luer-Lock Syringe"], Output -> Options];
		...
		Example[{Messages, "AliquotAmountPrecision", "Throw a warning and rounds the amount option if the value is more precise than the achievable precision:"},
			options = ExperimentNMR[Object[Sample, "ExperimentNMR New Test Chemical 1 (100 uL)" <> $SessionUUID], AliquotAmount -> 0.08101 Milliliter, Output -> Options];
			Lookup[options, AliquotAmount],
			81 Microliter,
			EquivalenceFunction -> Equal,
			Variables :> {options},
