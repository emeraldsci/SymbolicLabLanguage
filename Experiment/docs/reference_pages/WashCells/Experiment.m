    ...
    BasicDefinitions -> {
      {
        ...
        Description->"creates a 'Protocol' object to change the media of the `Samples` containing living cells in order to replenish nutrients required for the cell growth.",
        Inputs:>{
          IndexMatching[
            {
              InputName -> "Samples",
        ...
        Outputs:>{
          {
            OutputName->"Protocol",
            Description->"The protocol object(s) describing how to preform the cell media change procedure.",
            Pattern:>ListableP[ObjectP[Object[Protocol, RoboticCellPreparation]]]
          }
        }
      }
...
DefineUsage[ChangeMedia,
  {
    BasicDefinitions -> {
      {"ChangeMedia[changeMediaRules]","primitive","generates an ExperimentRoboticCellPreparation-compatible 'primitive' that describes a wash cells process."}
    },
    Input:>{
      {
        "changeMediaRules",
    ...
    Output:>{
      {"primitive",_ChangeMedia,"A robotic cell preparation primitive containing information for specification and execution of a general wash cells process."}
    },
    Sync -> Automatic,
    SeeAlso -> {
      "ExperimentRoboticCellPreparation",
