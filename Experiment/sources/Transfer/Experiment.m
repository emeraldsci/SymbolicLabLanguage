		...
		Function[{sourceObject, sourceContainerObject, sourceContainerModelObject, destinationObject, destinationContainerObject, destinationContainerModelObject, collectionContainerSample, sourceIsModelQ, amount, destinationIsWasteQ, options, manipulationIndex, sourceInput, destinationInput},
			Module[
				...
				transferEnvironment = Which[
					...
					(* Are we doing FTV into a VolumetricFlask? If so, resolve to $TransferBalanceBenchModel so we can use IRProbe to ensure we are 18-28C during transfer to VolFlask*)
					...
					(MatchQ[Lookup[options,FillToVolume],True] || MatchQ[Lookup[options,ParentProtocol],ObjectP[Object[Protocol,FillToVolume]]]) && MatchQ[destinationContainerModelPacket,ObjectP[{Model[Container,Vessel,VolumetricFlask],Object[Container,Vessel,VolumetricFlask]}]],
						Download[$TransferBalanceBenchModel ,Object],
					...
					MatchQ[Lookup[options, Balance], ObjectP[Model[Instrument, Balance]]],
						If[
							...
							(* Otherwise, both Macro and Analytical balances can be found for the bench model: $TransferBalanceBenchModel *)
				...
				compatibleBalanceModels[balanceModelOptions:OptionsPattern[]]:=Module[{containerWeight, totalMassCompatibleBalanceModels, totalMassWithBufferCompatibleBalanceModels, transferMassCompatibleBalanceModels, massCompatibleBalanceModels, filteredMassCompatibleBalanceModels,availableBalanceModels,finalBalanceModels,finalBalanceModelPackets},
					...
					availableBalanceModels=Which[
						...
						MatchQ[transferEnvironment, ObjectP[Model[Container, Bench]]],
							fastAssocLookup[fastAssoc, #, {Model, Object}]& /@ Cases[Flatten[{fastAssocLookup[fastAssoc, $TransferBalanceBenchModel, {Objects, Balances}]}],ObjectP[]],
				...
				Module[{specifiedTransferEnvironment, transferEnvironmentObjects,transferEnvironmentObjectPackets,balancesInTransferEnvironmentObjects,balanceModelsInTransferEnvironmentObjects},
					...
					transferEnvironmentObjects=Which[
						...
						MatchQ[specifiedTransferEnvironment, ObjectP[Model[Container, Bench]]],
							(* Following the same logic as compatibleBalanceModels, we turn our bench request to be $TransferBalanceBenchModel request. We will turn it into the real bench model in resource packets  *)
							Flatten[fastAssocLookup[fastAssoc, $TransferBalanceBenchModel, Objects]],
						...
						Which[
										...
										Link[
											Resource[
												Sample->Which[
													...
													MemberQ[Flatten@groupedBalances,ObjectP[]] && MemberQ[groupedTransferEnvironments, ObjectP[{$TransferBalanceBenchModel, $TransferEnclosureModel, Object[]}]],
														First@groupedTransferEnvironments,
													(* If we will need a balance or thermometer some point, use the magical transfer bench *)
													MemberQ[Flatten@groupedBalances,ObjectP[]] || MemberQ[Flatten@groupedSourceTemperatures,TemperatureP]||MemberQ[Flatten@groupedDestinationTemperatures,TemperatureP],
							...
							MatchQ[First[groupedTransferEnvironments], ObjectP[$TransferFumeHoodModel]],
							{
								...
								groupedBalances/.(
									(
										ObjectP[#]->Resource[
											(* We are making our balance models exchangable across Model[Instrument, Balance, "id:vXl9j5qEnav7"] - Ohaus Pioneer PA124, Model[Instrument, Balance, "id:KBL5DvYl3zGN"] - Ohaus Pioneer PA224, Model[Instrument, Balance, "id:rea9jl5Vl1ae"] - Ohaus EX225AD. They are essentially the same and we need to allow all models. *)
											(* Ohaus Pioneer 224 is also being added to this list - Model[Instrument, Balance, "id:KBL5DvYl3zGN"] *)
											Instrument->#/.{(Model[Instrument, Balance, "id:vXl9j5qEnav7"] | Model[Instrument, Balance, "id:KBL5DvYl3zGN"] | Model[Instrument, Balance, "id:rea9jl5Vl1ae"])->{Model[Instrument, Balance, "id:vXl9j5qEnav7"], Model[Instrument, Balance, "id:KBL5DvYl3zGN"], Model[Instrument, Balance, "id:rea9jl5Vl1ae"]}},
											Name->CreateUUID[],
											Time->(5 Minute * Length[Cases[groupedBalances, ObjectP[#]]])
										]&)/@DeleteDuplicates[Download[Cases[groupedBalances, ObjectP[]], Object]]
								)
										...
										Which[
											...
											MatchQ[balance, ObjectP[]]&&!MatchQ[sourceTemperature,TemperatureP]&&!MatchQ[destinationTemperature,TemperatureP],
												Module[{balancesInTransferEnvironmentObjects,equivalentBalances},
													...
													(* We are making our balance models exchangable across Model[Instrument, Balance, "id:vXl9j5qEnav7"] - Ohaus Pioneer PA124, Model[Instrument, Balance, "id:KBL5DvYl3zGN"] - Ohaus Pioneer PA224, Model[Instrument, Balance, "id:rea9jl5Vl1ae"] - Ohaus EX225AD. They are essentially the same and we need to allow all models. *)
													(* Ohaus Pioneer 224 is also being added to this list - Model[Instrument, Balance, "id:KBL5DvYl3zGN"] *)
													equivalentBalances=balance/.{(Model[Instrument, Balance, "id:vXl9j5qEnav7"] | Model[Instrument, Balance, "id:KBL5DvYl3zGN"] | Model[Instrument, Balance, "id:rea9jl5Vl1ae"])->{Model[Instrument, Balance, "id:vXl9j5qEnav7"], Model[Instrument, Balance, "id:KBL5DvYl3zGN"], Model[Instrument, Balance, "id:rea9jl5Vl1ae"]}},
