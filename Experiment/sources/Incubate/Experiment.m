...
resolveExperimentIncubateNewOptions[mySamples:{ObjectP[Object[Sample]]...},myOptions:{_Rule...},myResolutionOptions:OptionsPattern[resolveExperimentIncubateNewOptions]]:=Module[
	...
	vortexInstrumentPackets,shakeInstrumentPackets,rollInstrumentPackets,sonicationInstrumentPackets,heatBlockInstrumentPackets,homogenizerInstrumentPackets,sonicationContainerPackets,
	samplePackets,sampleContainerModelPackets,bottleRollerInstrumentPackets,stirInstrumentPackets,preferredVessels,instrumentPackets,disruptorInstrumentPackets, nutatorInstrumentPackets,
	...
	resolvedRoboticOptionsWithoutMultichannelMixing,resolvedMultichannelMix,resolvedMultichannelMixName,resolvedDeviceChannel,
		...
		transformHeatShockTimes, transformPreHeatCoolingTimes, transformPostHeatCoolingTimes, transformIncompatibleInstrumentErrors,
	...
	(* Bundle our instrument packets into a ball together. *)
	instrumentPackets=Flatten[
		{
			vortexInstrumentPackets,
			shakeInstrumentPackets,
			bottleRollerInstrumentPackets,
	...
	(* Bundle our rolling instrument packets together. *)
	rollerInstrumentPackets=Flatten[{bottleRollerInstrumentPackets,rollInstrumentPackets}];
	...
	cacheBall=FlattenCachePackets[{fullCacheBall,samplePackets,sampleContainerPackets,sampleContainerModelPackets,vortexInstrumentPackets,shakeInstrumentPackets,bottleRollerInstrumentPackets,rollInstrumentPackets,stirInstrumentPackets,sonicationInstrumentPackets,heatBlockInstrumentPackets,preferredVesselPackets,homogenizerInstrumentPackets,disruptorInstrumentPackets,nutatorInstrumentPackets,stirInstrumentImpellerPackets,environmentalChamberInstrumentPackets,thermocyclerInstrumentPackets,suppliedStirBarPackets}];
	...
	{roundedMixOptions,precisionTests}=If[gatherTests,
		RoundOptionPrecision[
			...
			AvoidZero->{False,False,False,True,True,False,True,False,False,False,False,False},
			Output->{Result,Tests}
		],
		{
			RoundOptionPrecision[
				...
				If[MatchQ[instrument,Except[Null|Automatic]] && MatchQ[{rate, temperature},Except[{Automatic|Null, Automatic|Null|AmbientTemperatureP}]],
					...
					instrumentPacket=If[MatchQ[instrument,ObjectP[Object[Instrument]]],
						fetchPacketFromCache[
							Lookup[fetchPacketFromCache[instrument,instrumentPackets],Model],
							instrumentPackets
						],
						fetchPacketFromCache[instrument,instrumentPackets]
					];
	...
	volumeOptionMismatches=MapThread[
		Function[{volume,sample},
			Module[{samplePacket,sampleVolume},
				(* Is Instrument supplied? *)
				If[MatchQ[volume,Except[Null|Automatic]],
					(* Volume is given. Is it more than the volume of our sample? *)
					(* Get the packet of our sample. *)
					samplePacket=fetchPacketFromCache[sample,samplePackets];

					(* Get the volume of our sample. *)
					sampleVolume=Lookup[samplePacket,Volume,0Liter]/.{$Failed|Null->0Liter};
	...
	maxTemperatureMismatches=MapThread[
		Function[{temperature,thawTemperature,sample},
			Module[{samplePacket,sampleContainerObject,sampleContainerPacket,sampleContainerModelObject,sampleContainerModelPacket,maxTemperature},
				(* Get the packet of our sample. *)
				samplePacket=fetchPacketFromCache[sample,samplePackets];
		...
		Function[{sample,mixType,aliquot},
			Module[{sampleContainerObject},
				...
				If[MatchQ[sampleContainerObject, ObjectP[Object[Container,Vessel,VolumetricFlask]]]&&MatchQ[aliquot, False],
					If[MatchQ[mixType, Except[Automatic|Null|Invert|Swirl|Shake]],
						{mixType,sample},
						Nothing
					],
					Nothing
	...
	relativeHumidities, lightExposures, lightExposureIntensities, totalLightExposures, transforms, transformHeatShockTemperatures,
		transformHeatShockTimes, transformPreHeatCoolingTimes, transformPostHeatCoolingTimes,
			...
			Module[
				{
					...
					temperature,incubate,annealingTime,potentialAliquotContainers,
					samplePacket,modelPacket,samplesContainer,containerPacket,otherSamplesInContainer,allOtherMixTypes,
				...
				samplePacket=fetchPacketFromCache[mySample,samplePackets];
				...
				thawInstrument=If[MatchQ[thaw,True],
					...
					If[KeyExistsQ[myMapThreadOptions,ThawInstrument]&&!MatchQ[Lookup[myMapThreadOptions,ThawInstrument],Automatic],
						...
						thawInstrumentModel=If[MatchQ[givenThawInstrument,ObjectP[Object[Instrument]]],
							Lookup[fetchPacketFromCache[givenThawInstrument,cacheBall],Model]/.{link_Link:>Download[link, Object]},
							Lookup[fetchPacketFromCache[givenThawInstrument,cacheBall],Object]
						];
						...
						Module[{containerPackets,filteredContainerPackets},
							...
							If[(!MatchQ[Lookup[samplePacket,Volume,0 Liter],VolumeP] && MatchQ[Lookup[containerPacket,MaxVolume,0 Liter],GreaterP[4 Liter]]) || Lookup[samplePacket,Volume,0 Liter]>4Liter*1.04,
								...
								If[MatchQ[Lookup[containerPacket,OpenContainer,True],True]||MatchQ[Lookup[containerPacket,MaxVolume,0 Liter],GreaterP[4 Liter]],
									...
									containerPackets=fetchPacketFromCache[#,preferredVesselPackets]&/@suitableContainers;
								...
								Module[
									...
									myTipType=If[MatchQ[Lookup[myMapThreadOptions, TipType],Automatic],
										Which[
											...
											(If[MatchQ[Lookup[
												fetchPacketFromCache[
													Lookup[fetchPacketFromCache[myTips, suppliedTipObjectPackets], Model], allTipModelPackets], #],
												True],
											...
											] & /@ {WideBore, Aspirator, Filtered, GelLoading}) /. {Filtered->Barrier, {} -> {Normal}}),
											...
											First@(
											(If[MatchQ[
												Lookup[fetchPacketFromCache[myTips, allTipModelPackets], #],
												True],
												(*If the looked up value is True, append the TipTypeP to the resolved myTipType*)
												#,
												Nothing
									...
									myTipMaterial=If[MatchQ[Lookup[myMapThreadOptions, TipMaterial],Automatic],
										Which[
											...
											Lookup[
												fetchPacketFromCache[
													Lookup[fetchPacketFromCache[myTips, suppliedTipObjectPackets], Model],
													allTipModelPackets
												],
												Material
											],
											MatchQ[myTips, ObjectP[Model[Item, Tips]]],
											Lookup[fetchPacketFromCache[myTips, allTipModelPackets], Material],
											True,
											Null
										],
								...
								True,
									Module[{tipsModel, potentialPipettes},
										...
										tipsModel=If[MatchQ[tips, ObjectP[Object[Item, Tips]]],
											Lookup[fetchPacketFromCache[tips, suppliedTipObjectPackets], Model],
											tips
										];
						...
						Module[{instrumentModel,vortexPacket,compatibleFootprintQ,aliquotContainers,potentialInstruments,potentialAliquotInstruments,preResolvedRate,preResolvedTemperature},
							...
							If[MatchQ[Lookup[myMapThreadOptions,Instrument],ObjectP[{Object[Instrument,Vortex],Model[Instrument,Vortex]}]],
								...
								instrumentModel=If[MatchQ[instrument,ObjectP[Model[Instrument,Vortex]]],
									...
									Download[Lookup[fetchPacketFromCache[instrument,vortexInstrumentPackets],Model,Null],Object]
								];
								...
								rate=If[MatchQ[Lookup[myMapThreadOptions,MixRate],Automatic],
									If[
										...
										vortexPacket=fetchPacketFromCache[instrumentModel,vortexInstrumentPackets];
								...
								compatibleFootprintQ=MemberQ[potentialInstruments,Lookup[fetchPacketFromCache[instrumentModel,vortexInstrumentPackets],Object,Null]];
								...
								instrument=If[Length[potentialInstruments]>0,
									...
									rate=If[MatchQ[preResolvedRate,Automatic],
										(* Resolve to the average RPM of the set instrument. *)
										vortexPacket=fetchPacketFromCache[First[potentialInstruments],vortexInstrumentPackets];
						...
						Module[{instrumentModel,rollPacket,compatibleFootprintQ,aliquotContainers,potentialInstruments,potentialAliquotInstruments,resolvedTemperature,resolvedAnnealingTime,preResolvedRate},
							...
							If[MatchQ[Lookup[myMapThreadOptions,Instrument],ObjectP[{Object[Instrument,BottleRoller],Model[Instrument,BottleRoller],Object[Instrument,Roller],Model[Instrument,Roller]}]],
								...
								instrumentModel=If[MatchQ[instrument,ObjectP[{Model[Instrument,BottleRoller],Model[Instrument,Roller]}]],
									...
									instrument,
									(* Go from the object to the model. *)
									Download[Lookup[fetchPacketFromCache[instrument,rollerInstrumentPackets],Model,Null],Object]
								];
								...
								rate=If[MatchQ[Lookup[myMapThreadOptions,MixRate],Automatic],
									If[
										...
										Lookup[samplePacket,ThawMixRate],
										(* Resolve to the average RPM of the set instrument. *)
										rollPacket=fetchPacketFromCache[instrumentModel,rollerInstrumentPackets];
								...
								(* Is the user-specified instrument part of this list? *)
								compatibleFootprintQ=MemberQ[potentialInstruments,Lookup[fetchPacketFromCache[instrumentModel,rollerInstrumentPackets],Object,Null]];
								...
								instrument=If[Length[potentialInstruments]>0,
									...
									rate=If[MatchQ[preResolvedRate,Automatic],
										(* Resolve to the average RPM of the set instrument. *)
										rollPacket=fetchPacketFromCache[First[potentialInstruments],rollerInstrumentPackets];
							...
							If[MatchQ[resolvedPreparation,Manual],
								...
								If[MatchQ[Lookup[myMapThreadOptions,Instrument],ObjectP[{Object[Instrument,Shaker],Model[Instrument,Shaker]}]],
									...
									instrumentModel=If[MatchQ[instrument,ObjectP[Model[Instrument,Shaker]]],
										...
										instrument,
										(* Go from the object to the model. *)
										Download[Lookup[fetchPacketFromCache[instrument,shakeInstrumentPackets],Model,Null],Object]
									];
									...
									rate=If[MatchQ[Lookup[myMapThreadOptions,MixRate],Automatic],
										If[
											...
											Lookup[samplePacket,ThawMixRate],
											(* Resolve to the average RPM of the set instrument. *)
											shakePacket=fetchPacketFromCache[instrumentModel,shakeInstrumentPackets];
									...
									(* Is the user-specified instrument part of this list? *)
									compatibleFootprintQ=MemberQ[potentialInstruments,Lookup[fetchPacketFromCache[instrumentModel,shakeInstrumentPackets],Object,Null]];
									...
									instrument=If[Length[potentialInstruments]>0,
										...
										rate=If[MatchQ[preResolvedRate,Automatic],
											If[!MatchQ[Lookup[myMapThreadOptions,MixRateProfile], _List],
												(* Resolve to the average RPM of the set instrument. *)
												shakePacket=fetchPacketFromCache[First[potentialInstruments],shakeInstrumentPackets];
					...
					Stir,
						Module[{instrumentModel,stirPacket,compatibleFootprintQ,aliquotContainers,potentialInstruments,potentialAliquotInstruments,resolvedTemperature,resolvedAnnealingTime,preResolvedRate, exampleAliquotContainer, allPotentialImpelllers, containerSafeMixRate},
							...
							If[MatchQ[Lookup[myMapThreadOptions,Instrument],ObjectP[{Object[Instrument,OverheadStirrer],Model[Instrument,OverheadStirrer]}]],
								...
								(* Is the user-specified instrument part of this list? *)
								compatibleFootprintQ=MemberQ[potentialInstruments,Lookup[fetchPacketFromCache[instrumentModel,stirInstrumentPackets],Object,Null]];
						...
						Module[{instrumentModel,stirPacket,compatibleFootprintQ,aliquotContainers,potentialInstruments,potentialAliquotInstruments,resolvedTemperature,resolvedAnnealingTime},
								...
								If[MatchQ[Lookup[myMapThreadOptions,Instrument],ObjectP[{Object[Instrument,Homogenizer],Model[Instrument,Homogenizer]}]],
									...
									instrumentModel=If[MatchQ[instrument,ObjectP[Model[Instrument,Homogenizer]]],
										...
										instrument,
										(* Go from the object to the model. *)
										Download[Lookup[fetchPacketFromCache[instrument,homogenizerInstrumentPackets],Model,Null],Object]
									];
									...
									(* Is the user-specified instrument part of this list? *)
									compatibleFootprintQ=MemberQ[potentialInstruments,Lookup[fetchPacketFromCache[instrumentModel,homogenizerInstrumentPackets],Object,Null]];
						...
						Module[{instrumentModel,sonicatePacket,compatibleFootprintQ,aliquotContainers,potentialInstruments,potentialAliquotInstruments},
							...
							If[MatchQ[Lookup[myMapThreadOptions,Instrument],ObjectP[{Object[Instrument,Sonicator],Model[Instrument,Sonicator]}]],
								...
								instrumentModel=If[MatchQ[instrument,ObjectP[Model[Instrument,Sonicator]]],
									...
									instrument,
									(* Go from the object to the model. *)
									Download[Lookup[fetchPacketFromCache[instrument,sonicationInstrumentPackets],Model,Null],Object]
								];
								...
								(* Is the user-specified instrument part of this list? *)
								compatibleFootprintQ=MemberQ[potentialInstruments,Lookup[fetchPacketFromCache[instrumentModel,sonicationInstrumentPackets],Object,Null]];
					...
					Disrupt,
						...
						If[MatchQ[Lookup[myMapThreadOptions,Instrument],ObjectP[{Object[Instrument,Disruptor],Model[Instrument,Disruptor]}]],
							...
							instrumentModel=If[MatchQ[instrument,ObjectP[Model[Instrument,Disruptor]]],
								...
								instrument,
								(* Go from the object to the model. *)
								Download[Lookup[fetchPacketFromCache[instrument,disruptorInstrumentPackets],Model,Null],Object]
							];
							...
							rate=If[MatchQ[Lookup[myMapThreadOptions,MixRate],Automatic],
								If[
									...
									Lookup[samplePacket,ThawMixRate],
									(* Resolve to the average RPM of the set instrument. *)
									disruptorPacket=fetchPacketFromCache[instrumentModel,disruptorInstrumentPackets];
							...
							(* Is the user-specified instrument part of this list? *)
							compatibleFootprintQ=MemberQ[potentialInstruments,Lookup[fetchPacketFromCache[instrumentModel,disruptorInstrumentPackets],Object,Null]];
							...
							instrument=If[Length[potentialInstruments]>0,
								...
								rate=If[MatchQ[preResolvedRate,Automatic],
									(* Resolve to the average RPM of the set instrument. *)
									disruptorPacket=fetchPacketFromCache[First[potentialInstruments],disruptorInstrumentPackets];
					...
					Nutate,
						...
						If[MatchQ[Lookup[myMapThreadOptions,Instrument],ObjectP[{Object[Instrument,Nutator],Model[Instrument,Nutator]}]],
							...
							instrumentModel=If[MatchQ[instrument,ObjectP[Model[Instrument,Nutator]]],
								...
								instrument,
								(* Go from the object to the model. *)
								Download[Lookup[fetchPacketFromCache[instrument,nutatorInstrumentPackets],Model,Null],Object]
							];
							...
							rate=If[MatchQ[Lookup[myMapThreadOptions,MixRate],Automatic],
								If[
									...
									Lookup[samplePacket,ThawMixRate],
									(* Resolve to the average RPM of the set instrument. *)
									nutatorPacket=fetchPacketFromCache[instrumentModel,nutatorInstrumentPackets];
							...
							(* Is the user-specified instrument part of this list? *)
							compatibleFootprintQ=MemberQ[potentialInstruments,Lookup[fetchPacketFromCache[instrumentModel,nutatorInstrumentPackets],Object,Null]];
							...
							instrument=If[Length[potentialInstruments]>0,
								...
								rate=If[MatchQ[preResolvedRate,Automatic],
									(* Resolve to the average RPM of the set instrument. *)
									nutatorPacket=fetchPacketFromCache[First[potentialInstruments],nutatorInstrumentPackets];
				...
				mixPosition,mixPositionOffset,mixFlowRate,correctionCurve,tips,tipType,tipMaterial,sampleLabel,sampleContainerLabel,
				relativeHumidity, lightExposure, lightExposureIntensity, totalLightExposure, transform, transformHeatShockTemperature,
				transformHeatShockTime, transformPreHeatCoolingTime, transformPostHeatCoolingTime,
	...
	{resolvedMultichannelMix,resolvedMultichannelMixName,resolvedDeviceChannel}=If[MatchQ[resolvedPreparation,Robotic]&&!MatchQ[mixTypes,{Null...}],
		Module[{pipetteIndices,pipetteSamples,pipetteOptions,pipetteSampleContainers,pipetteMultichannelMixes, pipetteMultichannelMixNames, pipetteDeviceChannels,multichannelMixes,multichannelMixNames,deviceChannels},
			...
			pipetteSampleContainers=Download[
				(If[MatchQ[#, ObjectP[Object[Sample]]],
					Lookup[fetchPacketFromCache[#, samplePackets], Container],
					#
				]&)/@mySamples,
				Object
			];
	...
	invertVolumeInvalidOptions=If[Or@@invertSampleVolumeErrors&&!gatherTests,
		Module[{invertVolumeInvalidSamples,sampleVolumes},
			...
			sampleVolumes=(Lookup[fetchPacketFromCache[#,samplePackets],Volume,0 Liter]&)/@invertVolumeInvalidSamples;
	...
	invertVolumeTest=If[gatherTests,
		...
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Invert]&),{invertSampleVolumeErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Invert]&),{invertSuitableContainerErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Vortex]&),{vortexIncompatibleInstrumentErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Vortex]&),{vortexNoInstrumentForRateErrors,mixTypes}]];
	...
	vortexNoInstrumentTest=If[gatherTests,
		...
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Vortex]&),{vortexNoInstrumentErrors,mixTypes}]];
	...
	disruptIncompatibleInstrumentTest=If[gatherTests,
		...
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Disrupt]&),{disruptIncompatibleInstrumentErrors,mixTypes}]];
	...
	disruptNoInstrumentForRateTest=If[gatherTests,
		...
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Disrupt]&),{disruptNoInstrumentForRateErrors,mixTypes}]];
	...
	disruptNoInstrumentTest=If[gatherTests,
		...
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Disrupt]&),{disruptNoInstrumentErrors,mixTypes}]];
	...
	nutateIncompatibleInstrumentTest=If[gatherTests,
		...
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Nutate]&),{nutateIncompatibleInstrumentErrors,mixTypes}]];
	...
	nutateNoInstrumentTest=If[gatherTests,
		...
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Roll]&),{nutateNoInstrumentErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Roll]&),{rollIncompatibleInstrumentErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Roll]&),{rollNoInstrumentErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Shake]&),{shakeIncompatibleInstrumentErrors,mixTypes}]];
	...
	shakeNoInstrumentTest=If[gatherTests,
		...
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Shake]&),{shakeNoInstrumentErrors,mixTypes}]];
	...
	instrumentModels=Map[
		Which[
			...
			(* Go from the object to the model. *)
			True,
			Download[Lookup[fetchPacketFromCache[#,cacheBall],Model,Null],Object]
		]&,
		instruments
	];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Stir]&),{stirIncompatibleInstrumentErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Stir]&),{stirNoInstrumentErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Sonicate]&),{sonicateIncompatibleInstrumentErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Sonicate]&),{sonicateNoInstrumentErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Sonicate]&),{homogenizeIncompatibleInstrumentErrors,mixTypes}]];
	...
	(* We're gathering tests. Create the appropriate tests. *)
		Module[{failingInputs,passingInputs,passingInputsTest,failingInputTest},
			...
			(* Get the inputs that pass this test. *)
			passingInputs=PickList[mySimulatedSamples,MapThread[(#1\[Xor]MatchQ[#2,Homogenize]&),{homogenizeNoInstrumentErrors,mixTypes}]];
							...
							MapThread[Function[{instrumentInformation,index},
								...
								resolvedRate=If[MatchQ[rates[[index]],Automatic],
									...
									instrumentPacket=If[MatchQ[chosenInstrument,ObjectP[Object[Instrument]]],
										fetchPacketFromCache[
											Lookup[fetchPacketFromCache[chosenInstrument,instrumentPackets],Model],
											instrumentPackets
										],
										fetchPacketFromCache[chosenInstrument,instrumentPackets]
									];
			...
			If[MatchQ[stirBar, ObjectP[]],
				Module[{compatibleStirBarModels, givenStirBarModel},
					...
					givenStirBarModel=If[MatchQ[stirBar, ObjectP[Object[Part]]],
						Download[Lookup[fetchPacketFromCache[stirBar, cacheBall], Model], Object],
						stirBar
					];
	...
	typeOptionMismatches=MapThread[
		Function[{type,mixUntilDissolved,instrument,time,maxTime,rate,numberOfMixes,maxNumberOfMixes,volume,temperature,annealingTime,residualIncubation,residualTemperature,residualMix,residualMixRate,amplitude,maxTemperature,dutyCycle,relativeHumidity,transform,sample},
			Module[{mustBeSpecifiedOptions,cannotBeSpecifiedOptions,specifiedOptionsBooleans,specifiedOptions,notSpecifiedOptions,incorrectlySpecifiedOptions},
				...
				{mustBeSpecifiedOptions,cannotBeSpecifiedOptions}=Switch[type,
					...
					Swirl,
						{{NumberOfMixes},{RelativeHumidity,LightExposure,LightExposureIntensity,TotalLightExposure,LightExposureStandard,StirBar,TemperatureProfile,MixRateProfile,Instrument,Time,MaxTime,MixRate,MixVolume,Temperature,AnnealingTime,ResidualIncubation,ResidualTemperature,ResidualMix,ResidualMixRate,Amplitude,MaxTemperature,DutyCycle}},
					Vortex,
						{{MixRate,Time},{RelativeHumidity,LightExposure,LightExposureIntensity,TotalLightExposure,LightExposureStandard,StirBar,TemperatureProfile,MixRateProfile,MixVolume,NumberOfMixes,MaxNumberOfMixes,Amplitude,MaxTemperature,DutyCycle}},
					Roll,
						{{MixRate,Time},{RelativeHumidity,LightExposure,LightExposureIntensity,TotalLightExposure,LightExposureStandard,StirBar,TemperatureProfile,MixRateProfile,MixVolume,NumberOfMixes,MaxNumberOfMixes,Amplitude,MaxTemperature,DutyCycle}},
					Shake,
						{{Time},{RelativeHumidity,LightExposure,LightExposureIntensity,TotalLightExposure,LightExposureStandard,StirBar,MixVolume,NumberOfMixes,MaxNumberOfMixes,Amplitude,MaxTemperature,DutyCycle}},
					Disrupt,
						{{MixRate,Time},{RelativeHumidity,LightExposure,LightExposureIntensity,TotalLightExposure,LightExposureStandard,StirBar,TemperatureProfile,MixRateProfile,MixVolume,NumberOfMixes,MaxNumberOfMixes,Amplitude,MaxTemperature,DutyCycle}},
					Nutate,
						{{MixRate,Time},{RelativeHumidity,LightExposure,LightExposureIntensity,TotalLightExposure,LightExposureStandard,StirBar,TemperatureProfile,MixRateProfile,MixVolume,NumberOfMixes,MaxNumberOfMixes,Amplitude,MaxTemperature,DutyCycle}},
					Stir,
						{{MixRate,Time},{RelativeHumidity,LightExposure,LightExposureIntensity,TotalLightExposure,LightExposureStandard,TemperatureProfile,MixRateProfile,MixVolume,NumberOfMixes,MaxNumberOfMixes,Amplitude,MaxTemperature,DutyCycle}},
					Sonicate,
						{{Time},{RelativeHumidity,LightExposure,LightExposureIntensity,TotalLightExposure,LightExposureStandard,StirBar,TemperatureProfile,MixRateProfile,MixRate,MixVolume,NumberOfMixes,MaxNumberOfMixes}},
					Homogenize,
						{{Time,Amplitude},{RelativeHumidity,LightExposure,LightExposureIntensity,TotalLightExposure,LightExposureStandard,StirBar,TemperatureProfile,MixRateProfile,MixRate,MixVolume,NumberOfMixes,MaxNumberOfMixes}},
					Null,
						Which[
							TrueQ[transform],
								{{},{StirBar,MixRateProfile,MixVolume,NumberOfMixes,MaxNumberOfMixes,MaxTime,MixUntilDissolved}},
							MatchQ[instrument, ObjectP[{Model[Instrument, EnvironmentalChamber], Object[Instrument, EnvironmentalChamber]}]],
		...
		Map[
			Function[{group},
				Module[{samples, packets, deviceChannels, multichannelMixNames},
					...
					packets = (fetchPacketFromCache[#, cacheBall]&)/@samples;
...
incubateNewResourcePackets[mySamples:{ObjectP[Object[Sample]]..},myUnresolvedOptions:{___Rule},myResolvedOptions:{___Rule},myCollapsedResolvedOptions:{___Rule},myOptions:OptionsPattern[]]:=Module[
	...
	cache=FlattenCachePackets[{cache, samplePackets, containerPackets, containerModelPackets}];
		...
		protocolFields=Flatten[Function[{groupedMixSamples},
			...
			Switch[mixType,
				...
				Sonicate,
					...
					Module[{incubateInformation, incubateSamples,instrumentSettings,instrumentObjects,sampleGrouping,instruments,groupedSettings,additionalTime,sonicateParameters,sonicationBatchLengths, groupSonicationSamplesOutput},
						...
						If[Length[incubateSamples]==0,
							Nothing,
							(* Compute our sample grouping. *)
							groupSonicationSamplesOutput = groupSonicationSamples[
								incubateSamples,
								Download[Lookup[options,Instrument], Object],
								instrumentSettings,
								Cache -> cache,
								Simulation -> updatedSimulation
							];

							(* Deconvolute the output of groupSonicationSamples. *)
							sampleGrouping = Flatten /@ Cases[groupSonicationSamplesOutput, ListableP[{ObjectP[{Object[Sample], Model[Sample]}]..}]];
							instruments = First /@ Cases[groupSonicationSamplesOutput, {ObjectP[{Object[Instrument], Model[Instrument]}] ..}];
							groupedSettings = Flatten /@ Cases[groupSonicationSamplesOutput, {{_Rule..}..}];
...
groupSonicationSamples[mySamples_List,myInstrument:ObjectP[{Model[Instrument,Sonicator],Object[Instrument,Sonicator]}],myInstrumentSettings_List,myOptions:OptionsPattern[]]:=Module[
	...
	sampleContainerModelFootprint, containerToFloatMapRaw, containerToFloatMapFootprint, samplesNotInFloats,
		samplesForOutput, instrumentsForOutput, instrumentSettingsForOutput},
	...
	(* Join the items we want to output. *)
	samplesForOutput = Join[finalBatches, samplesNotInFloats];
	instrumentsForOutput = Join[
		ConstantArray[myInstrument,Length[finalBatches]],
		ConstantArray[myInstrument,Length[samplesNotInFloats]]
	];
	instrumentSettingsForOutput = Join[
...
compatibleImpeller[mySample:ObjectP[Object[Sample]],myStirInstrument:ObjectP[{Object[Instrument],Model[Instrument]}],ops:OptionsPattern[]]:=Module[
	...
	filteredImpellers=Cases[impellerInformation,{_,RangeP[containerInternalDepth, containerInternalDepth * 2],LessP[containerAperture],LessEqualP[containerInternalDiameter-clearence]}];
		...
		Module[{oscillationAnglePattern,filterPattern,instrumentPackets,filteredInstrumentPackets,compatibleInstruments,filteredCompatibleInstruments,filteredContainers,compatibleFootprintsQ,containers,containerResult,containerRules},
			...
			instrumentPackets=Switch[type,
				...
				Shake,
					shakeInstrumentPackets,
				Stir,
					If[MatchQ[Lookup[First@samplePackets, Fuming], True],
			...
			If[Length[filteredInstrumentPackets]==0,
						...
						(* If our vessel is self standing and can fit a self-standing weight, we don't need to put it in a float. *)
							Which[
								...
								And[
									(* Self-standing check *)
									MatchQ[
										Lookup[fetchPacketFromCache[sampleContainerModel,sampleContainerPackets],SelfStanding,False],
										True
									],
									(* About as large as a 250mL vessel in order to fit a sonciation weight. *)
									MatchQ[
										Lookup[fetchPacketFromCache[sampleContainerModel,sampleContainerPackets],Dimensions,{0Meter,0Meter,0Meter}],
										...
										Module[{packet},
											...
											If[And[
													If[MatchQ[#, ObjectP[Model[Instrument, Shaker, "id:bq9LA0JYrN66"]]],
														MatchQ[Lookup[fetchPacketFromCache[sampleContainerModel, cacheBall], SelfStanding], True],
														True
													],
													MatchQ[Lookup[packet, GripperDiameter], GreaterP[0 Millimeter]],
													!MatchQ[
														Lookup[packet, GripperDiameter],
														GreaterEqualP[Lookup[fetchPacketFromCache[sampleContainerModel, cacheBall], Aperture]]
													]
												],
												Nothing,
												#
									...
									Function[{instrument},
										...
										If[MatchQ[sampleVolume,Null]||MatchQ[sampleVolume,GreaterEqualP[.95*containerModelMaxVolume]],
											...
											If[MatchQ[bestSonicationHorn,Null],
												...
												If[TrueQ[Lookup[fetchPacketFromCache[sampleContainerModel,sampleContainerPackets],SelfStanding,False]]||MatchQ[sampleContainerModel,ObjectP[{Model[Container,Vessel,"id:3em6Zv9NjjN8"],Model[Container,Vessel,"id:xRO9n3vk11pw"],Model[Container,Vessel,"id:bq9LA0dBGGR6"]}]],
													...
													If[MatchQ[amplitude,GreaterP[70 Percent]]&&!MatchQ[bestSonicationHorn,ObjectP[Model[Part, SonicationHorn, "id:WNa4ZjKzw8p4"]]],
														...
														If[MatchQ[maxTemperature,TemperatureP]&&MatchQ[Lookup[fetchPacketFromCache[sampleContainerModel,sampleContainerPackets],Aperture]-Lookup[fetchPacketFromCache[bestSonicationHorn,cacheBall],HornDiameter],LessEqualP[1 Centimeter]],
															Nothing,
															instrument
														]
													],
				...
				containers=If[MemberQ[output,Containers],
							...
							Map[
								(If[!MatchQ[#,ObjectP[Model[Container,Vessel]]],
									...
									If[And[
											(* Self-standing check *)
											MatchQ[
												Lookup[fetchPacketFromCache[#,preferredVesselPackets],SelfStanding,False],
												True
											],
											(* About as large as a 250mL vessel in order to fit a sonciation weight. *)
											MatchQ[
												Lookup[fetchPacketFromCache[#,preferredVesselPackets],Dimensions,{0Meter,0Meter,0Meter}],
								...
								Function[{container},
									...
									If[!(MatchQ[sampleVolume,Null]||MatchQ[sampleVolume,GreaterEqualP[.95*containerModelMaxVolume]]),
										...
										If[MatchQ[bestSonicationHorn,Null],
											Nothing,
											(* If we have a compatible sonication horn, make sure the container of the sample is also self-standing, or has an approved holder (2mL, 15mL, or 50mL tube). *)
											If[TrueQ[Lookup[fetchPacketFromCache[container,preferredVesselPackets],SelfStanding,False]]||MatchQ[container,ObjectP[{Model[Container,Vessel,"id:3em6Zv9NjjN8"],Model[Container,Vessel,"id:xRO9n3vk11pw"],Model[Container,Vessel,"id:bq9LA0dBGGR6"]}]],
												(* If we were given an Amplitude over 70 Percent, the sonication horn used can't be one of the small ones. *)
												If[MatchQ[amplitude,GreaterP[70 Percent]]&&!MatchQ[bestSonicationHorn,ObjectP[Model[Part, SonicationHorn, "id:WNa4ZjKzw8p4"]]],
													Nothing,
													(* If we were given a MaxTemperature, there has to be at least 1 CM of clearance between the container aperture and the sonication horn's diameter *)
													If[MatchQ[maxTemperature,TemperatureP]&&MatchQ[Lookup[fetchPacketFromCache[container,preferredVesselPackets],Aperture]-Lookup[fetchPacketFromCache[bestSonicationHorn,cacheBall],HornDiameter],LessEqualP[1 Centimeter]],
														Nothing,
														container
													]
												],
					...
					Cases[containerRules,x:(_->Except[{}])->x,{1}],
					{}
				];
