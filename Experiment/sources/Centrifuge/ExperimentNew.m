...
Warning::ContainerCentrifugeIncompatible="The samples `1` are in containers that cannot fit on the centrifuges `2`. These samples will be transferred to `3`. Check that the relevant centrifuge fields {Counterweights, MaxCentrifugationForce, Dimensions} have been populated for the samples container models if you still wish to run them in their original containers.";
	...
	centrifugesAndContainersByOptionSet = If[MatchQ[resolvedPreparation, Robotic],
		...
		Module[
			...
			{expandedTimeByContainer, expandedTemperatureByContainer, expandedIntensityByContainer, expandedCollectionContainerByContainer, expandedBiohazardSampleQs} = Map[
				...
				(* expand as nested lists so that each input container generates its own option set list *)
				Thread[ConstantArray[#,Length[allCentrifugableContainers]+1]]&,
				{specifiedTimes, resolvedTemperature, specifiedIntensities,resolvedCollectionContainers, biohazardSampleQs}
			];

			(* remove duplicates by option set*)
			containerOptionSets = MapThread[
				Transpose[
					(* flatten the sublists for each input container option set, not everything combined *)
					Flatten /@ {#1, #2, #3, #4, #5, #6}
				]&,
				{
					expandedContainers,
					expandedTimeByContainer,
					expandedTemperatureByContainer,
					expandedIntensityByContainer,
					expandedCollectionContainerByContainer,
					expandedBiohazardSampleQs
				}
			];
			uniqueOptionSets = DeleteDuplicates/@containerOptionSets;
			{uniqueContainers, uniqueTimes, uniqueTemperatures, uniqueIntensity, uniqueCollectionContainer, uniqueBiohazardSampleQs} = Thread[Transpose/@uniqueOptionSets];

			(* get the compatible centrifuges for each input container*)
			compatibleCentrifuges = MapThread[
				CentrifugeDevices[
					#1,
					Time -> #2,
					Temperature -> #3 /. {Ambient -> $AmbientTemperature},
					Intensity -> #4,
					CollectionContainer -> #5,
					Biohazard -> #6,
					Preparation -> resolvedPreparation,
					WorkCell -> Lookup[myOptions, WorkCell, Automatic],
					Cache -> cacheBall,
				...
				{uniqueContainers, uniqueTimes, uniqueTemperatures, uniqueIntensity, uniqueCollectionContainer, uniqueBiohazardSampleQs}
			];
			...
			optionSetToCompatibleCentrifuges = MapThread[
				AssociationThread[#1, #2]&,
				{uniqueOptionSets, compatibleCentrifuges}
			];

			(* Partition the result to reflect the centrifuges for each option set for each possible container *)
			centrifugesByOptionsByContainer = MapThread[
				ReplaceAll[#1, #2]&,
				{containerOptionSets, optionSetToCompatibleCentrifuges}
			];
