...
DefineOptionSet[DilutionSharedOptions :>
	{
		IndexMatching[
			...
			},
			{
				...
				Default -> Automatic,
				Description -> "Indicates if only the final sample (Endpoint) or all diluted samples (Series) produced by serial dilution are used for downstream operations. If set to Series, all diluted samples are used for downstream operations. If set to Endpoint, only the final sample is used for downstream operations. DilutionStrategy is only applicable when DilutionType is Serial.",
				ResolutionDescription -> "Automatically set to Endpoint if DilutionType is Serial.",
				AllowNull -> True,
				Widget -> Widget[
					Type -> Enumeration,
			...
			},
			{
				...
				Description -> "For each sample, the total fold dilution relative to the original sample by which the concentration of the TargetAnalyte is reduced, calculated as the product of all individual dilution factors applied across each step for Serial DilutionType or across all dilutions for Linear DilutionType, where the length of this list must match the corresponding value in NumberOfDilutions. For example, if CumulativeDilutionFactor is {10,100,100} and the initial concentration of TargetAnalyte is 10 M, the concentration of TargetAnalyte of the diluted samples will be {1 M, 0.1 M, 0.01 M}.",
			...
			},
			{
				...
				Description -> "For each sample, the fold dilution applied at each step in a serial dilution series, representing the factor by which the concentration of the TargetAnalyte is reduced per step, where the length of this list must match the corresponding value in NumberOfDilutions. For example, if SerialDilutionFactor is set to {10,10,10} and the initial concentration of TargetAnalyte is 10 M, the concentration of TargetAnalyte of the diluted samples will be {1 M, 0.1 M, 0.01 M}. SerialDilutionFactor is only applicable when DilutionType is Serial.",
				ResolutionDescription -> "Automatically set based on the systems of equations specified under the DilutionType option.",
				NestedIndexMatching -> True,
				Category -> "Sample Dilution"
			},
			...
			},
			{
				...
				Description -> "For each sample, if DilutionType is set to Linear, the amount of sample that is diluted with Buffer. If DilutionType is set to Serial, the amount of sample transferred from the resulting sample of one round of the dilution series to the next sample in the series. The first transfer source is the original sample provided. The length of this list must match the corresponding value in NumberOfDilutions.",
				ResolutionDescription -> "Automatically set based on the systems of equations specified under the DilutionType option.",
				NestedIndexMatching -> True,
				Category -> "Sample Dilution"
			},
        ...
        ConcentratedBuffer/ConcentratedBufferDiluent it is possible there is no corresponding Diluent sample *)
			...
			},
			{
				...
				ResolutionDescription -> "If there is no Product or Inventory for the Diluent or Diluent does not match the Media/Solvent of the input sample, automatically set to a concentrated version of Diluent with standing Product or Inventory.",
				Category -> "Diluent Preparation"
			},
			{
...
Error::InvalidCumulativeDilutionFactors = "`1`. The sample(s), `2`, at indices, `4`, have invalid CumulativeDilutionFactor set to `3`. Please fix the values for option CumulativeDilutionFactor or let it set automatically.";
...
ResolveDilutionSharedOptions[mySamples : ListableP[ObjectP[Object[Sample]]], myOptions : OptionsPattern[]] := Module[
	{
		outputSpecification, output, gatherTests, messages, warnings, cache, simulation, validLengths, validLengthTests,
		expandedSafeOps, validExpandedLengths, validExpandedLengthErrors, validExpandedLengthTests, listedSamplesNamed,
		listedOptionsNamed, safeOpsNamed, safeOpsTests, listedSamples, safeOps, listedOptions, currentSimulation,
		...
		(* Downloaded Packets *)
		samplePackets, diluentObjectPackets, diluentModelPackets, concentratedBufferObjectPackets, concentratedBufferModelPackets,
		concentratedBufferDiluentObjectPackets, concentratedBufferDiluentModelPackets, sampleSolventPackets, sampleMediaPackets,
		(* Caching *)
		...
		(* Errors and warnings *)
		noDiluentSpecifiedWarnings, missingTargetAnalyteInitialConcentrationWarnings, conflictingFactorsAndVolumesErrors,
		noSampleVolumeWarnings, multipleTargetAnalyteConcentrationUnitsErrors, conflictingTargetAnalyteConcentrationUnitsErrors,
		invalidCumulativeDilutionErrors, targetAnalyteTests,
		(* Resolved mapthread options *)
		resolvedDilutionTypes, resolvedDilutionStrategies, resolvedNumberOfDilutions, resolvedTargetAnalytes, resolvedDiluents,
		resolvedConcentratedBuffers, resolvedConcentratedBufferDiluents, resolvedCumulativeDilutionFactors, resolvedSerialDilutionFactors,
		resolvedTargetAnalyteConcentrations, resolvedTransferVolumes, resolvedTotalDilutionVolumes, resolvedDiscardFinalTransfers,
		resolvedFinalVolumes, resolvedDiluentVolumes, resolvedConcentratedBufferVolumes, resolvedConcentratedBufferDiluentVolumes,
		resolvedConcentratedBufferDilutionFactors, resolvedIncubates,
		(* Sample creation for mix *)
		simulatedSamples, simulatedSampleAndContainerSimulation,
		(* Mix options *)
		resolvedIncubationTimes, resolvedIncubationInstruments, resolvedIncubationTemperatures, resolvedMixTypes, resolvedNumberOfMixes,
		resolvedMixRates, resolvedMixOscillationAngles, experimentMixTests,
		(* Invalid options and Conflicting option checks *)
		conflictingFactorsAndVolumesOptions, conflictingFactorsAndVolumesTests, multipleTargetAnalyteConcentrationUnitsOptions,
		multipleTargetAnalyteConcentrationUnitsTests, conflictingTargetAnalyteConcentrationUnitsOptions, conflictingTargetAnalyteConcentrationUnitsTests,
		invalidCumulativeDilutionErrorCases, invalidCumulativeDilutionTests, dilutionTypeStrategyMismatch, dilutionTypeStrategyMismatchTests,
		overspecifiedDilutionMismatch, overspecifiedDilutionMismatchTests, conflictingConcentratedBufferOptionsResult,
		conflictingConcentratedBufferOptionsTests, conflictingDiluentOptionsResult, conflictingDiluentOptionsTests,
		invalidResolvedVolumes, invalidResolvedVolumeTests, invalidMixOptions,
		(* Resolved options *)
		resolvedOptions, collapsedResolvedOptions, invalidOptions, invalidInputs, allTests
	},
	...
	validExpandedLengthTests = If[gatherTests,
		Module[{failingSamples, passingSamples, failingSampleTests, passingSampleTests},
			...
			failingSampleTests = If[Length[failingSamples] > 0,
				Test["For the following samples " <> ObjectToString[failingSamples, Cache -> cache, Simulation -> currentSimulation] <> ", the provided nested lengths align with NumberOfDilutions:", False, True],
				Nothing
			];

			(*Create a test for the passing inputs*)
			passingSampleTests = If[Length[passingSamples] > 0,
				Test["For the following samples " <> ObjectToString[passingSamples, Cache -> cache, Simulation -> currentSimulation] <> ", the provided nested lengths align with NumberOfDilutions:", True, True],
				Nothing
			];
	...
	{diluent, concentratedBuffer, concentratedBufferDiluent} = Lookup[
		expandedSafeOps,
	...
	preparationResult = Check[
		{allowedPreparation, preparationTest} = If[MatchQ[gatherTests, False],
			{
				resolveDilutionSharedOptionsMethod[listedSamples, ReplaceRule[listedOptions, {Cache -> combinedCache, Simulation -> currentSimulation, Output -> Result}]],
				{}
			},
			resolveDilutionSharedOptionsMethod[listedSamples, ReplaceRule[listedOptions, {Cache -> combinedCache, Simulation -> currentSimulation, Output -> {Result, Tests}}]]
		],
		$Failed
	];
	...
	{
		(*1*)noDiluentSpecifiedWarnings,
		(*2*)missingTargetAnalyteInitialConcentrationWarnings,
		(*3*)conflictingFactorsAndVolumesErrors,
		(*4*)noSampleVolumeWarnings,
		(*5*)multipleTargetAnalyteConcentrationUnitsErrors,
		(*6*)conflictingTargetAnalyteConcentrationUnitsErrors,
		(*7*)invalidCumulativeDilutionErrors,
		(*8*)resolvedDilutionTypes,
		(*9*)resolvedDilutionStrategies,
		(*10*)resolvedNumberOfDilutions,
		(*11*)resolvedTargetAnalytes,
		(*12*)resolvedDiluents,
		(*13*)resolvedConcentratedBuffers,
		(*14*)resolvedConcentratedBufferDiluents,
		(*15*)resolvedCumulativeDilutionFactors,
		(*16*)resolvedSerialDilutionFactors,
		(*17*)resolvedTargetAnalyteConcentrations,
		(*18*)resolvedTransferVolumes,
		(*19*)resolvedTotalDilutionVolumes,
		(*20*)resolvedDiscardFinalTransfers,
		(*21*)resolvedFinalVolumes,
		(*22*)resolvedDiluentVolumes,
		(*23*)resolvedConcentratedBufferVolumes,
		(*24*)resolvedConcentratedBufferDiluentVolumes,
		(*25*)resolvedConcentratedBufferDilutionFactors,
		(*26*)resolvedIncubates,
		(*27*)targetAnalyteTests
		...
		Module[
			{
				...
				conflictingFactorAndVolumeError, noSampleVolumeWarning, unableToConvertError, multipleTargetAnalyteConcentrationUnitsError,
				conflictingTargetAnalyteConcentrationUnitsError, invalidCumulativeDilutionQ,
				...
				specifiedSerialDilutionFactor, specifiedTargetAnalyteConcentration, specifiedTransferVolume, specifiedTotalDilutionVolume,
				specifiedDiscardFinalTransfer, specifiedFinalVolume, specifiedDiluentVolume, specifiedConcentratedBufferVolume,
				specifiedConcentratedBufferDiluentVolume, specifiedConcentratedBufferDilutionFactor, specifiedIncubate,
				specifiedIncubationTime, specifiedIncubationInstrument, specifiedIncubationTemperature, specifiedMixType, specifiedNumberOfMixes,
				specifiedMixRate, specifiedMixOscillationAngle, expandedSpecifiedCumulativeDilutionFactor,
				expandedSpecifiedSerialDilutionFactor, expandedSpecifiedTargetAnalyteConcentration, expandedSpecifiedTransferVolume,
				expandedSpecifiedTotalDilutionVolume, expandedSpecifiedFinalVolume, expandedSpecifiedDiluentVolume,
				expandedSpecifiedConcentratedBufferVolume, expandedSpecifiedConcentratedBufferDiluentVolume,
				expandedSpecifiedConcentratedBufferDilutionFactor,
				(* Other Variables *)
				targetAnalyteTest, diluentCase, targetAnalyteInitialConcentration, updatedDiluentCase, concentrationUnits,
				convertedTargetAnalyteInitialConcentration, intermediateTransferVolume, intermediateDiluentVolume,
				intermediateConcentratedBufferVolume, intermediateConcentratedBufferDiluentVolume, intermediateFinalVolume,
				intermediateTotalDilutionVolume, intermediateSerialDilutionFactor,
				(* Resolved options *)
				...
				resolvedDiscardFinalTransfer, roundedResolvedFinalVolume, resolvedFinalVolume, resolvedDiluentVolume, resolvedConcentratedBufferVolume,
			...
			resolvedDilutionStrategy = Which[
				...
				MatchQ[resolvedDilutionType, Serial], Endpoint,
				True, Null
			];
			...
			{resolvedTargetAnalyte, targetAnalyteTest} = If[MatchQ[specifiedTargetAnalyte, Except[Automatic]],
				...
				Module[{potentialAnalytesToUse, potentialAnalyteTests},
					...
					{potentialAnalytesToUse, potentialAnalyteTests} = If[gatherTests,
						selectAnalyteFromSample[mySample, Cache -> combinedCache, Simulation -> currentSimulation, Output -> {Result, Tests}],
						{selectAnalyteFromSample[mySample, Cache -> combinedCache, Simulation -> currentSimulation, Output -> Result], {}}
					];
			...
			noTargetAnalyteFound = MatchQ[resolvedTargetAnalyte, Null];
			...
			diluentCase = Module[{diluentOptionSetBool, concentratedBufferOptionSetBool},
				...
				Which[
					...
					diluentOptionSetBool && concentratedBufferOptionSetBool, "Not Possible",
					(* If only Diluent, use it *)
					diluentOptionSetBool && !concentratedBufferOptionSetBool, "Diluent",
					(* If only ConcentratedBuffer, use it *)
					concentratedBufferOptionSetBool && !diluentOptionSetBool, "Concentrated Buffer",
			...
			(* Resolve Diluent *)
			{resolvedDiluent, noDiluentSpecifiedWarning} = Which[
				MatchQ[diluentCase, "Concentrated Buffer"],
				...
				MatchQ[fastAssocLookup[fastAssoc, mySample, Solvent], ObjectP[Model[Sample]]],
					{Download[fastAssocLookup[fastAssoc, mySample, Solvent], Object], False},
				(* Finally, resolve to water and Mark a warning *)
				True,
					{Model[Sample, "id:8qZ1VWNmdLBD"], True} (* Model[Sample, "Milli-Q water"] *)
			];
			...
			resolvedDiscardFinalTransfer = Which[
				...
				MatchQ[specifiedDiscardFinalTransfer, Except[Automatic]], specifiedDiscardFinalTransfer,
				(* If DilutionType is Serial, default to True *)
				MatchQ[resolvedDilutionType, Serial], True,
			...
			targetAnalyteInitialConcentration = If[!NullQ[resolvedTargetAnalyte],
				FirstCase[fastAssocLookup[fastAssoc, mySample, Composition], {conc : MassConcentrationP | ConcentrationP | CellConcentrationP | CFUConcentrationP | OD600P, ObjectP[resolvedTargetAnalyte], _} :> conc, Null],
				Null
			];

			(* Did we find a concentration? *)
			noTargetAnalyteInitialConcentrationFound = NullQ[targetAnalyteInitialConcentration];
			...
			(* Make sure all of the nested index matching options are fully expanded before resolving them *)
			{
				...
				Function[{optionValue},
					Which[
						...
						MatchQ[optionValue, {Automatic..}], ConstantArray[Automatic, resolvedNumberOfDilutions],
						(* Otherwise, if the option value is a list - keep it *)
						MatchQ[optionValue, _List], optionValue,
						(* If the option is not a list, do a constant array of NumberOfDilutions *)
						True, ConstantArray[optionValue, resolvedNumberOfDilutions]
					]
				],
				{
			...
			(* When the specified cumulativeDilutionFactor does not make sense, do not need to resolve options *)
			invalidCumulativeDilutionQ = Which[
				MatchQ[resolvedDilutionType, Linear] && MemberQ[expandedSpecifiedCumulativeDilutionFactor, EqualP[1]], True,
				MatchQ[resolvedDilutionType, Serial] && MemberQ[Differences[expandedSpecifiedCumulativeDilutionFactor], LessP[0]], True,
				True, False
			];

			updatedDiluentCase = If[TrueQ[invalidCumulativeDilutionQ],
				"Not Possible",
				diluentCase
			];
			...
			{concentrationUnits, convertedTargetAnalyteInitialConcentration} = Module[{unitFromTargetAnalyteConcentration, initialTargetAnalyteUnits, cellUnits},
				...
				unitFromTargetAnalyteConcentration = Switch[expandedSpecifiedTargetAnalyteConcentration,
					...
					{MassConcentrationP..} | {ConcentrationP..} | {OD600P..} | {CFUConcentrationP..} | {CellConcentrationP..},
					...
					Alternatives[
						{Automatic | Null | MassConcentrationP..},
						{Automatic | Null | ConcentrationP..},
						{Automatic | Null | CellConcentrationP..},
						{Automatic | Null | CFUConcentrationP..},
						{Automatic | Null | OD600P..}
					],
						Units[FirstCase[expandedSpecifiedTargetAnalyteConcentration, MassConcentrationP | ConcentrationP | Alternatives @@ cellUnits]],
					...
					_,
						Module[{},
							...
							Units[FirstCase[expandedSpecifiedTargetAnalyteConcentration, MassConcentrationP | ConcentrationP | Alternatives @@ cellUnits, Null]]
						]
				];
				...
				Switch[{initialTargetAnalyteUnits, unitFromTargetAnalyteConcentration},
					...
					{Automatic | Null, Automatic | Null},
						Which[
							MatchQ[resolvedTargetAnalyte, ObjectP[Model[Cell]]],
								{Cell / Milliliter, 1 Cell / Milliliter},
							MatchQ[resolvedTargetAnalyte, ObjectP[]],
								{Mole / Liter, 1 Micromole / Liter},
							True,
								{Mole / Liter, 1 Micromole / Liter}
						],

					(* If only initial units are populated, use our canonical form of that unit *)
					{MassConcentrationP, Automatic | Null}, {Milligram / Milliliter, Convert[targetAnalyteInitialConcentration, Milligram / Milliliter]},
					...
					{CFUConcentrationP, Automatic | Null}, {CFU / Milliliter, Convert[targetAnalyteInitialConcentration, CFU / Milliliter]},
					...
					{Automatic | Null, MassConcentrationP}, {Milligram / Liter, Convert[targetAnalyteInitialConcentration, Milligram / Liter]},
					...
					{Automatic | Null, CFUConcentrationP}, {CFU / Milliliter, Convert[targetAnalyteInitialConcentration, CFU / Milliliter]},
					{Automatic | Null, OD600P}, {OD600, targetAnalyteInitialConcentration},

					(* If they are both MassConcentrations, return Milligram / Milliliter *)
					{MassConcentrationP, MassConcentrationP}, {Milligram / Milliliter, Convert[targetAnalyteInitialConcentration, Milligram / Milliliter]},
					...
					{CFUConcentrationP, CFUConcentrationP}, {CFU / Milliliter, Convert[targetAnalyteInitialConcentration, CFU / Milliliter]},
					...
					(* also do this if we have MassConcentration and Concentration or vice versa *)
					(* Just strip the units of the initial concentration - the math will always be wrong we just need to not crash *)
					Alternatives[
						{Alternatives @@ cellUnits, ConcentrationP | MassConcentrationP},
						{ConcentrationP | MassConcentrationP, Alternatives @@ cellUnits},
						{ConcentrationP, MassConcentrationP},
						{MassConcentrationP, ConcentrationP}
					],
						Module[{},
							conflictingTargetAnalyteConcentrationUnitsError = {True, unitFromTargetAnalyteConcentration, initialTargetAnalyteUnits};
			...
			{
				(*1*)intermediateTransferVolume,
				(*2*)intermediateDiluentVolume,
				(*3*)intermediateConcentratedBufferVolume,
				(*4*)intermediateConcentratedBufferDiluentVolume,
				(*5*)intermediateFinalVolume,
				(*6*)intermediateTotalDilutionVolume,
				(*7*)resolvedTargetAnalyteConcentration,
				(*8*)resolvedCumulativeDilutionFactor,
				(*9*)intermediateSerialDilutionFactor,
				(*10*)resolvedConcentratedBufferDilutionFactor,
				(*11*)conflictingFactorAndVolumeError
			} = Switch[updatedDiluentCase,

				"Diluent",
					Module[
						{
							intermediateConcentratedBufferVolumesForDiluent, intermediateConcentratedBufferDiluentVolumesForDiluent,
							resolvedConcentratedBufferDilutionFactorsForDiluent, currentTotalSerialDilutionFactor,
							currentSampleVolume, currentSampleAnalyteConcentration, intermediateTransferVolumesForDiluent,
							intermediateDiluentVolumesForDiluent,intermediateFinalVolumesForDiluent, intermediateTotalDilutionVolumesForDiluent,
							resolvedTargetAnalyteConcentrationsForDiluent, resolvedCumulativeDilutionFactorsForDiluent,
							intermediateSerialDilutionFactorsForDiluent, conflictingFactorsAndVolumesErrorsForDiluent
						},

						(* We first know that the concentrated buffer options resolve to Null in this case - so do that *)
						intermediateConcentratedBufferVolumesForDiluent = expandedSpecifiedConcentratedBufferVolume/.Automatic -> Null;
						intermediateConcentratedBufferDiluentVolumesForDiluent = expandedSpecifiedConcentratedBufferDiluentVolume/.Automatic -> Null;
						resolvedConcentratedBufferDilutionFactorsForDiluent = expandedSpecifiedConcentratedBufferDilutionFactor/.Automatic -> Null;
						...
						{
							(*1*)intermediateTransferVolumesForDiluent,
							(*2*)intermediateDiluentVolumesForDiluent,
							(*3*)intermediateFinalVolumesForDiluent,
							(*4*)intermediateTotalDilutionVolumesForDiluent,
							(*5*)resolvedTargetAnalyteConcentrationsForDiluent,
							(*6*)resolvedCumulativeDilutionFactorsForDiluent,
							(*7*)intermediateSerialDilutionFactorsForDiluent,
							(*8*)conflictingFactorsAndVolumesErrorsForDiluent
						} = Transpose@MapThread[
							Function[
								{
									diluentVolume,
								...
								Module[
									{
										...
										conflictingFactorsAndVolumesError, intermediateTransferVolumeForDiluent, intermediateDiluentVolumeForDiluent,
										intermediateFinalVolumeForDiluent, intermediateTotalDilutionVolumeForDiluent, resolvedTargetAnalyteConcentrationForDiluent,
										resolvedCumulativeDilutionFactorForDiluent, intermediateSerialDilutionFactorForDiluent
									},
									...
									(* If CumulativeDilutionFactor is correct but myEquation is not solvable, mark it as ConflictingFactorsAndVolumes error *)
									conflictingFactorsAndVolumesError = And[
										!TrueQ[allOptionsSolvedForBool],
										MatchQ[solvedEquationsNoDefaults, {}]
									];

									{
										(*1*)intermediateTransferVolumeForDiluent,
										(*2*)intermediateDiluentVolumeForDiluent,
										(*3*)intermediateFinalVolumeForDiluent,
										(*4*)intermediateTotalDilutionVolumeForDiluent,
										(*5*)resolvedTargetAnalyteConcentrationForDiluent,
										(*6*)resolvedCumulativeDilutionFactorForDiluent,
										(*7*)intermediateSerialDilutionFactorForDiluent
									} = Which[
										...
										TrueQ[allOptionsSolvedForBool],
											Module[{},
												...
												{
													(*1*)If[MatchQ[transferVolume, Except[Automatic]], transferVolume, UnitScale[(scaledTransferVolume /. solvedEquationsNoDefaults) * Liter]],
													(*2*)If[MatchQ[diluentVolume, Except[Automatic]], diluentVolume, UnitScale[(scaledDiluentVolume /. solvedEquationsNoDefaults) * Liter]],
													(*3*)If[MatchQ[finalVolume, Except[Automatic]], finalVolume, UnitScale[(scaledFinalVolume /. solvedEquationsNoDefaults) * Liter]],
													(*4*)If[MatchQ[totalDilutionVolume, Except[Automatic]], totalDilutionVolume, UnitScale[(scaledTotalDilutionVolume /. solvedEquationsNoDefaults) * Liter]],
													(*5*)If[MatchQ[targetAnalyteConcentration, Except[Automatic]], targetAnalyteConcentration, UnitScale[(scaledTargetAnalyteConcentration /. solvedEquationsNoDefaults) * concentrationUnits]],
													(*6*)If[NumericQ[cumulativeDilutionFactor], cumulativeDilutionFactor, N[scaledCumulativeDilutionFactor /. solvedEquationsNoDefaults]],
													(*7*)If[NumericQ[serialDilutionFactor], serialDilutionFactor, N[scaledSerialDilutionFactor /. solvedEquationsNoDefaults]]
												}
											],
										...
										!MatchQ[solvedEquationsNoDefaults, {}],
											Module[{varsNeededToSolve, solvedEquationsWithDefaults},
												...
												(* deleting False case because if we have a False then we probably have an error that's going to be thrown below and having the False makes us trainwreck *)
												solvedEquationsWithDefaults = Quiet[FirstOrDefault[Solve[DeleteCases[myEquations, False]], {}]];

												(* Do a final check to see if we have solved everything *)
												Module[{LocalVarsNeededToSolve},

													(* Extract the variables that are still needed to solve the equations *)
													LocalVarsNeededToSolve = DeleteDuplicates@Cases[solvedEquationsWithDefaults[[All, 2]], _Symbol, Infinity];

													If[ContainsOnly[LocalVarsNeededToSolve, {scaledTargetAnalyteInitialConcentration, scaledTargetAnalyteConcentration}],
														...
														Module[{},
															...
															(* Gather our resolved options in reasonable units *)
															{
																(*1*)If[MatchQ[transferVolume, Except[Automatic]], transferVolume, UnitScale[(scaledTransferVolume /. solvedEquationsWithDefaults) * Liter]],
																(*2*)If[MatchQ[diluentVolume, Except[Automatic]], diluentVolume, UnitScale[(scaledDiluentVolume /. solvedEquationsWithDefaults) * Liter]],
																(*3*)If[MatchQ[finalVolume, Except[Automatic]], finalVolume, UnitScale[(scaledFinalVolume /. solvedEquationsWithDefaults) * Liter]],
																(*4*)If[MatchQ[totalDilutionVolume, Except[Automatic]], totalDilutionVolume, UnitScale[(scaledTotalDilutionVolume /. solvedEquationsWithDefaults) * Liter]],
																(*5*)If[MatchQ[targetAnalyteConcentration, Except[Automatic]], targetAnalyteConcentration, If[noTargetAnalyteInitialConcentrationFound, Null, UnitScale[(scaledTargetAnalyteConcentration /. solvedEquationsWithDefaults) * concentrationUnits]]],
																(*6*)If[NumericQ[cumulativeDilutionFactor], cumulativeDilutionFactor, N[scaledCumulativeDilutionFactor /. solvedEquationsWithDefaults]],
																(*7*)If[NumericQ[serialDilutionFactor], serialDilutionFactor, N[scaledSerialDilutionFactor /. solvedEquationsWithDefaults]]
															}
														],
														...
														Module[{},
															...
															(* Gather our resolved options in reasonable units *)
															{
																(*1*)If[MatchQ[transferVolume, Except[Automatic]], transferVolume, Null],
																(*2*)If[MatchQ[diluentVolume, Except[Automatic]], diluentVolume, Null],
																(*3*)If[MatchQ[finalVolume, Except[Automatic]], finalVolume, Null],
																(*4*)If[MatchQ[totalDilutionVolume, Except[Automatic]], totalDilutionVolume, Null],
																(*5*)If[MatchQ[targetAnalyteConcentration, Except[Automatic]], targetAnalyteConcentration, Null],
																(*6*)If[NumericQ[cumulativeDilutionFactor], cumulativeDilutionFactor, Null],
																(*7*)If[NumericQ[serialDilutionFactor], serialDilutionFactor, Null]
															}
														]
													]
												]
									...
									(* Return values and error checkings *)
									{
										(*1*)intermediateTransferVolumeForDiluent,
										(*2*)intermediateDiluentVolumeForDiluent,
										(*3*)intermediateFinalVolumeForDiluent,
										...
										(*6*)resolvedCumulativeDilutionFactorForDiluent,
										(*7*)intermediateSerialDilutionFactorForDiluent,
										(*8*)conflictingFactorsAndVolumesError
									}
								]
							],
							{
						...
						];
						{
							(*1*)intermediateTransferVolumesForDiluent,
							(*2*)intermediateDiluentVolumesForDiluent,
							(*3*)intermediateConcentratedBufferVolumesForDiluent,
							(*4*)intermediateConcentratedBufferDiluentVolumesForDiluent,
							(*5*)intermediateFinalVolumesForDiluent,
							(*6*)intermediateTotalDilutionVolumesForDiluent,
							(*7*)resolvedTargetAnalyteConcentrationsForDiluent,
							(*8*)resolvedCumulativeDilutionFactorsForDiluent,
							(*9*)intermediateSerialDilutionFactorsForDiluent,
							(*10*)resolvedConcentratedBufferDilutionFactorsForDiluent,
							(*11*)conflictingFactorsAndVolumesErrorsForDiluent
						}
					],
				...
				"Concentrated Buffer",
					Module[
						{
							intermediateDiluentVolumeForConcentratedBuffer, resolvedConcentratedBufferCBDF, currentTotalSerialDilutionFactor,
							currentSampleVolume, currentSampleSolventDilutionFactor, currentSampleAnalyteConcentration,
							intermediateTransferVolumesForConcentratedBuffer, intermediateConcentratedBufferVolumesForConcentratedBuffer,
							intermediateConcentratedBufferDiluentVolumesForConcentratedBuffer, intermediateFinalVolumesForConcentratedBuffer,
							intermediateTotalDilutionVolumesForConcentratedBuffer, resolvedTargetAnalyteConcentrationsForConcentratedBuffer,
							resolvedCumulativeDilutionFactorsForConcentratedBuffer, intermediateSerialDilutionFactorsForConcentratedBuffer,
							resolvedConcentratedBufferDilutionFactorsForConcentratedBuffer, conflictingFactorsAndVolumesErrorsForConcentratedBuffer
						},
						(* We first know that the diluent options resolve to Null in this case - so do that*)
						intermediateDiluentVolumeForConcentratedBuffer = expandedSpecifiedDiluentVolume/.Automatic -> Null;
						...
						(* Map over our expanded values *)
						{
							(*1*)intermediateTransferVolumesForConcentratedBuffer,
							(*2*)intermediateConcentratedBufferVolumesForConcentratedBuffer,
							(*3*)intermediateConcentratedBufferDiluentVolumesForConcentratedBuffer,
							(*4*)intermediateFinalVolumesForConcentratedBuffer,
							(*5*)intermediateTotalDilutionVolumesForConcentratedBuffer,
							(*6*)resolvedTargetAnalyteConcentrationsForConcentratedBuffer,
							(*7*)resolvedCumulativeDilutionFactorsForConcentratedBuffer,
							(*8*)intermediateSerialDilutionFactorsForConcentratedBuffer,
							(*9*)resolvedConcentratedBufferDilutionFactorsForConcentratedBuffer,
							(*10*)conflictingFactorsAndVolumesErrorsForConcentratedBuffer
						} = Transpose@MapThread[
							Function[
								{
								...
								Module[
									{
										firstDilutionSolventNotConcBool, sameBaselineStockBool, myEquations, scaledConcentratedBufferVolume,
										scaledConcentratedBufferDiluentVolume, scaledFinalVolume, scaledTransferVolume,
										scaledTotalDilutionVolume, scaledTargetAnalyteInitialConcentration, scaledTargetAnalyteConcentration,
										scaledCumulativeDilutionFactor, scaledSerialDilutionFactor, scaledConcentratedBufferDilutionFactor,
										scaledSDFTilNow, scaledSolventConcentrationFactor, solvedEquationsNoDefaults, allOptionsSolvedForBool,
										conflictingFactorsAndVolumesError, intermediateTransferVolumeForConcentratedBuffer,
										intermediateConcentratedBufferVolumeForConcentratedBuffer, intermediateConcentratedBufferDiluentVolumeForConcentratedBuffer,
										intermediateFinalVolumeForConcentratedBuffer, intermediateTotalDilutionVolumeForConcentratedBuffer,
										resolvedTargetAnalyteConcentrationForConcentratedBuffer, resolvedCumulativeDilutionFactorForConcentratedBuffer,
										intermediateSerialDilutionFactorForConcentratedBuffer, resolvedConcentratedBufferDilutionFactorForConcentratedBuffer
									},
									...
									{
										(*1*)intermediateTransferVolumeForConcentratedBuffer,
										(*2*)intermediateConcentratedBufferVolumeForConcentratedBuffer,
										(*3*)intermediateConcentratedBufferDiluentVolumeForConcentratedBuffer,
										(*4*)intermediateFinalVolumeForConcentratedBuffer,
										(*5*)intermediateTotalDilutionVolumeForConcentratedBuffer,
										(*6*)resolvedTargetAnalyteConcentrationForConcentratedBuffer,
										(*7*)resolvedCumulativeDilutionFactorForConcentratedBuffer,
										(*8*)intermediateSerialDilutionFactorForConcentratedBuffer,
										(*9*)resolvedConcentratedBufferDilutionFactorForConcentratedBuffer
									} = Which[
										(* WOO we solved everything *)
										TrueQ[allOptionsSolvedForBool],
											Module[{},
												...
												{
													(*1*)If[MatchQ[transferVolume, Except[Automatic]], transferVolume, UnitScale[(scaledTransferVolume /. solvedEquationsNoDefaults) * Liter]],
													(*2*)If[MatchQ[concentratedBufferVolume, Except[Automatic]], concentratedBufferVolume, UnitScale[(scaledConcentratedBufferVolume /. solvedEquationsNoDefaults) * Liter]],
													(*3*)If[MatchQ[concentratedBufferDiluentVolume, Except[Automatic]], concentratedBufferDiluentVolume, UnitScale[(scaledConcentratedBufferDiluentVolume /. solvedEquationsNoDefaults) * Liter]],
													(*4*)If[MatchQ[finalVolume, Except[Automatic]], finalVolume, UnitScale[(scaledFinalVolume /. solvedEquationsNoDefaults) * Liter]],
													(*5*)If[MatchQ[totalDilutionVolume, Except[Automatic]], totalDilutionVolume, UnitScale[(scaledTotalDilutionVolume /. solvedEquationsNoDefaults) * Liter]],
													(*6*)If[MatchQ[targetAnalyteConcentration, Except[Automatic]], targetAnalyteConcentration, UnitScale[(scaledTargetAnalyteConcentration /. solvedEquationsNoDefaults) * concentrationUnits]],
													(*7*)If[NumericQ[cumulativeDilutionFactor], cumulativeDilutionFactor, N[scaledCumulativeDilutionFactor /. solvedEquationsNoDefaults]],
													(*8*)If[NumericQ[serialDilutionFactor], serialDilutionFactor, N[scaledSerialDilutionFactor /. solvedEquationsNoDefaults]],
													(*9*)If[MatchQ[concentratedBufferDilutionFactor, Except[Automatic]], concentratedBufferDilutionFactor, N[scaledConcentratedBufferDilutionFactor /. solvedEquationsNoDefaults]]
												}
											],
												...
												Module[{localVarsNeededToSolve},
													...
													If[ContainsOnly[localVarsNeededToSolve, {scaledTargetAnalyteInitialConcentration, scaledTargetAnalyteConcentration}],
														...
														Module[{},
															...
															(* Gather our resolved options in reasonable units *)
															{
																(*1*)If[MatchQ[transferVolume, Except[Automatic | Null]], transferVolume, UnitScale[(scaledTransferVolume /. solvedEquationsWithDefaults) * Liter]],
																(*2*)If[MatchQ[concentratedBufferVolume, Except[Automatic | Null]], concentratedBufferVolume, UnitScale[(scaledConcentratedBufferVolume /. solvedEquationsWithDefaults) * Liter]],
																(*3*)If[MatchQ[concentratedBufferDiluentVolume, Except[Automatic | Null]], concentratedBufferDiluentVolume, UnitScale[(scaledConcentratedBufferDiluentVolume /. solvedEquationsWithDefaults) * Liter]],
																(*4*)If[MatchQ[finalVolume, Except[Automatic | Null]], finalVolume, UnitScale[(scaledFinalVolume /. solvedEquationsWithDefaults) * Liter]],
																(*5*)If[MatchQ[totalDilutionVolume, Except[Automatic | Null]], totalDilutionVolume, UnitScale[(scaledTotalDilutionVolume /. solvedEquationsWithDefaults) * Liter]],
																(*6*)If[MatchQ[targetAnalyteConcentration, Except[Automatic | Null]], targetAnalyteConcentration, If[noTargetAnalyteInitialConcentrationFound, Null, UnitScale[(scaledTargetAnalyteConcentration /. solvedEquationsWithDefaults) * concentrationUnits]]],
																(*7*)If[NumericQ[cumulativeDilutionFactor], cumulativeDilutionFactor, N[scaledCumulativeDilutionFactor /. solvedEquationsWithDefaults]],
																(*8*)If[NumericQ[serialDilutionFactor], serialDilutionFactor, N[scaledSerialDilutionFactor /. solvedEquationsWithDefaults]],
																(*9*)If[MatchQ[concentratedBufferDilutionFactor, Except[Automatic | Null]], concentratedBufferDilutionFactor, N[scaledConcentratedBufferDilutionFactor /. solvedEquationsWithDefaults]]
															}
														],
														(* Otherwise, the math must still be impossible *)
														Module[{},
															...
															{
																(*1*)If[MatchQ[transferVolume, Except[Automatic]], transferVolume, Null],
																(*2*)If[MatchQ[concentratedBufferVolume, Except[Automatic]], concentratedBufferVolume, Null],
																(*3*)If[MatchQ[concentratedBufferDiluentVolume, Except[Automatic]], concentratedBufferDiluentVolume, Null],
																(*4*)If[MatchQ[finalVolume, Except[Automatic]], finalVolume, Null],
																(*5*)If[MatchQ[totalDilutionVolume, Except[Automatic]], totalDilutionVolume, Null],
																(*6*)If[MatchQ[targetAnalyteConcentration, Except[Automatic]], targetAnalyteConcentration, Null],
																(*7*)If[NumericQ[cumulativeDilutionFactor], cumulativeDilutionFactor, Null],
																(*8*)If[NumericQ[serialDilutionFactor], serialDilutionFactor, Null],
																(*9*)If[MatchQ[concentratedBufferDilutionFactor, Except[Automatic]], concentratedBufferDilutionFactor, Null]
															}
														]
													]
												]
									...
									];
									{
										...
										(*8*)intermediateSerialDilutionFactorForConcentratedBuffer,
										(*9*)resolvedConcentratedBufferDilutionFactorForConcentratedBuffer,
										(*10*)conflictingFactorsAndVolumesError
									}
								]
							],
							{
						...
						{
							(*1*)intermediateTransferVolumesForConcentratedBuffer,
							(*2*)intermediateDiluentVolumeForConcentratedBuffer,
							(*3*)intermediateConcentratedBufferVolumesForConcentratedBuffer,
							(*4*)intermediateConcentratedBufferDiluentVolumesForConcentratedBuffer,
							(*5*)intermediateFinalVolumesForConcentratedBuffer,
							(*6*)intermediateTotalDilutionVolumesForConcentratedBuffer,
							(*7*)resolvedTargetAnalyteConcentrationsForConcentratedBuffer,
							(*8*)resolvedCumulativeDilutionFactorsForConcentratedBuffer,
							(*9*)intermediateSerialDilutionFactorsForConcentratedBuffer,
							(*10*)resolvedConcentratedBufferDilutionFactorsForConcentratedBuffer,
							(*11*)conflictingFactorsAndVolumesErrorsForConcentratedBuffer
						}
					],

				"Not Possible",
					{
						(*1*)expandedSpecifiedTransferVolume,
						(*2*)expandedSpecifiedDiluentVolume,
						(*3*)expandedSpecifiedConcentratedBufferVolume,
						(*4*)expandedSpecifiedConcentratedBufferDiluentVolume,
						(*5*)expandedSpecifiedFinalVolume,
						(*6*)expandedSpecifiedTotalDilutionVolume,
						(*7*)expandedSpecifiedTargetAnalyteConcentration,
						(*8*)expandedSpecifiedCumulativeDilutionFactor,
						(*9*)expandedSpecifiedSerialDilutionFactor,
						(*10*)expandedSpecifiedConcentratedBufferDilutionFactor,
						(*11*)ConstantArray[False, Length[mySample]]
					} /. {Automatic -> Null}
			];
				...
				Function[{transferVolume, finalVolume, discardFinalTransfer, index},
					Module[{},
						Which[
							...
							NullQ[transferVolume] || NullQ[finalVolume], Null,
							...
							MatchQ[resolvedDilutionType, Serial] && discardFinalTransfer == False && index == Length[resolvedTransferVolume] && Length[resolvedTransferVolume] > 1, finalVolume + transferVolume,
			...
			resolvedSerialDilutionFactor = If[MatchQ[resolvedDilutionType, Linear],
				expandedSpecifiedSerialDilutionFactor/.Automatic -> Null,
			...
			{
				(*1*)noDiluentSpecifiedWarning,
				(*2*)missingTargetAnalyteInitialConcentrationWarning,
				(*3*)Or@@conflictingFactorAndVolumeError,
				(*4*)noSampleVolumeWarning,
				(*5*)multipleTargetAnalyteConcentrationUnitsError,
				(*6*)conflictingTargetAnalyteConcentrationUnitsError,
				(*7*)invalidCumulativeDilutionQ,
				(*8*)resolvedDilutionType,
				(*9*)resolvedDilutionStrategy,
				(*10*)resolvedNumberOfDilutions,
				(*11*)resolvedTargetAnalyte,
				(*12*)resolvedDiluent,
				(*13*)resolvedConcentratedBuffer,
				(*14*)resolvedConcentratedBufferDiluent,
				(*15*)resolvedCumulativeDilutionFactor,
				(*16*)resolvedSerialDilutionFactor,
				(*17*)resolvedTargetAnalyteConcentration,
				(*18*)resolvedTransferVolume,
				(*19*)resolvedTotalDilutionVolume,
				(*20*)resolvedDiscardFinalTransfer,
				(*21*)resolvedFinalVolume,
				(*22*)resolvedDiluentVolume,
				(*23*)resolvedConcentratedBufferVolume,
				(*24*)resolvedConcentratedBufferDiluentVolume,
				(*25*)resolvedConcentratedBufferDilutionFactor,
				(*26*)resolvedIncubate,
				(*27*)targetAnalyteTest
			}
		]
	],
	...
	{simulatedSamples, simulatedSampleAndContainerSimulation} = Module[{sampleMaxTotalVolumes, preferredContainersForSimulatedSamples, flattenedSimulatedSamplesAndContainers},
		...
		preferredContainersForSimulatedSamples = Module[{sterileContainerBools, cellTypeValues},
			...
			cellTypeValues = MapThread[
				Which[
					MemberQ[{#1, #2, #3, #4}, MicrobialCellTypeP], Bacterial,
					MemberQ[{#1, #2, #3, #4}, NonMicrobialCellTypeP], Mammalian,
					True, Null
				]&,
				{
			...
			MapThread[
				Module[{possiblePlate, possibleVessel},
					...
					Which[
						...
						!MatchQ[possiblePlate, $Failed], possiblePlate,
						(* If we couldn't find a plate but can find a vessel *)
						!MatchQ[possibleVessel, $Failed], possibleVessel,
	...
	(* Resolve the Mix options with a Single Mix call *)
	{
		...
		{
			dilutionLabels, dilutionLabelsNoNull, specifiedIncubationTimes, specifiedIncubationInstruments, specifiedIncubationTemperatures,
			specifiedMixTypes, specifiedNumberOfMixes, specifiedMixRates, specifiedMixOscillationAngles, incubationTimeWithLabels,
			incubationTimeToMix, incubationInstrumentWithLabels, incubationInstrumentToMix, incubationTemperatureWithLabels,
			incubationTemperatureToMix, mixTypeWithLabels, mixTypeWithToMix, numberOfMixesWithLabels, numberOfMixesToMix,
			mixRateWithLabels, mixRateToMix, mixOscillationAngleWithLabels, mixOscillationAngleToMix, samplesForMix, optionsForMix
		},
		...
		If[MatchQ[Length[samplesForMix], 0],
			...
			Module[{mixResult, mixTests},
				...
				{mixResult, mixTests} = If[gatherTests,
					...
					{
						Module[{invalidOptionsBool, resolvedMixOptions},

							(* Run the analysis function to get the resolved options *)
							{resolvedMixOptions, invalidOptionsBool, invalidMixOptions} = ModifyFunctionMessages[
								ExperimentMix,
								{samplesForMix},
								"",
								{},
	...
	If[MemberQ[noDiluentSpecifiedWarnings, True] && messages,
		Message[
			Warning::NoDiluentSpecified,
			ObjectToString[PickList[listedSamples, noDiluentSpecifiedWarnings], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	If[MemberQ[missingTargetAnalyteInitialConcentrationWarnings, True] && warnings,
		Message[
			Warning::MissingTargetAnalyteInitialConcentration,
			ObjectToString[PickList[listedSamples, missingTargetAnalyteInitialConcentrationWarnings], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	conflictingFactorsAndVolumesOptions = If[MemberQ[conflictingFactorsAndVolumesErrors, True] && messages,
		(
			Message[
				Error::ConflictingFactorsAndVolumes,
				ObjectToString[PickList[listedSamples, conflictingFactorsAndVolumesErrors], Cache -> combinedCache, Simulation -> currentSimulation],
			...
			];
			{
				...
				ConcentratedBufferVolume, ConcentratedBufferDiluentVolume, ConcentratedBufferDilutionFactor
			}
		),
		{}
	];
	...
	If[MemberQ[noSampleVolumeWarnings, True] && messages,
		Message[
			Warning::NoSampleVolume,
			ObjectToString[PickList[listedSamples, noSampleVolumeWarnings], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	multipleTargetAnalyteConcentrationUnitsOptions = If[MemberQ[multipleTargetAnalyteConcentrationUnitsErrors[[All, 1]], True] && messages,
		Message[
			Error::MultipleTargetAnalyteConcentrationUnitsError,
			ObjectToString[PickList[listedSamples, multipleTargetAnalyteConcentrationUnitsErrors[[All, 1]]], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	conflictingTargetAnalyteConcentrationUnitsOptions = If[MemberQ[conflictingTargetAnalyteConcentrationUnitsErrors[[All, 1]], True] && messages,
		Message[
			Error::ConflictingTargetAnalyteConcentrationUnitsError,
			ObjectToString[PickList[listedSamples, conflictingTargetAnalyteConcentrationUnitsErrors[[All, 1]]], Cache -> combinedCache, Simulation -> currentSimulation],
			...
			ObjectToString[PickList[resolvedTargetAnalytes, conflictingTargetAnalyteConcentrationUnitsErrors[[All, 1]]], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	(* InvalidCumulativeDilutionFactors *)
	invalidCumulativeDilutionErrorCases = MapThread[
		Function[{sample, invalidCumulativeDilutionQ, cumulativeDilutionFactor, dilutionType, index},
			If[invalidCumulativeDilutionQ,
				{sample, dilutionType, cumulativeDilutionFactor, index},
				Nothing
			]
		],
		{
			listedSamples,
			invalidCumulativeDilutionErrors,
			resolvedCumulativeDilutionFactors,
			resolvedDilutionTypes,
			Range[Length[listedSamples]]
		}
	];
	If[MemberQ[invalidCumulativeDilutionErrors, True] && messages,
		Module[{captureSentence},
			captureSentence = Capitalize@joinClauses[{
				If[MemberQ[Flatten@invalidCumulativeDilutionErrorCases, Serial],
					"when DilutionType is Serial, each subsequent CumulativeDilutionFactor must be greater than the previous one, as it represents the cumulative product of all prior individual dilution steps",
					Nothing
				],
				If[MemberQ[Flatten@invalidCumulativeDilutionErrorCases, Linear],
					"when DilutionType is Linear, CumulativeDilutionFactor must be greater than 1",
					Nothing
				]
			}];
			Message[
				Error::InvalidCumulativeDilutionFactors,
				captureSentence,
				ObjectToString[invalidCumulativeDilutionErrorCases[[All, 1]], Cache -> combinedCache, Simulation -> currentSimulation],
				invalidCumulativeDilutionErrorCases[[All, 3]],
				invalidCumulativeDilutionErrorCases[[All, 4]]
			]
		]
	];
	invalidCumulativeDilutionTests = If[gatherTests,
		Module[{affectedSamples, failingTest, passingTest},
			affectedSamples = invalidCumulativeDilutionErrorCases[[All, 1]];

			failingTest = If[Length[affectedSamples] == 0,
				Nothing,
				Test["The sample(s) " <> ObjectToString[affectedSamples, Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have invalid CumulativeDilutionFactor options:", True, False]
			];

			passingTest = If[Length[affectedSamples] == Length[listedSamples],
				Nothing,
				Test["The sample(s) " <> ObjectToString[Complement[listedSamples, affectedSamples], Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have invalid CumulativeDilutionFactor options:", True, True]
			];
	...
	If[Length[dilutionTypeStrategyMismatch] > 0 && messages,
		Message[
			...
			ObjectToString[dilutionTypeStrategyMismatch[[All, 1]], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	dilutionTypeStrategyMismatchTests = If[gatherTests,
		Module[{affectedSamples, failingTest, passingTest},
			...
			failingTest = If[Length[affectedSamples] == 0,
				Nothing,
				Test["The sample(s) " <> ObjectToString[affectedSamples, Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have conflicting DilutionType and DilutionStrategy options:", True, False]
			];

			passingTest = If[Length[affectedSamples] == Length[listedSamples],
				Nothing,
				Test["The sample(s) " <> ObjectToString[Complement[listedSamples, affectedSamples], Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have conflicting DilutionType and DilutionStrategy options:", True, True]
			];
	...
	If[Length[overspecifiedDilutionMismatch] > 0,
		Message[
			...
			ObjectToString[overspecifiedDilutionMismatch[[All, 1]], Cache -> combinedCache, Simulation -> currentSimulation],
			ObjectToString[overspecifiedDilutionMismatch[[All, 2]], Cache -> combinedCache, Simulation -> currentSimulation],
			ObjectToString[overspecifiedDilutionMismatch[[All, 3]], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	overspecifiedDilutionMismatchTests = If[gatherTests,
		Module[{affectedSamples, failingTest, passingTest},
			...
			failingTest = If[Length[affectedSamples] == 0,
				Nothing,
				Test["The sample(s) " <> ObjectToString[affectedSamples, Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have both Diluent and ConcentratedBuffer specified:", True, False]
			];

			passingTest = If[Length[affectedSamples] == Length[listedSamples],
				Nothing,
				Test["The sample(s) " <> ObjectToString[Complement[listedSamples, affectedSamples], Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have both Diluent and ConcentratedBuffer specified:", True, True]
			];
	...
	(* Note for cases where invalidCumulativeDilutionQ is specified improperly, we are throwing InvalidCumulativeDilutionFactors. Do not double throw here. *)
		...
		Function[
			{
				...
				concentratedBufferDilutionFactor,
				invalidCumulativeDilutionQ,
				index
			},
			If[
				!invalidCumulativeDilutionQ && Or[
					And[
						MatchQ[concentratedBuffer, ObjectP[{Object[Sample], Model[Sample]}]],
						Or[
	...
	If[Length[conflictingConcentratedBufferOptionsResult] > 0,
		Message[
			...
			ObjectToString[conflictingConcentratedBufferOptionsResult[[All, 1]], Cache -> combinedCache, Simulation -> currentSimulation],
			ObjectToString[conflictingConcentratedBufferOptionsResult[[All, 2]], Cache -> combinedCache, Simulation -> currentSimulation],
			conflictingConcentratedBufferOptionsResult[[All, 3]],
			conflictingConcentratedBufferOptionsResult[[All, 4]],
			ObjectToString[conflictingConcentratedBufferOptionsResult[[All, 5]], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	conflictingConcentratedBufferOptionsTests = If[gatherTests,
		Module[{affectedSamples, failingTest, passingTest},
			...
			failingTest = If[Length[affectedSamples] == 0,
				Nothing,
				Test["The sample(s) " <> ObjectToString[affectedSamples, Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have conflicting Concentrated Buffer options:", True, False]
			];

			passingTest = If[Length[affectedSamples] == Length[listedSamples],
				Nothing,
				Test["The sample(s) " <> ObjectToString[Complement[listedSamples, affectedSamples], Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have conflicting Concentrated Buffer options:", True, True]
			];
	...
	conflictingDiluentOptionsResult = MapThread[
		Function[{sample, Diluent, DiluentVolume, invalidCumulativeDilutionQ, index},
			If[
				!invalidCumulativeDilutionQ && Or[
					And[
						MatchQ[Diluent, ObjectP[{Object[Sample], Model[Sample]}]],
		...
		{listedSamples, resolvedDiluents, resolvedDiluentVolumes, invalidCumulativeDilutionErrors, Range[Length[listedSamples]]}
	];

	If[Length[conflictingDiluentOptionsResult] > 0,
		Message[
			Error::ConflictingDiluentParameters,
			ObjectToString[conflictingDiluentOptionsResult[[All, 1]], Cache -> combinedCache, Simulation -> currentSimulation],
			ObjectToString[conflictingDiluentOptionsResult[[All, 2]], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	conflictingDiluentOptionsTests = If[gatherTests,
		Module[{affectedSamples, failingTest, passingTest},
			...
			failingTest = If[Length[affectedSamples] == 0,
				Nothing,
				Test["The sample(s) " <> ObjectToString[affectedSamples, Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have conflicting Diluent options:", True, False]
			];

			passingTest = If[Length[affectedSamples] == Length[listedSamples],
				Nothing,
				Test["The sample(s) " <> ObjectToString[Complement[listedSamples, affectedSamples], Cache -> combinedCache, Simulation -> currentSimulation] <> " do not have conflicting Diluent options:", True, True]
			];
	...
	(* Note if DilutionType is Serial and DiluentVolume is 0 Liter, it is okay. This is because we allow no dilution for serial dilution point1. *)
	invalidResolvedVolumes = MapThread[
		Function[{sample, transferVolume, diluentVolume, concentratedBufferVolume, concentratedBufferDiluentVolume, dilutionType, index},
			If[
				Or[
					Or @@ ((Which[NullQ[#], False, RangeQ[#, {0.1 Microliter, $MaxTransferVolume}], False, True, True])& /@ transferVolume),
					MatchQ[dilutionType, Serial] && !NullQ[diluentVolume] && MemberQ[Rest[diluentVolume], LessP[0.1 Microliter]|GreaterP[$MaxTransferVolume]],
					!MatchQ[dilutionType, Serial] && !NullQ[diluentVolume] && MemberQ[diluentVolume, LessP[0.1 Microliter]|GreaterP[$MaxTransferVolume]],
	...
	If[Length[invalidResolvedVolumes] > 0,
		Message[
			Error::VolumesTooLarge,
			ObjectToString[invalidResolvedVolumes[[All, 1]], Cache -> combinedCache, Simulation -> currentSimulation],
	...
	invalidResolvedVolumeTests = If[gatherTests,
		Module[{affectedSamples, failingTest, passingTest},
			...
			failingTest = If[Length[affectedSamples] == 0,
				Nothing,
				Test["The sample(s) " <> ObjectToString[affectedSamples, Cache -> combinedCache, Simulation -> currentSimulation] <> " have specified inputs that simplify to valid volume values in ECL:", True, False]
			];

			passingTest = If[Length[affectedSamples] == Length[listedSamples],
				Nothing,
				Nothing,
				Test["The sample(s) " <> ObjectToString[Complement[listedSamples, affectedSamples], Cache -> combinedCache, Simulation -> currentSimulation] <> " have specified inputs that simplify to valid volume values in ECL:", True, True]
			];
	...
	invalidOptions = DeleteDuplicates[Flatten[{
		...
		If[MemberQ[invalidCumulativeDilutionErrors, True],
			{CumulativeDilutionFactor},
			{}
		],
	...
	If[Length[invalidInputs] > 0 && !gatherTests,
		Message[Error::InvalidInput, ObjectToString[invalidInputs, Cache -> cache, Simulation -> currentSimulation]]
	];
	...
	allTests = Flatten[{
		...
		invalidCumulativeDilutionTests,
