...
convertCellCompositions[mySamplePackets : {PacketP[Object[Sample]]..}] := Module[
  ...
  (* NOTE: For any composition entries that are in the form {nonCellConcentrationP, ObjectP[], _} -> {CellConcentrationP, ObjectP[], _}. *)
  cellConcentrationLookup = Module[{allSampleCompositions, cellCompositionsInOtherUnits},
    ...
    cellCompositionsInOtherUnits = Cases[Flatten[allSampleCompositions, 1], {nonCellConcentrationP, ObjectP[], _}];

    (* If we have any cell compositions in other units, try to convert them to Cells / mL. *)
    If[Length[cellCompositionsInOtherUnits] > 0,
      Module[{convertedConcentrations, convertedSampleCompositions, convertedSampleCompositionsTime},
        ...
        convertedSampleCompositionsTime = cellCompositionsInOtherUnits[[All, 3]];
        ...
        MapThread[
          If[MatchQ[#2[[1]], Except[$Failed]],
            #1 -> {#2[[1]], #2[[2]], #3},
            Nothing] &,
          {cellCompositionsInOtherUnits, convertedSampleCompositions, convertedSampleCompositionsTime}]
      ],
      {}]
  ];
