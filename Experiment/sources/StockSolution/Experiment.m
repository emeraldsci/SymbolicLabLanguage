	...
	*)
				...
				Which[
					...
					Module[{formula, solvent, totalVolume, components, componentAmounts, scalingRatio, scaledComponentAmounts,
						...
						If[inEngine,
							...
							(* otherwise we're not in engine, don't change the input Volume, but report if it's fucked, and return tests *)
							{
								volumeOption,
								{},
								componentsOutOfRange,
	...
	resolvedpHingOptionsPerModel = MapThread[
        ...
        Lookup[pHOptions,{pH, MinpH, MaxpH, pHingAcid, pHingBase}];
				...
				resolvedMinpH = Which[
					NumericQ[nominalpH] && MatchQ[specifiedMinpH, Automatic] && NumericQ[Lookup[modelPacket, MinpH]], Lookup[modelPacket, MinpH],
					NumericQ[nominalpH] && MatchQ[specifiedMinpH, Automatic], nominalpH - 0.1,
				...
				resolvedMaxpH = Which[
					NumericQ[nominalpH] && MatchQ[specifiedMaxpH, Automatic] && NumericQ[Lookup[modelPacket, MaxpH]], Lookup[modelPacket, MaxpH],
					NumericQ[nominalpH] && MatchQ[specifiedMaxpH, Automatic], nominalpH + 0.1,
...
resolveStockSolutionOptionsFormula[
		...
		Transpose[MapThread[
			Function[{componentModels, componentPackets, solventPacketOrNull, componentAmounts, finalVolume, unitOpsQ, fillToVolumeMethod},
				Module[
					...
					(* honestly though this check is fucked; if someone wanted to make a stock solution of 50 mL water and 50 mL of ethanol and FtV to 99 mL,
	...
	(* get the storage condition symbols of all the components; be a bit paranoid in case of deprecated shit, maybe doesn't have SC *)
	...
	(* don't freeze if not asked; just do Refrigerator if we have shit colder than refrigerator *)
...
stockSolutionResourcePackets[
	...
	(* need to use Table and not ConstantArray because we want these to be distinct resources and not the same one (and Table will evaluate the Resource call every time) *)
	fixedAliquotResources = Flatten[MapThread[
		Function[{faComponents, scalingFactor, totalAmountPerComponent},
			If[MatchQ[faComponents, {}],
				Nothing,
				MapThread[
					Function[{faComponent,requestedAmount},
						Module[{amountToPick,numberOfResources},
							If[MatchQ[Lookup[faComponent, FixedAmounts],{(MassP|VolumeP)..}],
								...
								amountToPick=Which[
									...
									MatchQ[SelectFirst[Lookup[faComponent,FixedAmounts],#==requestedAmount&],MassP|VolumeP],
									SelectFirst[Lookup[faComponent,FixedAmounts],#==requestedAmount&],
									...
									MatchQ[Cases[Lookup[faComponent,FixedAmounts],LessP[requestedAmount]],{(MassP|VolumeP)..}],
									Last[Sort[Cases[Lookup[faComponent,FixedAmounts],LessP[requestedAmount]]]],
									(* if the requested volume is smaller than any of the FixedAmounts, just pick the smallest fixed amount *)
									True, First[Sort[Lookup[faComponent,FixedAmounts]]]
								];
								...
								Table[Resource[Sample -> Lookup[faComponent, Object], Amount -> amountToPick, Name->ToString[Unique[]], ExactAmount -> True], numberOfResources],
								Resource[Sample -> Lookup[faComponent, Object], Amount -> requestedAmount, Name->ToString[Unique[]]]
							]
						]
					],
