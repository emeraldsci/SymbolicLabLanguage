	...
	bufferLabelSamplePrimitives = DeleteDuplicates[MapThread[
		...
		If[Not[MemberQ[mySourcesToTransfer, #2]],
			...
			LabelSample[
				Sample -> #1,
				Label -> #2,
				Amount -> SafeRound[(#2 /. sourceLabelsToAmountRules) * 1.1, 1 Microliter],(* Round to the precision allowed by LabelSample *)
	...
	mixPrimitives = Flatten[MapThread[
		Function[{destLabel, destWell, destSampleLabel},
			If[MatchQ[preparation, Robotic] && VolumeQ[{destLabel, destWell} /. destinationAndWellToVolumeRules],
				Mix[
					Sample -> destSampleLabel,
					MixType -> Pipette,
					MixVolume -> SafeRound[
						Max[Min[({destLabel, destWell} /. destinationAndWellToVolumeRules) / 2, 970*Microliter], 1*Microliter],
						(* This needs to be the same level of precision as Incubate *)
						(10^(-1)) Microliter],
					NumberOfMixes -> 10
				],
				Nothing
		...
		Function[{pooledSamplePackets, options, maxResolutionAmounts, potentialAnalytes},
			Module[
				...
				totalSampleVolume = If[MatchQ[Cases[amounts, VolumeP], {}],
					Null,
					SafeRound[Total[Cases[amounts, VolumeP]], 10^-1 Microliter]
				];
