...
measureDensityResourcePackets[mySamples:{PacketP[Object[Sample]]..},myResolvedOptions:{_Rule..},myUnresolvedOptions:Alternatives[{_Rule..},{}],myOptions:OptionsPattern[]] := Module[
		...
		If[MemberQ[resolvedBalance, ObjectP[Object[Instrument, Balance]]],
			...
			(* otherwise we can be dealing with a list of balance models and that's fine *)
			{
				...
				Module[{handlingStationModels},
					(* get the handling station models - we only want non-specialized ambient handling stations *)
					handlingStationModels = Cases[DeleteDuplicates[Flatten[Lookup[Lookup[Experiment`Private`balanceHandlingStationLookup["Memoization"], #, {}], "Model", {}]& /@ ToList[resolvedBalance]]], Except[ObjectP[specializedHandlingStationModels["Memoization"]], ObjectP[Model[Instrument, HandlingStation, Ambient]]]];
	...
	densityMeterSampleNeedles = PadRight[
		Table[
			Link[Resource[Sample -> Model[Item, Needle, "id:Y0lXejrKm0Po"], Name -> ToString[Unique[]]]],(*"14Ga x 10.7In Disposable Blunt Tip Luer Lock Dispensing Needle"*)
