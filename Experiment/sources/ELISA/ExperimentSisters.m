...
DefineOptions[ExperimentELISAPreview,
...
ExperimentELISAPreview[myInputs:ListableP[ObjectP[{Object[Container],Object[Sample],Model[Sample]}]|_String],myOptions:OptionsPattern[ExperimentELISAPreview]]:=Module[
	...
	ExperimentELISA[myInputs,ReplaceRule[listedOptions,Output->Preview]]
];

(* ::Subsection:: *)
(*ExperimentELISAOptions*)

DefineOptions[ExperimentELISAOptions,
	Options:>{
		{
...
ExperimentELISAOptions[myInputs:ListableP[ObjectP[{Object[Container],Object[Sample],Model[Sample]}]|_String],myOptions:OptionsPattern[ExperimentELISAOptions]]:=Module[
	...
	(* Get only the options for ExperimentELISA *)
	options=ExperimentELISA[myInputs,Append[noOutputOptions,Output->Options]];

	(* Return the option as a list or table *)
	If[MatchQ[Lookup[listedOptions,OutputFormat,Table],Table],
		LegacySLL`Private`optionsToTable[options,ExperimentELISA],
		options
	]
];
...
DefineOptions[ValidExperimentELISAQ,
	Options:>
			{
				VerboseOption,
				OutputFormatOption
...
ValidExperimentELISAQ[myInputs:ListableP[ObjectP[{Object[Container],Object[Sample],Model[Sample]}]|_String],myOptions:OptionsPattern[ValidExperimentELISAQ]]:=Module[
	...
	(* Return only the tests for ExperimentELISA *)
	experimentELISATests=ExperimentELISA[myInputs,Append[preparedOptions,Output->Tests]];

	(* Define the general test description *)
	initialTestDescription="All provided options and inputs match their provided patterns (no further testing can proceed if this test fails):";

	(*Make a list of all of the tests, including the blanket test *)
	allTests=If[MatchQ[experimentELISATests,$Failed],
		...
		Module[
			...
			Flatten[{initialTest,experimentELISATests,voqWarnings}]
		]
	];
	...
	Lookup[RunUnitTest[<|"ValidExperimentELISAQ"->allTests|>,OutputFormat->outputFormat,Verbose->verbose],"ValidExperimentELISAQ"]
];
