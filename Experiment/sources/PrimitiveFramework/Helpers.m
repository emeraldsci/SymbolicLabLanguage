...
(* joinClauses is a helper function that takes a list of strings (parts), then 1)delete duplicates of the strings *)
(* 2)Depends on how many parts, either add "and" or ", and" to connect the parts, and change later parts to lower case if applicable, *)
(* 3)StringJoin all parts together as a single String starting with a capital letter. *)
...
joinClauses[inputClauses: {_String..}, ops: OptionsPattern[joinClauses]] := Module[
	{uniqueClauses, numberOfClauses, correctedCasedClauses, conjuction},

	(* Remove duplicates from input *)
	uniqueClauses = DeleteDuplicates[inputClauses];
	numberOfClauses = Length[uniqueClauses];

	(* Check option *)
	conjuction = Lookup[SafeOptions[joinClauses, ToList[ops]], ConjunctionWord];
	...
	correctedCasedClauses = If[EqualQ[numberOfClauses, 1],
