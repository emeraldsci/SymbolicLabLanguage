...
(* ::Section:: *)
(*ExperimentCoulterCount Sisters*)

(* ::Subsection:: *)
(*ValidExperimentCoulterCountQ*)


DefineOptions[ValidExperimentCoulterCountQ,
	Options :> {
		VerboseOption,
		OutputFormatOption
	},
	SharedOptions :> {ExperimentCoulterCount}
];
...
ValidExperimentCoulterCountQ[mySamples:ListableP[ObjectP[{Object[Container], Object[Sample], Model[Sample]}] | _String], myOptions:OptionsPattern[ValidExperimentCoulterCountQ]] := Module[
	...
	(* Return only the tests for ExperimentCoulterCount *)
	experimentCoulterCountTests = ExperimentCoulterCount[mySamples, Append[preparedOptions, Output -> Tests]];

	(* Define the general test description *)
	initialTestDescription = "All provided options and inputs match their provided patterns (no further testing can proceed if this test fails):";

	(*Make a list of all of the tests, including the blanket test *)
	allTests = If[MatchQ[experimentCoulterCountTests, $Failed],
		...
		Module[
			...
			Flatten[{initialTest, experimentCoulterCountTests, voqWarnings}]
		]
	];
	...
	Lookup[RunUnitTest[<|"ValidExperimentCoulterCountQ" -> allTests|>, OutputFormat -> outputFormat, Verbose -> verbose], "ValidExperimentCoulterCountQ"]
];
...
DefineOptions[ExperimentCoulterCountOptions,
	Options :> {
		{
...
ExperimentCoulterCountOptions[mySamples:ListableP[ObjectP[{Object[Container], Object[Sample], Model[Sample]}] | _String], myOptions:OptionsPattern[ExperimentCoulterCountOptions]] := Module[
	...
	(* Get only the options for ExperimentCoulterCount *)
	options = ExperimentCoulterCount[mySamples, Append[noOutputOptions, Output -> Options]];

	(* Return the option as a list or table *)
	If[MatchQ[Lookup[listedOptions, OutputFormat, Table], Table],
		LegacySLL`Private`optionsToTable[options, ExperimentCoulterCount],
		options
	]
];
...
DefineOptions[ExperimentCoulterCountPreview,
	SharedOptions :> {ExperimentCoulterCount}
];
...
ExperimentCoulterCountPreview[mySamples:ListableP[ObjectP[{Object[Container], Object[Sample], Model[Sample]}] | _String], myOptions:OptionsPattern[ExperimentCoulterCountPreview]] := Module[
	...
	ExperimentCoulterCount[mySamples, ReplaceRule[listedOptions, Output -> Preview]]
]