...
(* ::Subsubsection::Closed::*)
(*makeTestKFInstruments*)

(* need this function for tests here and elsewhere because as of writing this, we only have customer-owned versions of these instruments and so on stage those get blanked out *)
(* this function existingg in the source code and not the test code is a weird artifact of how reloading files and where tests look for their function definitions *)
(* since we're calling this function from many tests, it actually successfully finds everything more easily this way *)
makeTestKFInstruments[myFunction:(_Symbol | TypeP[])]:=Module[
	{funcString, volInst, volBench, coulInst, coulBench, allObjs, existingObjs, stickerSheet, volComputer, coulComputer},

	funcString = ToString[myFunction];

	allObjs = {
		Object[Container, Bench, "Test Volumetric Instrument Bench for " <> funcString <> " tests" <> $SessionUUID],
		Object[Container, Bench, "Test Coulometric Instrument Bench for " <> funcString <> " tests" <> $SessionUUID],
		Object[Instrument, KarlFischerTitrator, "Test Volumetric Instrument for " <> funcString <> " tests" <> $SessionUUID],
		Object[Instrument, KarlFischerTitrator, "Test Coulometric Instrument for " <> funcString <> " tests" <> $SessionUUID],
		Object[Item, Consumable, "Test Sticker Sheet for Coulometric Instrumetn for " <> funcString <> " tests" <> $SessionUUID],
		Object[Part, Computer, "Test Volumetric Instrument Computer for " <> funcString <> " tests" <> $SessionUUID],
		Object[Part, Computer, "Test Coulometric Instrument Computer for " <> funcString <> " tests" <> $SessionUUID]
	};
	existingObjs = PickList[allObjs, DatabaseMemberQ[allObjs]];
	EraseObject[existingObjs, Force -> True];

	(* make the benches the instruments will live on *)
	{volBench, coulBench} = Upload[{
		<|
			Type -> Object[Container, Bench],
			Model -> Link[Model[Container, Bench, "The Bench of Testing"], Objects],
			Name -> "Test Volumetric Instrument Bench for " <> funcString <> " tests" <> $SessionUUID,
			Site -> Link[$Site]
		|>,
		<|
			Type -> Object[Container, Bench],
			Model -> Link[Model[Container, Bench, "The Bench of Testing"], Objects],
			Name -> "Test Coulometric Instrument Bench for " <> funcString <> " tests" <> $SessionUUID,
			Site -> Link[$Site]
		|>
	}];

	{stickerSheet, volComputer, coulComputer} = UploadSample[
		{
			Model[Item, Consumable, "id:Y0lXejOz4oGx"], (* Model[Item, Consumable, "Sticker collection sheet for Coulometric Karl Fischer Titration Carousel"] *)
			Model[Part, Computer, "id:mnk9jOJDMPLw"], (* Model[Part, Computer, "Instrument PC with i5-13500, 32GB DDR5 RAM"] *)
			Model[Part, Computer, "id:mnk9jOJDMPLw"] (* Model[Part, Computer, "Instrument PC with i5-13500, 32GB DDR5 RAM"] *)
		},
		{
			{"Work Surface", coulBench},
			{"Work Surface", volBench},
			{"Work Surface", coulBench}
		},
		FastTrack -> True,
		Name -> {
			"Test Sticker Sheet for Coulometric Instrumetn for " <> funcString <> " tests" <> $SessionUUID,
			"Test Volumetric Instrument Computer for " <> funcString <> " tests" <> $SessionUUID,
			"Test Coulometric Instrument Computer for " <> funcString <> " tests" <> $SessionUUID
		}
	];

	{volInst, coulInst} = Upload[{
		<|
			Type -> Object[Instrument, KarlFischerTitrator],
			Name -> "Test Volumetric Instrument for " <> funcString <> " tests" <> $SessionUUID,
			Model -> Link[Model[Instrument, KarlFischerTitrator, "id:8qZ1VWkJqRJb"], Objects], (* Model[Instrument, KarlFischerTitrator, "Metrohm 901 Titrando"] *)
			Status -> Available,
			Software -> "OMNIS 4.0.1",
			VideoCaptureComputer -> Link[volComputer, Instruments]
		|>,
		<|
			Type -> Object[Instrument, KarlFischerTitrator],
			Name -> "Test Coulometric Instrument for " <> funcString <> " tests" <> $SessionUUID,
			Model -> Link[Model[Instrument, KarlFischerTitrator, "id:J8AY5jGM8rMj"], Objects], (* Model[Instrument, KarlFischerTitrator, "Metrohm 851 Titrando with generator electrode without diaphragm"] *)
			Status -> Available,
			StickerSheet -> Link[stickerSheet],
			Software -> "OMNIS 4.0.1",
			VideoCaptureComputer -> Link[coulComputer, Instruments]
		|>
	}];
	UploadLocation[{volInst, coulInst}, {{"Work Surface", volBench}, {"Work Surface", coulBench}}, FastTrack -> True];
