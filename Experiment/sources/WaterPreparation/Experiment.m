...
resolveExperimentWaterPreparationOptions[
...
]:=Module[
	{
		...
		waterModelToWaterSourceInstrumentLookup,
	...
	(* we create a look up for each unique water model to a single water source instrument model so that it is consolidated and only a single instrument resource is generated for each water model *)
	waterModelToWaterSourceInstrumentLookup = Map[
		Function[{waterModel},
			Module[{filteredWaterSourceInstruments, selectedWaterSourceInstrument},
				...
				selectedWaterSourceInstrument = RandomChoice[
				...
				waterModel->selectedWaterSourceInstrument
			]
		],
		DeleteDuplicates[Download[myWaterModels,Object]]
	];
		...
		Function[{waterModel, container, amount, manipulationIndex, options},
			Module[
				...
				waterSourceInstrument = If[MatchQ[Lookup[options,WaterSourceInstrument],Except[Automatic]],
						...
						Lookup[waterModelToWaterSourceInstrumentLookup, Download[waterModel, Object]]
				];
