...
DefineTests[DownstreamSampleUsage,
	{
		...
		Example[{Options,GraphLayout,"Use the GraphLayout option to display the graph in a radial or layered form:"},
			{
				DownstreamSampleUsage[
					Object[Sample, "Sample 6 for DownstreamSampleUsage tests "<>$SessionUUID],
		...
		Example[{Options,Output,"Output -> List returns a list of all samples into which the parent sample was transferred either directly or indirectly:"},
			DownstreamSampleUsage[
				Object[Sample, "Sample 9 for DownstreamSampleUsage tests "<>$SessionUUID],
				Output -> List
			],
			_List
		],
		...
		Example[{Messages,"NoTransfersOut","If a sample has not been transferred out of, a warning is shown:"},
			DownstreamSampleUsage[
				Object[Sample, "Sample 20 for DownstreamSampleUsage tests "<>$SessionUUID]
			],
			Null,
			Messages:>{DownstreamSampleUsage::NoTransfersOut}
		]
	},
	...
	SymbolSetUp :> (
		...
		Module[{objs, existingObjs},
			objs = Quiet[
				Cases[
					Flatten[{
						Object[Container, Bench, "Fake bench for DownstreamSampleUsage tests"],
						Table[Object[Container, Vessel, StringJoin["Container ",ToString[x]," for DownstreamSampleUsage tests ", $SessionUUID]], {x, 1,20}],
						Table[Object[Sample, StringJoin["Sample ",ToString[x]," for DownstreamSampleUsage tests ", $SessionUUID]], {x, 1,20}]
					}],
					ObjectP[]
				]
			];
		...
		Module[
			...
			fakeBench = Upload[
				<|Type -> Object[Container, Bench], Model -> Link[Model[Container, Bench, "The Bench of Testing"], Objects], Name -> "Fake bench for DownstreamSampleUsage tests", DeveloperObject -> True, StorageCondition -> Link[Model[StorageCondition, "Ambient Storage"]]|>
			];

			(* lets make several samples so that the graphs look good - do one sample with aliquotting, one with consolidation, and all with more than one transfer *)
			(* make a sample with no downstream *)
			(* create containers *)
			containers = ECL`InternalUpload`UploadSample[
				...
				Name -> Table[
					StringJoin["Container ",ToString[x]," for DownstreamSampleUsage tests ", $SessionUUID],
					{x, 1,20}
				]
			];
			...
			(* put samples into containers - start with a reasonable initial volume *)
			{
				...
				Name -> Table[StringJoin["Sample ",ToString[x]," for DownstreamSampleUsage tests ", $SessionUUID], {x, 1,20}]
			];
			...
			transferTuples = {
				(*an aliquot from 1 to 2-6*)
				{sample1, sample2, 10 Microliter},
				{sample1, sample3, 10 Microliter},
				{sample1, sample4, 10 Microliter},
				{sample1, sample5, 10 Microliter},
				{sample1, sample6, 10 Microliter},

				(*an aliquot from 6 to 7-9*)
				{sample6, sample7, 10 Microliter},
				{sample6, sample8, 10 Microliter},
				{sample6, sample9, 10 Microliter},
