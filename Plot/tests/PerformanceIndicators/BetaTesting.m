...
DefineTests[PlotBetaTesting,
	{
		...
		Example[{Options, UnusedProcedures, "If beta testing is not testing a subset of procedures, provide options to exclude those procedures:"},
			PlotBetaTesting[
				ExperimentImageSample,
				UnusedProcedures -> {Object[Qualification, SampleImager], "ImageSample Confirm No More Intense Containers"}
			],
			ObjectP[Object[EmeraldCloudFile]],
			TimeConstraint -> 600
		],
		...
		Example[{Messages, "TypeLookup", "The type corresponding to the experiment must be listed in Experiment`Private`experimentFunctionTypeLookup or in ProcedureFramework`Private`experimentFunctionTypeLookup in order to run the function:"},
			PlotBetaTesting[ExperimentScience],
			$Failed,
	...
	SymbolSetUp :> Module[
		{
			functionTestID, notebookPacket, tubePacket, unitTestSuitePacket, unitTestFunctionPacket, operatorPacket,
			tube1, tube2, tube3, tube4, notebook,unitTestSuite,unitTestFunction,operator, waterSample1, waterSample2, waterSample3,
			waterSample4, protocol1, protocol2, protocol3, protocol4, tsTickets, statusPackets, devObjectPackets, namePackets
		},
		...
		{tube1, tube2, tube3, tube4, notebook, unitTestSuite, unitTestFunction, operator} = Upload[
			{tubePacket, tubePacket, tubePacket, tubePacket, notebookPacket, unitTestSuitePacket, unitTestFunctionPacket, operatorPacket}
		];

		{waterSample1, waterSample2, waterSample3, waterSample4} = ECL`InternalUpload`UploadSample[
			ConstantArray[Model[Sample, "Milli-Q water"], 4],
			{{"A1", tube1}, {"A1", tube2}, {"A1", tube3}, {"A1", tube4}},
			InitialAmount -> {100 Microliter, 100 Microliter, 1200 Microliter, 1200 Microliter}
		];
		...
		protocol4 = ExperimentMeasureWeight[{waterSample3, waterSample4}, Name -> "PlotBetaTesting Test Protocol 4 "<>$SessionUUID];
