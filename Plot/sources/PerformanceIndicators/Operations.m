...
UploadOperationsStatistics[protocols:{ObjectP[{Object[Protocol],Object[Maintenance],Object[Qualification]}]..},ops:OptionsPattern[]]:=Module[
	{safeOps,upload,debug,protocolPackets,invalidProtocolPackets,validProtocolPackets,uploadPackets},

	safeOps=SafeOptions[UploadOperationsStatistics,ToList[ops]];
	{upload,debug}=Lookup[safeOps,{Upload,Debug}];

	protocolPackets = Quiet[
		Download[protocols,
			Packet[
				Status,
				DateRequested,
	...
	(* Only operate on complete protocols *)
	invalidProtocolPackets=Select[protocolPackets,(!MatchQ[Lookup[#,Status],Completed] || !MatchQ[Lookup[#,ParentProtocol],Null] || MatchQ[Lookup[#,StatusLog], {}])&];
	validProtocolPackets=Complement[protocolPackets,invalidProtocolPackets];
	...
	uploadPackets=calculateOperationsStatistics[#,safeOps]&/@validProtocolPackets;
...
calculateOperationsStatistics[protocolPacket:PacketP[{Object[Protocol],Object[Maintenance],Object[Qualification]}],ops:OptionsPattern[]]:=Module[{
	...
	safeValue,orderedValues,comparisonDescriptions,roundTime,problematicComparisons,problematicMessage},
	...
	{operationConstraintTime, waitingTimesBreakdown} = protocolWaitingTimeBreakdown[protocolPacket,statusTimesAfterStarting];
...
protocolWaitingTimeBreakdown[protocolPacket:PacketP[],parsedLog_]:=Module[{initialReadyCheckWaitingTimesBreakdown,
	readyCheckWaitingTimesBreakdown,readyCheckOperationConstraint,sciSupportTime,shippingMaterialsTime,
	repairingInstrumentationTime,operationConstraintTime,waitingTimesBreakdown},
