...
DefineOptions[PlotChromatography,
	Options :> {
		...
		},
		{
			...
			Default -> Medium,
			Description -> "The width of the Hamming window that is convolved with the Absorbance data before plotting. The setting can be Null (no convolution), one of Small, Medium, or Large - corresponding to 1, 2, or 4 seconds, or a numeric value between 0.0125 seconds and 5.0 seconds. No convolution is applied for data other than Absorbance.",
			AllowNull -> True,
			Category -> "Data Specifications",
			Widget -> Alternatives[
				Widget[
...
PlotChromatography[input:ListableP[ObjectP[Object[Data,Chromatography]]]|ListableP[ObjectP[Object[Data,ChromatographyMassSpectra]]],inputOptions:OptionsPattern[]]:=Module[
	...
	hammingWindowWidth=Replace[
		OptionDefault[OptionValue[HammingWindowWidth]],
		{
			Small :> 1 Second,
			Medium :> 2 Second,
			Large :> 4 Second
	...
	{result,options}=Which[
		...
		MatchQ[resolvedPlotType,ListLinePlot],Module[
			...
			updatedPackets = MapAt[applyHammingWindow[#, hammingWindowWidth]&, updatedPackets, {All, Key[Absorbance]}];
...
applyHammingWindow[qa_QuantityArray, size: ECL`GreaterP[0 ECL`Minute]] :=
	Module[{
		...
		hammingKernel = Table[0.54 - 0.46 Cos[2 Pi n / (windowPoints - 1)], {n, 1,windowPoints}];
