...
PlotqPCR[
	...
	If[!MatchQ[data, {ObjectP[Object[Data, qPCR]]..}],
		Message[Error::NoqPCRDataToPlot];
		Return[$Failed]
	];
	...
	previewPlot = If[MemberQ[output, Preview],
		PlotqPCR[data, Sequence @@ ReplaceRule[safeOps, Output -> Preview]],
		Null
	];
	...
	{plots, resolvedOptions} = If[MemberQ[output, (Result | Options)],
		Transpose[
			(PlotqPCR[#, Sequence @@ ReplaceRule[safeOps, Output -> {Result, Options}]]& /@ data) /. $Failed -> Nothing
		],
		{{}, {}}
	];
	...
	If[MatchQ[plots, (ListableP[{}] | ListableP[Null])] && MatchQ[previewPlot, (Null | $Failed)],
		Message[Error::qPCRProtocolDataNotPlotted];
		Return[$Failed],
		Nothing
	];
