...
(* Protocol Overload *)
PlotDissolvedOxygen[
	obj: ObjectP[Object[Protocol, MeasureDissolvedOxygen]],
	...
	safeOps=SafeOptions[PlotDissolvedOxygen, ToList[ops]];
	...
	If[!MatchQ[data, {ObjectP[Object[Data, DissolvedOxygen]]..}],
		Message[Error::NoDissolvedOxygenDataToPlot];
		Return[$Failed]
	];
	...
	previewPlot = If[MemberQ[output, Preview],
		PlotDissolvedOxygen[data, Sequence @@ ReplaceRule[safeOps, Output -> Preview]],
		Null
	];
	...
	{plots, resolvedOptions} = If[MemberQ[output, (Result | Options)],
		Transpose[
			(PlotDissolvedOxygen[#, Sequence @@ ReplaceRule[safeOps, Output -> {Result, Options}]]& /@ data) /. $Failed -> Nothing
		],
		{{}, {}}
	];
	...
	If[MatchQ[plots, (ListableP[{}] | ListableP[Null])] && MatchQ[previewPlot, (Null | $Failed)],
		Message[Error::MeasureDissolvedOxygenProtocolDataNotPlotted];
		Return[$Failed],
		Nothing
	];
