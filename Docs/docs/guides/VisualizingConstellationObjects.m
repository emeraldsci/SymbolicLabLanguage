...
Guide[
	Title -> "Visualizing Constellation Objects",
	...
	Reference -> {
		...
		"Visualizing Constellation Objects" -> {
			...
			{PlotCloudFile, "Provides an image or preview of an EmeraldCloudFile stored in Constellation."}
		},
		...
		"Plotting Extracted Data from Constellation" -> {
			...
			{PlotTableOptions, "Returns all the resolved options for when PlotTable is called on the specified input."}
		},
		...
		"Plotting Data Objects" -> {
			...
			{PlotFreezeCells, "Displays a temperature over time graph obtained from a cell freezing experiment."},
			...
			{PlotMicroscope, "Generates an image of the microscope data with frame labels demarking the real-world distances of the items seen in the image in the provided microscope data."},
		...
		"Plotting Analysis Objects" -> {
			...
			{PlotFitOptions, "Returns the list of resolved options for when PlotFit is called using the specified input."},
		...
		"Report Generation" -> {
			{ExportReport, "Given plots, tables, text or any other data, generates a new notebook page with that information, allowing for automatic report generation."}
		}
